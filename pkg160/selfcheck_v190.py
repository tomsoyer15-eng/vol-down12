# -*- coding: utf-8 -*-
"""Самопроверка ADD-FUT v1.6.0 ред. 30. Запуск из корня замороженного пакета данных после
его 10 PASS; файлы ред. 30 скопированы в корень. Корень доверия — SHA-256 ZIP.
...
ALLOW = ['ADD-FUT-v1_6_0-r30.pdf', 'sim_v164.py', 'sim_etf.py', 'mr_engine.py', 'transition.py',
...
try:
    entries = [(h, fn.strip()) for h, fn in
               (line.strip().split(maxsplit=1) for line in open('MANIFEST-190.txt', encoding='utf-8') if line.strip())]
    ...
    bad = [fn for h, fn in entries if fn != 'selfcheck_v190.py'
...
chk('МР: pre-OPEN ABORT отклоняется всегда (строгая привязка к OPEN, ред. 30)',
