...
def _default_base():
    """База по умолчанию — ПОСЛЕДНИЙ ВЫПУСК, а не origin/master.

    origin/master был подвижной мишенью: wip_push.sh из cron пушит master каждые полчаса, и
    сразу после коммита база СОВПАДАЕТ с HEAD. Ворота при этом честно печатали «проверять
    нечего» — то есть в самый обычный момент работы измеряли пустоту. Вопрос, на который
    ворота отвечают, звучит «что изменилось в этом круге правок», и естественный якорь для
    него — последний ВЫПУСК: он неподвижен, помечен в истории и означает «состояние, которое
    уже заверено корнем доверия».
    Если выпусков в истории нет (свежий клон без глубины), честнее сказать об этом и взять
    HEAD~1, чем молча вернуться к подвижной базе.
    """
    _r = subprocess.run(['git', 'log', '--grep=^Выпуск', '-1', '--format=%H'],
                        cwd=str(ROOT), capture_output=True, text=True)
    _sha = (_r.stdout or '').strip()
    if _r.returncode == 0 and _sha:
        return _sha
    print('ВНИМАНИЕ: коммит выпуска в истории не найден — база взята как HEAD~1; '
          'для круга правок укажите базу явно')
    return 'HEAD~1'
...
def main():
    ...
    if not diff:
        # ПУСТОЙ DIFF — ЭТО НЕ ПРОХОЖДЕНИЕ (27.08.2026, находка №15 сплошного аудита).
        # Здесь стоял `return 0`, и это третий fail-open того же семейства: отказ git и
        # отказ разбора этот файл уже закрыл, а законно пустой diff остался.
        # ПОЧЕМУ ЭТО НЕ ТЕОРИЯ. База по умолчанию была origin/master, а wip_push.sh из cron
        # пушит master в origin каждые полчаса, и я сам пушу после каждого коммита. Значит
        # ШТАТНОЕ состояние сразу после коммита — HEAD == origin/master, чистое дерево, и
        # ворота печатают «проверять нечего», выходя с нулём. Воспроизведено на чистом
        # клоне: diff пуст, код возврата 0, батарея и монитор не запускались вовсе.
        # Ворота, зеленеющие ничего не измерив, хуже отсутствующих: они снимают вопрос.
        print(f'изменённых строк питона против {base} нет.')
        print('ВОРОТА НИЧЕГО НЕ ИЗМЕРИЛИ — ЭТО НЕ ПРОХОЖДЕНИЕ. Проверять нечего либо база '
              'выбрана неверно (например, она уже содержит все правки круга). Укажите базу '
              'явно: tools/branch_cover.py <коммит>.')
        return 4
