...
PRODUCT = ROOT / 'r33build'      # что уезжает в пакет — то и судится боевым порядком
...
def _tier_of(path, seen):
    ...
    """
    if not str(Path(path)).startswith(str(PRODUCT)):
        return 'справочно', 'инструмент вне пакета: батарея его не грузит, мерить нечем'
    # ПОКРЫВАЕТСЯ СВОИМ ПРОГОНОМ, А НЕ БАТАРЕЕЙ (восьмой прогон /code-review). Три файла
    # пакета исполняются не батареей, а собственными запусками: selfcheck идёт при ВЫПУСКЕ
    # (правило 3), мутационный прогон — свой многочасовой запуск, contour_test — отдельный
    # сценарный скрипт. Судить их по батарее значит держать вечный красный на коде, который
    # на деле исполняется; молчать о них — та самая поблажка, за которую этот круг и
    # ругают. Поэтому: справочный ярус, но с ИМЕНЕМ ПРОГОНА, который обязан их покрыть.
    _OWN_RUN = {'selfcheck_v192.py': 'selfcheck выпуска (tools/release.py)',
                'mutation.py': 'мутационный прогон (live/mutation.py)',
                'contour_test.py': 'сценарный прогон (live/contour_test.py)'}
    _own = _OWN_RUN.get(Path(path).name)
    if _own:
        return 'справочно', f'исполняется не батареей, а своим прогоном: {_own}'
