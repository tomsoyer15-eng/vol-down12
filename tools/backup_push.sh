...
set -u
# ИСЧЕЗНУВШИЙ КАТАЛОГ ПРОЕКТА — НЕ УСПЕХ (27.08.2026, находка №11). Здесь стоял `exit 0`:
# если каталога нет, выгрузка не делается, а автопилот получает «всё хорошо». Молчание и
# успех обязаны различаться.
...
# КОД ВОЗВРАТА ГОВОРИТ ПРАВДУ (27.08.2026, находка №11 сплошного аудита).
# ЧТО БЫЛО: последней командой скрипта был `echo`, поэтому статус ВСЕГДА 0 — при любом
# исходе. Автопилот сбрасывает счётчик неудач, когда скрипт вернул успех, значит счётчик
# обнулялся КАЖДОЕ замыкание и порог «три отказа подряд = тревога» был НЕДОСТИЖИМ. Механизм
# существовал только на бумаге. Доказательство измерено, а не выведено: в журнале
# автопилота 16 строк «drive ПРОПУЩЕН» (16-17.08 — выгрузки не было вовсе), а файл
# ~/.addfut/push-fails не создавался НИ РАЗУ за всё время работы.
#
# ПРОПУСК ТЕПЕРЬ ТОЖЕ НЕНУЛЕВОЙ, И ЭТО ОСОЗНАННЫЙ ПЕРЕСМОТР РЕШЕНИЯ. Комментарий выше
# объявлял отсутствие rclone или удалёнки «не отказом выгрузки»; так было верно 16.08, пока
# rclone ещё не поставили. Сегодня он настроен, и его исчезновение означает ровно одно:
# копий вне машины нет. Оставить это нулём значит вернуть молчание, неотличимое от успеха,
# — тот самый класс, против которого заведена вся дисциплина отказов. Порог в три замыкания
# даёт запас на разовый сбой сети и поднимает тревогу на третьи сутки без копий.
# Коды разделены намеренно: 1 — выгрузка пыталась и не смогла, 2 — не пыталась вовсе,
# 3 — нет самого каталога проекта. В журнале это уже различимо строками.
exit "${_rc:-0}"
