...
СОБЫТИЯ_ВПЕРЕДИ = [                      # короткий календарь; прошедшее скрывается само
    ...
    # 04.09 перевод НЕ состоялся (журнальные акты и общее окно LSE/CME — см. §12);
    # перенесён на вторник 08.09: 05-06.09 выходные, 07.09 Labor Day.
    ('2026-09-07', 'Labor Day — биржа закрыта, контур пропустит день'),
    ('2026-09-08', 'ПЕРЕВОД ПИЛОТА НА МАРШРУТ Е — перенос с 04.09 (исполнитель 16:50)'),
...
def собрать():
    ...
    def нлв():
        # ЦИФРА НЕСЁТ СВОЮ ДАТУ (рецензия 31.08): раньше последний замкнутый NLV
        # подписывался номером ТЕКУЩЕЙ сессии — даже незамкнутой; читатель получал
        # вчерашнее число под сегодняшним номером.
        т = (ДОМ / 'autopilot.log').read_text(encoding='utf-8', errors='replace')
        м2 = re.findall(r"вход \{'spy_close'.*?'(\d{4}-\d{2}-\d{2})'", т)
        ц = re.findall(r'замкнуто: NLV закрытия ([\d,]+\.\d+)', т)
        return (ц[-1], м2[-1] if м2 else '?') if ц else ('?', '?')
    _нлв, _нлв_дата = безопасно(нлв, ('?', '?'))
    з.append(f'Счёт: NLV последнего ЗАМКНУТОГО дня ({_нлв_дата}): {_нлв} $')
    з.append(f'Текущая сессия №{сессия} от {посл}' + (' — ещё не замкнута'
             if str(посл) >= str(_нлв_дата) and _нлв_дата != '?' else ''))
