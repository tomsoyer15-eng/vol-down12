...
# ДОЛЛАР — ЭТО ДОЛЛАР, А НЕ НАЧАЛО ФОРМУЛЫ (найдено 26.08.2026 при сверке пакета с
# текстом). matplotlib по умолчанию трактует пару $...$ как mathtext, а нормативный
# текст содержит фрагменты оболочки: `[ "$v" -gt "$now" ]`, `${LINENO}`, `$ST`. Пара
# долларов в одной строке превращала кусок норматива в формулу, парсер ронял
# ParseException, и текст в PDF НЕ ПОПАДАЛ. Измерено: из 22 строк с долларом до PDF
# доходило 12. Документ записи терял ровно те места, где норматив цитирует код.
# ПОЧЕМУ ЭТО НЕ ЗАМЕЧАЛОСЬ ГОДАМИ ВЫПУСКОВ: падение печатается в stderr, но КОД
# ВОЗВРАТА ОСТАЁТСЯ НУЛЕВЫМ, и вызывающий, глядящий на код, видит успех. Потеря
# обнаруживается только сверкой содержимого PDF с исходником — её теперь делает
# release.py (_sverit_normativ).
# ПРОВЕРЕНО ОПЫТОМ 27.08 на трёх строках (падающая формула, разбираемая формула,
# строка без доллара): БЕЗ этой настройки в PDF не попадает НИ ОДНА из трёх, С нею —
# все три. Рецензия 27.08 утверждала, что matplotlib здесь не падает, а молча рисует
# формулу; на этом материале это НЕ ТАК — он падает, и терялось больше, чем один
# искажённый фрагмент.
