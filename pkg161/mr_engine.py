...
ЕДИНСТВЕННЫЙ ИСТОЧНИК ИСТИНЫ — журнал событий mr_journal.csv (одна атомарная запись на событие):
  SWITCH_SIGNAL,<цель>   — решение движка; фактический маршрут НЕ меняется;
...
KNOWN_EVENTS = ('SWITCH_SIGNAL', 'TRANSITION_OPEN', 'TRANSITION_COMPLETE', 'TRANSITION_ABORT',
                'TRANSITION_MIXED', 'TRANSITION_RESOLVED', 'GRANULARITY_EXCEPTION',
                'REDUCE_SIGNAL', 'REVIEW_SIGNAL')
...
def journal_rows(journal, asof=None):
    ...
    for i, r in enumerate(rows):
        ...
        need = {'SWITCH_SIGNAL': 2, 'TRANSITION_OPEN': 3, 'TRANSITION_COMPLETE': 3,
    ...
    if emerg and decision == HOLD:
        ...
        if m > 1.5:
            if not avail: decision, note = REDUCE, '20-дн >1,5, альтернатива недоступна'
                ...
                t2 = 'E' if g > 0 else 'F'
                if t2 != route and pending is None and not budget_exhausted(n_sw):
