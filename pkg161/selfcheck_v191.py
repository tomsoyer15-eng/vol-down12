# -*- coding: utf-8 -*-
"""Самопроверка ADD-FUT v1.6.0 ред. 31. Запуск из корня замороженного пакета данных после
его 10 PASS; файлы ред. 31 скопированы в корень. Корень доверия — SHA-256 ZIP.
...
ALLOW = ['ADD-FUT-v1_6_0-r31.pdf', 'sim_v164.py', 'sim_etf.py', 'mr_engine.py', 'transition.py',
...
try:
    entries = [(h, fn.strip()) for h, fn in
               (line.strip().split(maxsplit=1) for line in open('MANIFEST-191.txt', encoding='utf-8') if line.strip())]
    ...
    bad = [fn for h, fn in entries if fn != 'selfcheck_v191.py'
...
_cspx = b4.np['CSPX']
os.remove(SP)
M.append_event(JX, '2026-08-08', 'SWITCH_SIGNAL', 'F|s2'); M.grant_granularity(JX, SX, '2026-08-08', 's2', 38780.0)
...
chk('МР: pre-OPEN ABORT отклоняется всегда (строгая привязка к OPEN, ред. 31)',
