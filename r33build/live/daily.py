...
def run_session(broker, market, *, dirpath, route='F', band=None, cap=CAP_LEV,
    ...
    with ST.hold_book_lock(ST.book_lock_dir(bp)):
        ...
        if not dry_run and journal_path:
            try:
                J.verify(journal_path)
            except Exception as ex:
                raise RuntimeError(f'журнал §7 повреждён: {ex} — торговля запрещена '
                                   f'до починки (О-5)')
            _jr0 = J.read(journal_path)
            if _jr0 and not str(_jr0[-1].get('note', '')).startswith('итог сессии'):
                raise RuntimeError('журнал §7 не закрыт итоговой строкой предыдущей '
                                   'сессии — записи исполнения могли не записаться, '
                                   'ручной разбор (О-5)')
            if not _jr0 and int(sess or 0) > 0 and getattr(book, 'last_session', None):
                raise RuntimeError(
