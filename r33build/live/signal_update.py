...
def _commit_levels(live, pending):
    ...
    for sym, me in pending.items():
        for d, v in me.items():
            ...
            # ПУСТАЯ ЯЧЕЙКА — НЕ ЗАПИСАННЫЙ УРОВЕНЬ (27.08.2026, находка №34).
            # Здесь проверялось НАЛИЧИЕ КЛЮЧА, а pandas отдаёт пустую ячейку как NaN —
            # то есть ключ есть, значения нет, и ветка «уровень уже заморожен» срабатывала
            # на месяце, для которого уровень не записывали НИКОГДА. Пустые ячейки
            # возникают штатно: когда у двух ног разные наборы месяцев (источник отдал
            # разные окна для SPY и IEF), строка пишется общая, а ячейка одной ноги пуста.
            # СЛЕДСТВИЕ ДЕНЕЖНОЕ: уровень месяца не фиксируется, дальше _check_levels берёт
            # df[sym].dropna() — месяц исчезает из сверки и не попадает ни в проверку
            # дрейфа, ни в проверку дыр. А monthly_adjusted, в отличие от замороженного
            # движка (тот использует ffill), делает dropna: месяц не переносится, а
            # ВЫБРАСЫВАЕТСЯ, и rolling(12) усредняет 12 отсчётов, растянутых на 13
            # календарных месяцев. Это уже смещённая средняя, то есть другой сигнал.
            _прежний = rows.get(d, {}).get(sym)
            if _прежний is not None and _прежний == _прежний:   # не None и не NaN
                continue
...
def _verify_overlap(sym, col, ref, st):
    ...
    # ЗАПАС БЫЛ РОВНО НОЛЬ (27.08.2026, находка №30 сплошного аудита). Требовалось 12 из 12,
    # а двухлетнее окно источника даёт ровно 12 общих месяцев в штатном случае — измерено.
    # Достаточно ОДНОГО пропущенного месяца в живом ряду (а он пропускается штатно: при
    # пограничной тревоге месяц не записывается), чтобы перекрытие стало 11 и обновление
    # СТАЛО НЕВОЗМОЖНЫМ НАВСЕГДА: окно источника едет вперёд, разрыв не зарастает, и каждый
    # следующий месяц делает хуже. Контур встаёт без сигнала на здоровом ряду.
    # ЧТО ЗАЩИЩАЕТ ПРОВЕРКА: молча уехавший источник, который поменял бы стратегию без
    # ошибки. Эту силу даёт СРАВНЕНИЕ значений на общих месяцах, а не их точное ЧИСЛО:
    # разошедшийся источник не совпадёт и на шести месяцах. Поэтому порог смягчается до
    # пола, а само сравнение остаётся на всех общих месяцах, сколько бы их ни было.
    _ПОЛ = 6
    if len(overlap) < _ПОЛ:
        raise SignalError(f'{sym}: перекрытие с живым рядом {len(overlap)} мес из 12 — '
                          f'источник слишком короток или ряд разрежен')
    if len(overlap) < 12:
        # НЕ МОЛЧА: сокращённое перекрытие — признак пропущенного месяца, и оператор должен
        # знать, что история живого ряда неполна, даже если обновление прошло.
        print(f'ВНИМАНИЕ: {sym}: перекрытие с живым рядом {len(overlap)} мес из 12 — '
              f'в живом ряду пропущены месяцы; сверка идёт по общим, обновление '
              f'продолжается, но историю следует восстановить (О-5)')
...
def _update_locked(ib, LIVE, pd):
    # РЯД СВЕРЯЕТСЯ С DIGEST ДО ЧТЕНИЯ (27.08.2026, находка №31 сплошного аудита).
    # Здесь ряд читался сразу, проверялись только 12 последних строк перекрытия, а затем
    # файл переписывался ЦЕЛИКОМ и заверялся СВЕЖИМ digest. Любая правка старых строк —
    # неудачная запись, восстановление из устаревшего зеркала, ручное вмешательство —
    # проходила через обновлятор и получала новое заверение: тревога подмены, ради которой
    # digest и заведён, не срабатывала НИКОГДА, и следа не оставалось.
    # ПОЧЕМУ ЭТО НЕ ТЕОРИЯ: автопилот запускает signal_update ПЕРЕД первой торговлей месяца
    # (autopilot.sh), то есть обновлятор видит испорченный файл ПЕРВЫМ — раньше сторожа,
    # который отказался бы торговать. Из ~760 строк ряда проверкой перекрытия покрыты 12.
    # Один бит ряда открывает или закрывает целую ногу порядка NLV.
    # Сверка живёт отдельной функцией и fail-closed: отсутствие или пустота сайдкара у
    # ЖИВОГО ряда — тоже отказ, а не разрешение.
    import feed as _FD31
    # «ЖИВОСТЬ» ОПРЕДЕЛЯЕТСЯ ТАК ЖЕ, КАК В feed.signal_state, А НЕ ОБЪЯВЛЯЕТСЯ ЗДЕСЬ.
    # Первая редакция ставила live_series=True безусловно — и уронила ПЯТНАДЦАТЬ стендов,
    # которые строят ряд во временном файле без сайдкара. Это не придирка стендов: по
    # правилу самой сверки (её докстринг) явный путь — ручной разбор и стенды — сайдкара
    # штатно не имеет, а обязателен он именно у ЖИВОГО ряда, то есть у файла по пути из
    # ADDFUT_SIGNALS/~/.addfut. Объявлять живым что угодно значит переносить правило из
    # одного места в два и ждать, когда они разойдутся.
    # ВЫРАЖЕНИЕ ПОВТОРЯЕТ feed.signal_state ДОСЛОВНО, включая expanduser вместо Path.home():
    # различие проявилось бы ровно там, где HOME подменён, то есть на стендах и в разборе.
    _живой = pathlib.Path(os.environ.get('ADDFUT_SIGNALS')
                          or (pathlib.Path(os.path.expanduser('~/.addfut')) / 'signals_live.csv'))
    try:
        _есть_живой = pathlib.Path(LIVE).resolve() == _живой.resolve()
    except OSError:
        _есть_живой = False
    # ПЕРВИЧНОЕ ЗАПОЛНЕНИЕ РЯДА ОБЯЗАНО БЫТЬ ВОЗМОЖНЫМ (28.08.2026, рецензия, находка №3).
    # Вчерашняя правка требовала отпечаток ДО чтения, а создаёт его _ensure_digest на
    # тридцать четыре строки НИЖЕ — то есть единственный код, который заверяет ряд, стоял
    # за воротами, требующими заверения. Замкнутый круг: оператор делает ровно то, что
    # написано в docstring update() («посеять из пакетного до первого запуска»), копирует
    # data/signals_monthly.csv — у него нет сайдкара — и обновление падает НАВСЕГДА. Тот же
    # тупик после восстановления из WORM-архива, где сайдкар не обязателен. Это уже
    # случалось живьём 16.08.2026 (PROJECT-STATE §7), и автопилот превращает такой отказ в
    # ALARM — месяц без торговли.
    # РАЗЛИЧАЮТСЯ ДВА РАЗНЫХ СОБЫТИЯ, КОТОРЫЕ ВЧЕРА БЫЛИ ОДНИМ:
    #   сайдкара НЕТ вовсе      -> ряд никогда не заверялся: первичное заполнение, ЗАКОННО;
    #   сайдкар ЕСТЬ, но не сошёлся -> ряд правили мимо signal_update, ОТКАЗ.
    # Сила проверки от этого не теряется: она заведена против ПОДМЕНЫ СТРОК заверенного
    # ряда, а не против его отсутствия. Отсутствие всё равно заперто с другой стороны —
    # feed.signal_state отказывается ТОРГОВАТЬ по незаверенному живому ряду, поэтому
    # незаверенное состояние не переживёт этот же запуск: _ensure_digest ниже его закроет.
    _сайдкар = pathlib.Path(str(LIVE) + '.sha256')
    _заверен = _сайдкар.exists() and _сайдкар.read_text(encoding='utf-8').strip() != ''
    if _есть_живой and not _заверен:
        # ГРОМКО: молчаливое разрешение здесь неотличимо от снятой защиты.
        print(f'ВНИМАНИЕ: {LIVE}: живой ряд не заверён (нет {_сайдкар.name}) — принимаю как '
              f'ПЕРВИЧНОЕ ЗАПОЛНЕНИЕ и заверю по завершении. Если ряд не новый, это признак '
              f'утраты сайдкара: сверьте содержимое с последним WORM-архивом (О-5)')
    _FD31.verify_signal_digest(LIVE, live_series=(_есть_живой and _заверен))
