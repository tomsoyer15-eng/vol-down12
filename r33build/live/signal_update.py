...
def _поколение_сайдкара(rows, sym, me):
    """Множитель, переводящий текущий ряд поставщика в ПОКОЛЕНИЕ уже записанных уровней.

    САЙДКАР ОБЯЗАН БЫТЬ ОДНОГО ПОКОЛЕНИЯ (01.09.2026, живой отказ контура). Ряд
    ADJUSTED_LAST — полной доходности: каждая выплата двигает ВСЮ историю на общий
    множитель, и _check_levels именно поэтому нормирует на медианный k. Но месяц,
    дописанный ПОСЛЕ выплаты, замораживался в новом поколении, а прежние строки оставались
    в старом: сайдкар становился двухпоколенным, и следующая же сверка объявляла
    расхождение — 01.09 август ушёл на +0,361% при допуске 0,1%, ровно на один купон IEF,
    и месячное обновление сигнала стало падать КАЖДЫМ запуском. Для ежемесячного
    плательщика это отказ каждого месяца после первой дозаписи.

    Множитель считается ТЕМ ЖЕ способом, что в _check_levels (медиана отношений по общим
    месяцам). Сила сверки не меняется: она сравнивает месяцы между собой, а не с
    абсолютной ценой. На пустом сайдкаре — первичное заполнение — множитель равен единице.
    ОТДЕЛЬНОЙ ФУНКЦИЕЙ, А НЕ ЧЕТЫРЬМЯ СТРОКАМИ В ЦИКЛЕ: у парной мутации должна быть одна
    точка приложения, иначе защита ненаблюдаема (правило 8в, угол «от отрицания»)."""
    зам = [(me.loc[d], rows[d][sym]) for d in me.index
           if rows.get(d, {}).get(sym) is not None
           and rows.get(d, {}).get(sym) == rows.get(d, {}).get(sym)]
    отн = sorted(нов / стар for нов, стар in зам)
    return отн[len(отн) // 2] if отн else 1.0
...
def _commit_levels(live, pending):
    ...
    for sym, me in pending.items():
        # НОВЫЙ УРОВЕНЬ ЗАМОРАЖИВАЕТСЯ В ПОКОЛЕНИИ УЖЕ ЗАПИСАННЫХ (01.09.2026) — см.
        # _поколение_сайдкара.
        _k = _поколение_сайдкара(rows, sym, me)
        for d, v in me.items():
            ...
            if _прежний is not None and _прежний == _прежний:   # не None и не NaN
                continue
            rows.setdefault(d, {})[sym] = float(v) / _k
