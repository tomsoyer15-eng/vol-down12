...
def _update_locked(ib, LIVE, pd):
    ...
    try:
        ...
        _есть_живой = False
    # ПОД УЖЕ ВЗЯТЫМ ЗАМКОМ: update() держит LOCK_EX (строка 278), а flock не
    # рекурсивен — обычный вход завесил бы процесс на самом себе.
    _FD31.verify_signal_digest_под_замком(LIVE, live_series=_есть_живой)
