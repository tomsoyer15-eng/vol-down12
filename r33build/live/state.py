...
def book_from_broker(cls, positions, route, *, ser_a=None, ser_b=None, unit_is_mes=True,
    ...
    for k, v in pos.items():
        ...
        if _r == 'MES':
            ...
            # НЕ `elif _r == 'ZN'` (седьмой прогон, ворота 8в-1): сюда доходят только
            # корни из FUT_ROOTS — посторонние позиции отвергнуты проверкой alien выше,
            # а MES и ES разобраны ветками рядом. Условие было тождественно истинным и
            # потому вечно однобоким: ветка есть, а выбора нет.
