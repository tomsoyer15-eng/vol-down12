...
def active_route():
    ...
    # ПУСТОЙ ФАЙЛ — ЭТО ОТСУТСТВИЕ МАРШРУТА, А НЕ МАРШРУТ «» (27.08.2026, находка №20).
    # Здесь стояло `... if rt.exists() else None`, то есть проверялось СУЩЕСТВОВАНИЕ, а не
    # содержимое: файл нулевой длины (или из одних пробелов) давал _file = '' — значение
    # НЕ None, поэтому громкий отказ «маршрут неизвестен» не срабатывал, и наружу уходила
    # пустая строка.
    # ЧЕМ ЭТО ПЛОХО ИМЕННО ЗДЕСЬ: дальше по пустому маршруту строится путь книги
    # (book_path('') -> book-.json), load возвращает None, и оператор получает диагноз
    # «книга отсутствует — якорь не о чем писать». То есть при потере МАРШРУТА ему
    # сообщают об утрате КНИГИ и посылают чинить не то. Пустой route.txt в этом проекте
    # не гипотеза: он уже дважды приводил к тревогам (ALARM-route-2026-08-15 и -16).
    # Оболочка ту же ситуацию ловит верно — `[ -s "$ROUTE_F" ]` (autopilot.sh:33), то есть
    # правило разъехалось между двумя реализациями одного решения. Приводим питон к нему.
    _raw = rt.read_text(encoding='utf-8').strip() if rt.exists() else None
    _file = _raw or None
    _jrn = None
    try:
        import sys as _s
...
def check_one_series(pos):
    ...
    for k in pos:
        ...
        # НОГА ВНЕ АЛФАВИТА — ТОЖЕ ОТКАЗ (девятый прогон): вселенная ног здесь зашита
        # {'А','Б'}, и корень с любой третьей ногой ронял голый KeyError вместо разбора
        # О-5 — причём внутри hand_over_book, то есть уже ПОСЛЕ перевода денег.
        if _lg is not None and _lg not in _ser:
            raise ValueError(
                f'позиция {k!r}: корень {_CT.fut_root(k)!r} отнесён к ноге {_lg!r}, '
                f'которой книга не знает (есть только А и Б) — книга не строится (О-5)')
        if _lg is None:
...
def book_from_broker(cls, positions, route, *, ser_a=None, ser_b=None, unit_is_mes=True,
    ...
    for k, v in pos.items():
        ...
        # СИММЕТРИЧНО ПО ОБЕИМ НОГАМ (девятый прогон /code-review). Предыдущая редакция
        # спрашивала таблицу ног только для Б и отвергала неизвестный корень только на А —
        # то есть дыру, которую она закрывала, оставляла открытой с другой стороны:
        # ПРОВЕРЕНО ИСПОЛНЕНИЕМ, с четвёртым корнем ZB:'Б' позиции {'ZNU26':10,'ZBU26':7}
        # давали n_b=17, и ZB считался ZN — по нему шли дюрация, полоса, кап §1 и ролл.
        # Книга умеет ровно три корня, потому что для каждого знает МОДЕЛЬНУЮ ЕДИНИЦУ
        # (сетка ноги А: 1 ES = 10 MES; нога Б: 112000 x 0,88 x D_fix у ZN). Корень без
        # единицы книге не построить — значит отказ, а не догадка, на ЛЮБОЙ ноге.
        _r = _CT.fut_root(k)
        if _r == 'MES':
            ...
            raise ValueError(
                f'позиция {k!r}: корень {_r!r} объявлен в каноне (нога '
                f'{_CT.fut_leg(k)!r}), но модельной единицы для него в книге нет — '
                f'ни сетка ноги А (ES/MES), ни единица ноги Б (ZN) его не описывают; '
                f'книга не строится, ручной разбор (О-5)')
