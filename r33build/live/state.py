...
def book_lock_dir(path=None):
    ...
    Правило простое и одно: замок принадлежит книге, значит выводится из её пути.

    ПРАВИЛО ЖИВЁТ В УМОЛЧАНИИ hold_book_lock, А НЕ В ВЫЗЫВАЮЩИХ (разбор /code-review).
    В первой редакции я перевёл на новый помощник ДВА вызова из пяти — и переходный
    исполнитель (transition._execute_locked), единственное место, где книга существует
    РАЗОРВАННОЙ, и согласованный снимок WORM остались на прежнем голом замке. Чинить
    правило перечислением вызывающих нельзя: следующий вызывающий снова окажется забыт.
    Теперь голый hold_book_lock() САМ выводит каталог отсюда, и «два писателя — один
    замок» держится без участия того, кто его берёт.

    МАРШРУТ ДЛЯ ОТВЕТА НЕ НУЖЕН. Прежняя обёртка book_path_for звала active_route() ДО
    book_path — то есть требовала route.txt даже там, где ADDFUT_BOOK_PATH задан и маршрут
    всё равно был бы отброшен, и взятие замка могло упасть RuntimeError на ровном месте.
    Когда переменной нет, книга лежит в каталоге состояния, а маршрут стоит в ИМЕНИ файла,
    ...
    if path is not None:
        ...
        # всегда есть каталог.
        # ТРЕБОВАНИЕ — АБСОЛЮТНЫЙ ПУТЬ, а не «есть каталог» (разбор /code-review 22.08).
        # Проверка на Path('.') ловила 'book-F.json', но 'sub/book-F.json' — такой же
        # cwd-зависимый замок, просто на уровень глубже. Правило одно и звучит честно:
        # относительный путь к книге зависит от того, КТО и ОТКУДА запустил процесс, а
        # замок, зависящий от cwd, не исключает второго писателя.
        _p = Path(path)
        if not _p.is_absolute():
            raise ValueError(
                f'book_lock_dir ждёт АБСОЛЮТНЫЙ путь книги, а получил {path!r} — '
                f'относительный замок зависит от текущего каталога запустившего и не '
                f'исключает второго писателя')
        return _p.parent
    ...
    if not env:
        return lock_dir()
    # ЛЬГОТА ПО ТОЧКЕ УБРАНА (разбор /code-review 22.08): `not startswith('.')` пропускала
    # './book-F.json' — самую естественную шелл-форму относительного пути — и замок молча
    # уходил в текущий каталог, а явная ветка тот же вход ОТВЕРГАЛА: две ветки одного
    # правила противоречили друг другу. Правило теперь одно на обе: только абсолютный путь.
    _pe = Path(env)
    if not _pe.is_absolute():
        raise ValueError(
            f'ADDFUT_BOOK_PATH={env!r} относительный — замок книги зависел бы от текущего '
            f'каталога запустившего процесса; задать полный путь')
    return _pe.parent
