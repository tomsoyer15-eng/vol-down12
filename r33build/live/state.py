...
def book_lock_dir(path=None):
    ...
    единственное.
    """
    _src = path if path is not None else os.environ.get('ADDFUT_BOOK_PATH')
    if _src is None or _src == '':
        return lock_dir()
    _p = Path(_src)
    if not _p.is_absolute():
        raise ValueError(
            f'book_lock_dir ждёт АБСОЛЮТНЫЙ путь книги, а получил {_src!r} — '
            f'относительный замок зависит от текущего каталога запустившего процесса и '
            f'не исключает второго писателя; задать полный путь')
    return _p.parent
