...
def journal_path(route):
    """ПУТЬ ЖУРНАЛА §7 — ОДНОЙ ФУНКЦИЕЙ НА ПИСАТЕЛЯ И ВСЕХ ЧИТАТЕЛЕЙ (рецензия 20.08).

    Путь строился в семи местах двумя разными правилами: session.state_dir() (уважает
    ADDFUT_DIR) и state.lock_dir() (не уважает). При заданном ADDFUT_DIR они расходятся, и
    расхождение НЕ шумит, а тихо меняет смысл: торговый вход не находит журнал, читает []
    и объявляет ЛЮБОЙ штатный повтор дня «неполной сессией» (контур встаёт), а счётчик
    дневной квоты перехода возвращается к старому счёту по своему файлу прогресса — то есть
    правка №11 молча отменяется. ADDFUT_DIR — документированный режим оператора, его же
    специально сбрасывает selfcheck; значит и уважать его обязаны ВСЕ, либо НИКТО.
    Уважают все: ADDFUT_DIR, иначе каталог замка.
    """
    env = os.environ.get('ADDFUT_DIR')
    d = Path(env) if env else lock_dir()
    return d / f'journal-{route}.csv'


def book_lock_dir(path=None):
    """КАТАЛОГ ЗАМКА КНИГИ — ОДИН ДЛЯ ВСЕХ ЕЁ ПИСАТЕЛЕЙ (СОРОК ПЯТЫЙ КРУГ, №8).

    Правило простое и одно: замок принадлежит книге, значит выводится из её пути; правило
    живёт в УМОЛЧАНИИ hold_book_lock, а не в перечислении вызывающих. Путь обязан быть
    АБСОЛЮТНЫМ: относительный зависит от cwd запустившего процесса, и два писателя одной
    книги разошлись бы по разным flock.

    ОДНА ПРОВЕРКА НА ОБЕ ВЕТКИ (пятый прогон /code-review). Явный путь и ADDFUT_BOOK_PATH
    проверялись двумя рукописными raise, и они уже расходились однажды (льгота по точке
    пропускала './book-F.json' ровно в одной ветке). Источник нормализуется, правило —
    единственное.
    """
    # ПУСТОТА ЗАКОННА ТОЛЬКО У ОКРУЖЕНИЯ (шестой прогон /code-review, доказано зондом):
    # незаданная/пустая переменная — штатное «не задано», а ЯВНЫЙ пустой аргумент — ошибка
    # вызывающего, и молчать о ней значит вернуть класс №8 через вырожденный вход. Слияние
    # веток сплющило эту асимметрию: book_lock_dir('') молчал, book_lock_dir(Path('')) падал.
    if path is None:
        _src, _откуда = os.environ.get('ADDFUT_BOOK_PATH'), "ADDFUT_BOOK_PATH"
        if not _src:
            return lock_dir()
    else:
        ...
        # СТРОКОЙ, А НЕ repr'ом Path (восьмой прогон): сообщение печатало
        # «= PosixPath('book-F.json')», то есть предлагало оператору задать переменную
        # равной репрезентации объекта, которого он не писал.
        _src = str(path)
        # СРАВНЕНИЕ НОРМАЛИЗОВАННЫХ ФОРМ (восьмой прогон): book_path(route) отдаёт
        # Path(env), и str() съедает './' и двойные слэши — на формах './book-F.json' и
        # 'a//b/book-F.json' сырое сравнение строк было ложным, и сообщение снова называло
        # источником аргумент. Форма './имя' не выдумана: она названа в комментарии выше
        # как реально встречавшийся вход.
        _откуда = ('ADDFUT_BOOK_PATH'
                   if _env_bp and str(Path(path)) == str(Path(_env_bp))
                   else 'аргумент book_lock_dir')
    _p = Path(_src)
    if not _p.is_absolute():
        raise ValueError(
            f'{_откуда} = {_src!r} не абсолютный путь книги — относительный (или пустой) '
            f'замок зависит от текущего каталога запустившего процесса и не исключает '
            f'второго писателя; задать полный путь')
    return _p.parent
...
def _lock_fd(d):
    ...
    flock запирает ОТКРЫТЫЙ ФАЙЛ, то есть inode, а не имя. Пока замком служил
    addfut-book.lock, его замена — восстановлением каталога из копии, unlink'ом или
    атомарным os.replace — давала второму процессу НОВЫЙ inode: он честно брал СВОЙ замок,
    пока первый держал старый, и оба шли подавать заявки и сохранять одну книгу. Сверка
    «имя всё ещё ведёт к моему inode» этого НЕ закрывает, и это стоило отдельного зонда: у
    второго процесса имя и inode согласованы, расходятся они только у ПЕРВОГО, который в
    этот момент никого не спрашивает. Взаимность даёт лишь общий объект.

    Каталог рядовой операцией не подменить: восстановление кладёт файлы ВНУТРЬ него, tar
    состояния .lock-файлы вовсе не берёт, а перенос самого ~/.addfut запрещён сторожем
    разрушительных команд. Отдельной функцией — чтобы у правила была ОДНА точка мутации.
    """
    return os.open(str(d), os.O_RDONLY)


def _lock_target(dirpath=None):
    """КАКОЙ КАТАЛОГ БУДЕТ ЗАПЕРТ — ОТДЕЛЬНОЙ ФУНКЦИЕЙ, БЕЗ ПОБОЧНЫХ ДЕЙСТВИЙ.

    Умолчание — каталог книги (см. book_lock_dir); приоритет ADDFUT_LOCK_DIR сохранён,
    поэтому стенд, изолирующий машинное состояние одной переменной, продолжает изолировать
    всё. Вынесено из hold_book_lock, чтобы правило можно было ПРОВЕРИТЬ, не беря замок:
    стенд «замок книги один на всех писателей» звал hold_book_lock() по-настоящему, и под
    мутацией «замок не зависит от книги» тот честно писал отметку держателя в ~/.addfut —
    то есть проверка правила 5 нарушала правило 5 ровно в момент поимки дефекта.
    """
    return lock_dir(dirpath if dirpath is not None else book_lock_dir())
...
def hold_book_lock(dirpath=None, timeout_s=30):
    ...
    # ОБЪЕКТ ЗАМКА — КАТАЛОГ (см. _lock_fd, 44-й круг, №9): подмена файла замка больше не
    # разводит двух держателей по разным inode.
    # ЧАСЫ МОНОТОННЫЕ (тот же круг, №10, семья та же): перевод системных часов назад
    # растягивал ожидание на неопределённый срок, вперёд — обрывал законное ожидание.
    d = _lock_target(dirpath)
    ...
    try:
        ...
        while True:
            try:
                ...
                time.sleep(0.2)
        # ОТМЕТКА ДЕРЖАТЕЛЯ — ДЛЯ ЧЕЛОВЕКА, А НЕ ДЛЯ ВЗАИМНОСТИ (её держит каталог).
        # Поэтому неудача записи отметки не отменяет уже взятый замок: остаться без
        # взаимного исключения хуже, чем без подсказки оператору.
        try:
            (d / LOCK_NAME).write_text(f'{os.getpid()}\n', encoding='utf-8')
        except OSError:
            pass
        yield
    finally:
...
def check_one_series(pos):
    ...
    for k in pos:
        ...
        if _lg is None:
            # КОРЕНЬ БЕЗ НОГИ — ОТКАЗ, А НЕ ПРОПУСК (восьмой прогон). `if _lg:` молчал на
            # None, и сторож «одна серия на ногу» слеп для любого корня, объявленного в
            # FUT_ROOTS, но не отнесённого к ноге; заодно нестроковый ключ (conId вместо
            # имени) переставал ронять трассировку и просто выпадал из проверки.
            raise ValueError(
                f'позиция {k!r}: корень {_CT.fut_root(k)!r} не отнесён ни к одной ноге — '
                f'сторож «одна серия на ногу» его не проверяет, книга не строится (О-5)')
...
def book_from_broker(cls, positions, route, *, ser_a=None, ser_b=None, unit_is_mes=True,
    ...
    for k, v in pos.items():
        # НОГА — ПО ТАБЛИЦЕ, ОСТАТОК — ОТКАЗ (восьмой прогон /code-review). Седьмой круг
        # заменил `elif _r == 'ZN'` на голый `else` с доводом «сюда доходят только корни из
        # FUT_ROOTS». Довод верен, а вывод — нет: ЧЕТВЁРТЫЙ корень тоже пройдёт alien и
        # молча ляжет в ногу Б. Проверено исполнением: с FUT_ROOTS+('ZB',) позиции
        # {'ZNU26':10,'ZBU26':7} давали n_b=17, то есть книга считала ногу Б трежерис-
        # фьючерсом, которого там нет, а по ней идут дюрация, полоса, кап §1 и ролл.
        _r = _CT.fut_root(k)
        if _CT.fut_leg(k) == 'Б':
            ...
            raise ValueError(
                f'позиция {k!r}: корень {_r!r} отнесён к ноге А, но сетка ноги А знает '
                f'только ES и MES — множитель нового корня неизвестен, книга не строится '
                f'(О-5)')
