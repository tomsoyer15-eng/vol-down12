...
def verify_signal_digest(p, live_series):
    ...
    """
    # ЗАМКА ЗДЕСЬ БОЛЬШЕ НЕТ — ЭТО ПРАВКА УДАЛЕНИЕМ (28.08.2026, правило 8е).
    # ИСТОРИЯ МЕХАНИЗМА, РАДИ КОТОРОЙ КОММЕНТАРИЙ И НАПИСАН. Замок завели 27.08 против
    # гонки публикации: ряд и его сайдкар выкладываются двумя os.replace, и читатель,
    # попавший в зазор, поднял бы самую громкую тревогу системы на ровном месте. Гонка была
    # ТЕОРЕТИЧЕСКОЙ — ни разу не наблюдалась. Дальше механизм жил так:
    #   (1) через три часа он завесил батарею НАСМЕРТЬ: писатель держит LOCK_EX и зовёт
    #       сверку, которая берёт LOCK_SH вторым дескриптором; flock не рекурсивен;
    #   (2) починка раздвоила вход — и появилась ВТОРАЯ гонка: замок отпускался ДО разбора,
    #       то есть заверялись одни байты, а торговали другими (рецензент воспроизвёл);
    #   (3) плюс риск повесить торговую сессию: писатель держит замок через круговые запросы
    #       к IBKR (измерено 6,1 с), а читатель ждал без таймаута и без записи в журнал.
    # Итог: защита от события, которого не было, трижды создала событие, которое было.
    # ЛЕЧЕНИЕ ДЕШЕВЛЕ ЗАЩИТЫ. Гонка возможна лишь потому, что байты читались ДВАЖДЫ: раз для
    # отпечатка, раз для разбора. Читаем ОДИН раз и возвращаем прочитанное — разбирается
    # ровно то, что заверено, а между двумя чтениями нечему меняться, потому что второго
    # чтения нет. Замок не нужен: ни самоблокировки, ни ожидания, ни зазора.
    # ВОЗВРАТ ЕДИНООБРАЗЕН НА ВСЕХ ВЕТКАХ: функция ВСЕГДА отдаёт байты ряда. Иначе читатель
    # на одной из веток снова прочитал бы файл сам, и гонка вернулась бы через чёрный ход.
    import hashlib as _hl
    ...
    _данные = Path(p).read_bytes()
    if not _want:
        return _данные          # заверения нет и не требуется: пакетный снимок, стенд, явный path
    _got = _hl.sha256(_данные).hexdigest()
    if _want != _got:
...
def signal_state(today, path=None, holidays=None):
    ...
    # РАЗБИРАЕТСЯ ТО, ЧТО ЗАВЕРЕНО, А НЕ ФАЙЛ ЗАНОВО (28.08.2026). Второе чтение с диска
    # и было единственной причиной гонки: между хэшем и разбором файл успевал смениться.
    _байты = verify_signal_digest(p, live_series=(path is None and p == live))
    df = pd.read_csv(io.BytesIO(_байты), parse_dates=[0])
