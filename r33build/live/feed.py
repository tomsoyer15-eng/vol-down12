...
def _gap_tolerance_applies(expected_prev, min_prev):
    """ДЕЙСТВУЕТ ЛИ ПЛОСКИЙ ДОПУСК В КАЛЕНДАРНЫХ ДНЯХ — ОТДЕЛЬНОЙ ФУНКЦИЕЙ.

    Правило простое: допуск нужен, только когда календарь НЕ ЗАДАН НИ ОДНОЙ границей;
    любая заданная граница точнее допуска. Вынесено из условия ради ОДНОЙ точки мутации:
    пока правило жило внутри `if`, парная мутация ему была негде, и подмена «известен
    календарь» на «задан expected_prev» прошла в бой незамеченной — маршрут Е получил
    детерминированный отказ на 29.12.2026 (разрыв шесть дней при допуске пять).
    """
    return expected_prev is None and min_prev is None
...
def closes(ib, contract, today, expected_prev=None, min_prev=None):
    ...
    # min_prev — ТОЖЕ ИЗВЕСТНЫЙ КАЛЕНДАРЬ (сорок пятый круг, разбор /code-review). Условие
    # смотрело только на expected_prev, поэтому перевод полосы единицы фонда на min_prev
    # (правка №5 того же круга) молча вернул плоский пятидневный допуск на маршрут Е — и
    # ровно на 29.12.2026, дату из комментария выше. Правило одно: допуск действует, когда
    # НЕ ЗАДАНА НИ ОДНА граница; заданная граница любой строгости точнее допуска.
    if _gap_tolerance_applies(expected_prev, min_prev) and gap > MAX_BAR_GAP_D:
...
def verify_signal_digest(p, live_series):
    ...
    """
    # ЗАМОК НЕ БЕРЁТСЯ ПОВТОРНО ТЕМ, КТО ЕГО УЖЕ ДЕРЖИТ (27.08.2026, разбор ПОВИСШЕЙ
    # батареи; угол «от чужой правки»). Правка №33 научила читателя брать разделяемый flock,
    # правка №31 научила ПИСАТЕЛЯ сверять отпечаток перед чтением — и писатель зовёт сверку
    # ИЗ-ПОД своего исключительного замка. flock не рекурсивен: второй дескриптор на тот же
    # файл в том же процессе ждёт освобождения первого, то есть процесс блокируется сам о
    # себя НАВСЕГДА. Обнаружено не рассуждением, а тем, что батарея встала на 53 минуты:
    # wchan=locks_lock_inode_wait, два дескриптора на одном sig.lock. В бою это остановило бы
    # обновление сигнала — то есть контур, — и молча, без единой строки в журнале.
    # Поэтому вход теперь два: этот берёт замок, `verify_signal_digest_под_замком` — нет.
    # Развилки внутри одной функции сознательно НЕ делаю: флаг «я уже под замком» пришлось бы
    # передавать через три вызова, и ошибиться в нём было бы так же легко.
    # ПОД `except OSError` ИДЁТ ТОЛЬКО ОТКРЫТИЕ ФАЙЛА, А НЕ ЗАХВАТ (27.08.2026, найдено
    # собственным зондом). Пока сюда попадал и flock, поглощалось ЛЮБОЕ прерывание ожидания:
    # TimeoutError — подкласс OSError, и мой же отрицательный зонд из-за этого доложил
    # «блокировки нет» там, где она была. Тот же глушитель скрыл бы и сигнал оператора,
    # снимающего зависший процесс. Законный откат к чтению без замка — это невозможность
    # ОТКРЫТЬ файл замка (нет прав, нет каталога); всё остальное обязано быть слышно.
    import fcntl as _fc
    _lk = None
    try:
        _lk = open(str(Path(p).with_suffix('.lock')), 'a+')
    except OSError:
        _lk = None
    if _lk is not None:
        _fc.flock(_lk, _fc.LOCK_SH)
    try:
        return verify_signal_digest_под_замком(p, live_series)
    finally:
        if _lk is not None:
            _fc.flock(_lk, _fc.LOCK_UN)
            _lk.close()


def verify_signal_digest_под_замком(p, live_series=False):
    """Та же сверка, но БЕЗ захвата замка — для вызова из кода, который замок уже держит.

    ЗАЧЕМ ОТДЕЛЬНОЕ ИМЯ, А НЕ ФЛАГ: имя видно на месте вызова. Читая signal_update, сразу
    ясно, что сверка идёт под уже взятым замком; флаг же виден только в объявлении.
    """
    import hashlib as _hl
    _dp = Path(str(p) + '.sha256')
    if not (_dp.exists() or live_series):
        return
    # ПОЧЕМУ ЗАМОК ВООБЩЕ НУЖЕН (находка №33):
    # ряд и его сайдкар публикуются ДВУМЯ отдельными os.replace: сначала signals_live.csv,
    # затем .sha256. signal_update держит на это время flock на signals_live.lock, а
    # читатели не брали ничего. Читатель, попавший МЕЖДУ двумя публикациями, считает digest
    # НОВОГО ряда и сверяет со СТАРЫМ сайдкаром — получает несовпадение и поднимает самое
    # громкое сообщение системы: «ряд правили мимо signal_update, торговля запрещена (О-5)».
    # На ровном месте, из-за порядка в доли секунды.
    # СЛУЧАЙ ДОСТИЖИМ БЕЗ ЭКЗОТИКИ: оператор запускает signal_update руками, чтобы снять
    # пограничную тревогу (ADDFUT_SIGNAL_CONFIRM в автопилоте запрещён env_guard'ом), а
    # cron тикает каждые десять минут.
    # ПОРЯДОК ПУБЛИКАЦИИ НЕ МЕНЯЕТСЯ — при любом порядке окно остаётся, только зеркальное.
    # Разделяемый замок дешевле: писатель держит его исключительно, читатели друг другу не
    # мешают. Отсутствие замка (нет прав, другой каталог) не должно ронять торговлю: тогда
    # читаем как раньше — это возврат к прежнему поведению, не ухудшение.
    return _verify_signal_digest_locked(p, live_series, _dp, _hl)


def _verify_signal_digest_locked(p, live_series, _dp, _hl):
    """Тело сверки под замком. Отдельной функцией — чтобы у замка была одна точка входа и
    ни один ранний return не унёс управление мимо освобождения."""
