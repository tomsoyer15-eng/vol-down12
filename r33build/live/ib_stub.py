...
class _WhatIf:
    """Ответ whatIfOrder. ПУСТОЙ ОТВЕТ БЫВАЕТ ДВУХ ФОРМ, и обе обязаны читаться как
    «отложить», а не как ноль:
      * OrderState с ПУСТЫМИ строками — этот класс (предпросмотр недоступен);
      * ПУСТОЙ СПИСОК вместо объекта — наблюдалось на живом шлюзе 18.08.2026, когда пресет
        счёта переопределяет TIF заявки (предупреждение 10349). Прежде докстрока называла
        единственной формой пустую строку, и это утверждение было неверным: адаптер семь
        кругов подряд получал в бою именно список, а стенд такой формы не знал.
    Список отдаёт сам StubIB.whatIfOrder при заявке без явного TIF."""
...
class StubIB:
    ...
    def accountSummary(self, account=None):
        ...
        if self.behaviour == 'neg_maint':
            ...
            return out
        if self.behaviour == 'margin_call':
            # ГЛУБОКИЙ ДЕФИЦИТ, cushion НИЖЕ 1,0 (сорок третий круг, №3). Стаб умел только
            # «тонко» (1,20), и ранний барьер preview `cushion < 1.0` не наблюдался ничем —
            # а именно он запирал аварийный выход Е→Ф при фактическом маржин-колле, то есть
            # ровно тогда, когда книгу надо разгружать.
            maint = self._nlv / 0.85
        elif self.behaviour == 'thin_cushion':
            maint = self._nlv / 1.2
    ...
    def whatIfOrder(self, contract, order):
        ...
        """
        # ПРЕСЕТ СЧЁТА ЛОМАЕТ ОТВЕТ, ЕСЛИ TIF НЕ ЗАДАН ЯВНО (18.08.2026, живой шлюз).
        # Стаб отвечал ВСЕГДА, и потому дыра прожила семь кругов: preview() на боевом шлюзе
        # возвращал «отложить» при любом плане, а батарея была зелёной. Стенд обязан
        # воспроизводить ПРИЧИНУ отказа, а не только его следствие («пусто»), иначе удаление
        # строки _o.tif не ловится ничем. Живой шлюз отдаёт именно ПУСТОЙ СПИСОК.
        # ФОРМА ЗАЯВКИ ЗАПОМИНАЕТСЯ (сорок четвёртый круг, саморецензия). Без этого правило
        # «предпросмотр спрашивает про ТУ заявку, что уйдёт в рынок» не наблюдаемо ничем:
        # опыт показал, что подмена GTC+outsideRth на DAY не роняет ни одного утверждения.
        ...
        # ДРОБНОСТЬ ОТВЕРГАЕТСЯ ДЛЯ ЛЮБОГО ИНСТРУМЕНТА, А НЕ ТОЛЬКО ДЛЯ ФЬЮЧЕРСА
        # (11.09.2026, живой отказ перехода Ф->Е). Здесь стояло условие secType == 'FUT', и
        # ровно из-за него батарея была ЗЕЛЁНОЙ, пока боевой шлюз отвергал дробную заявку
        # по фонду: IBKR отвечает ошибкой 10243 «Fractional-sized order cannot be placed
        # via API» и ПУСТЫМ whatIf на CSPX так же, как на фьючерс. Измерено в тот день на
        # живом шлюзе: 1204 доли — маржа 286 443, 1 доля — 238, 1204,96 — пусто и 10243.
        # Пока стаб был мягче рынка, «дробность фондов» выглядела рабочей предпосылкой —
        # тот же класс ложного доказательства, что и с TIF 18.08 («стаб отвечал ВСЕГДА»).
        _q0 = abs(float(getattr(order, 'totalQuantity', 0) or 0))
        if abs(_q0 - round(_q0)) > 1e-9:
            return _WhatIf(None)
