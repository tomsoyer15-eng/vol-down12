    ...
    def preview(self, orders=None, emergency=False):
        ...
        try:
            ...
            if _worst <= 0:
                ...
                # сейчас мало.
                # Целевая книга требует не больше текущей: каждое приращение неположительно.
                # Это и есть то, что можно доказать, и этого достаточно, чтобы РАЗРЕШИТЬ
                # разгрузку. Флаг emergency передаётся исполнителем и здесь не нужен для
                # решения — он остаётся в подписи, чтобы вызывающий мог назвать намерение, а
                # смысл ветки не зависел от него.
