...
class IBBroker:
    ...
    def _o3e_gate(self, cushion, emergency, done_all, moves_money, why):
        """ВОРОТА НОРМАТИВА О-3-Е — ОДНИ НА ВСЕ ЧЕТЫРЕ ОТКАЗНЫЕ ВЕТКИ preview().

        Разбор /code-review 21.08 показал, что я вынес наружу только СРАВНЕНИЕ, а два
        законных исключения (аварийный выход и «покупать больше нечего») оставил в одной
        ветке из четырёх. Итог зеркален дефекту, который они чинили: аварийный выход Е→Ф,
        чей остаток округлился в ноль контрактов, отвергался веткой «план не содержит
        исполнимых заявок» — ровно при маржинальном стрессе, ради которого выход и нужен.
        Исключения принадлежат ПРАВИЛУ, а не месту; здесь они и живут.

        РАЗЛИЧИТЕЛЬ — ДВИГАЮТСЯ ЛИ ДЕНЬГИ, а не название ветки. Там, где заявок нет или все
        они округлились в ноль, разрешение не тратит ни доллара: отказ ничего не бережёт, а
        через три POSTPONED даёт MIXED на неразорванной книге. Там, где заявки есть и они
        требуют маржи, норматив держится безусловно — и отдельный жёсткий пол 1,0 ниже.

        emergency НЕ ПРОПУСКАЕТ ТАМ, ГДЕ ПЛАНА НЕТ ВОВСЕ (moves_money=False, done_all=False):
        это путь «переход объявляется завершённым, хотя ничего не делалось», и он обязан
        оставаться закрытым, иначе аварийность становится отмычкой к COMPLETE на пустом
        месте. Признак «работа кончилась» приносит вызывающий (done_all) — он один это знает.
        """
        if self._o3e_ok(cushion):
            return True
        if moves_money:
            if emergency:
                # Заявки есть, но их знак освобождает маржу: запрет здесь равен запрету на
                # спасение книги (сорок второй и сорок третий круги).
                self._preview_pass_why = (
                    f'{why}: запас {float(cushion):.2f}x ниже норматива, но это АВАРИЙНЫЙ '
                    f'выход — он маржу освобождает')
                return True
            return False
        if done_all or emergency:
            self._preview_pass_why = (
                f'{why}: запас {float(cushion):.2f}x ниже норматива, но денег этот шаг не '
                f'двигает — {"покупать больше нечего" if done_all else "это АВАРИЙНЫЙ выход"}; '
                f'отказ дал бы MIXED на неразорванной книге')
            return True
        return False
    ...
    def preview(self, orders=None, emergency=False, done_all=False):
        ...
        if not orders:
            ...
            if self._o3e_gate(_c, emergency=False, done_all=done_all, moves_money=False,
                              why='плана нет'):
                return True
        ...
        try:
            ...
            if not _ims:
                ...
                if self._o3e_gate(_c, emergency, done_all, moves_money=False,
                                  why='план не содержит исполнимых заявок'):
                    return True
            ...
            if _worst <= 0:
                ...
                if self._release_by_measure(orders):
                    return True
                if self._o3e_gate(_c, emergency, done_all, moves_money=True,
                                  why='приращения маржи неположительны'):
                    return True
