...
class IBBroker:
    ...
    def _o3e_ok(self, cushion):
        """ЗАПАС НЕ НИЖЕ НОРМАТИВА О-3-Е — ОДНОЙ ФУНКЦИЕЙ НА ВСЕ ВЕТКИ preview().

        Сравнение `float(c) >= O3E_MIN` стояло в четырёх местах одного метода (беспланная
        ветка, план без исполнимых заявок, разгрузка по знаку приращений, худшая оценка), и
        у правила не было общей точки мутации: подмена норматива в одной ветке оставалась
        ненаблюдаемой в трёх остальных. Любое будущее уточнение норматива теперь тоже
        делается один раз, а не находится в трёх ветках двухсотстрочного метода.
        """
        import daily as _DLo
        return float(cushion) >= _DLo.O3E_MIN
    ...
    def preview(self, orders=None, emergency=False, done_all=False):
        ...
        if not orders:
            ...
            if emergency or done_all:
                self._preview_pass_why = (
                    f'запас {float(_c):.2f}x ниже норматива О-3-Е {_DLc.O3E_MIN}, но '
                    f'{"это АВАРИЙНЫЙ выход — он маржу освобождает" if emergency else "покупать больше нечего: план исполнен целиком"}; '
