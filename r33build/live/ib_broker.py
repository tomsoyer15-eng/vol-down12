...
def _kolichestvo_whatif(instr, qty):
    """Количество, которое предпросмотр отправляет в шлюз: ЦЕЛОЕ для ЛЮБОГО инструмента.

    11.09.2026, ЖИВОЙ ОТКАЗ ПЕРЕХОДА Ф->Е. Фонды были исключены из округления «потому что
    UCITS-ETF торгуются долями», и предпросмотр уходил в IBKR дробной заявкой — 1204,96
    доли CSPX. Шлюз отвечает ошибкой 10243 «Fractional-sized order cannot be placed via
    API» и ПУСТЫМ whatIf, то есть предпросмотр объявляется недоказуемым, переход
    откладывается, и после трёх попыток ЗАКОННЫЙ перевод ушёл бы в ABORT.
    Измерено на боевом шлюзе в тот же день: 1204 доли — маржа 286 443, одна доля — 238,
    1204,96 — пусто плюс 10243. Цена округления мизерна: остаток меньше одной доли (около
    827 $, 0,08% счёта), тогда как неделимость ES и так даёт разрыв в сотни тысяч.

    ОТДЕЛЬНОЙ ФУНКЦИЕЙ — ради ОДНОЙ точки приложения парной мутации (правило 8в). Пока
    правило жило строкой внутри preview, мутация «вернуть дробность фондам» правила только
    аргументы и ничего не меняла: округление стояло ниже по тому же телу, и защита была
    ненаблюдаема — поймано первым же прогоном мутаций."""
    ...
    def preview(self, orders=None, emergency=False, done_all=False):
        ...
        try:
            ...
            for _inst, _qty in orders:
                ...
                # ОКРУГЛЯЕМ ВСЁ, ВКЛЮЧАЯ ФОНДЫ (11.09.2026, живой отказ перехода Ф->Е) —
                # правило вынесено в _kolichestvo_whatif ради ОДНОЙ точки приложения парной
                # мутации: пока оно жило строкой в цикле, мутация «вернуть дробность фондам»
                # ничего не меняла, потому что округление стояло ВНУТРИ preview.
                # Здесь фонды были исключены из округления «потому что торгуются долями» —
                # и предпросмотр уходил в шлюз ДРОБНОЙ заявкой (1204,96 доли CSPX). IBKR
                # отвечает ошибкой 10243 «Fractional-sized order cannot be placed via API»
                # и ПУСТЫМ whatIf: предпросмотр становится недоказуем, переход
                # откладывается, и после трёх попыток законный перевод ушёл бы в ABORT.
                # Измерено на боевом шлюзе в тот же день: 1204 доли — маржа 286 443,
                # 1 доля — 238, 1204,96 — пусто плюс ошибка 10243. Дробность фондов
                # оказалась предпосылкой, которой программный интерфейс не поддерживает;
                # цена округления — остаток меньше одной доли (около 827 $ по CSPX, 0,08%
                # счёта), тогда как неделимость ES и так даёт разрыв в сотни тысяч.
                # Соседнее правило «округляем ровно так же, как торгует исполнитель» этим
                # не нарушено, а восстановлено: предпросмотр обязан описывать заявку,
                # которую шлюз ПРИМЕТ.
                _qty = _kolichestvo_whatif(_inst, _qty)
                if not _qty:
                    continue
                ...
                while time.time() - t0 < 30:
                    ...
                    if t.orderStatus.status in TERMINAL_BAD + TERMINAL_OK:
                        ...
                        if _st0 == 'Inactive' and not done:
                            # ЗДЕСЬ БЫЛ raise, И ОН ДЕЛАЛ ХУЖЕ, ЧЕМ ДЕФЕКТ (28.08.2026,
                            # рецензия, находка №2; правило 8е).
                            # ФАКТ ОСТАЁТСЯ ВЕРНЫМ: «Cancelled» при Inactive пишет клиентская
                            # библиотека, а не шлюз (ib_insync ib.py:699-706), и заявку
                            # теоретически могут активировать на открытии площадки. Вчера я
                            # сделал из этого отказ.
                            # ЧТО ИЗ ЭТОГО ВЫШЛО, ИЗМЕРЕНО РЕЦЕНЗЕНТОМ НА СТАБЕ: отказ
                            # попадает в список «неснятых» в _cancel_all, restore_to видит
                            # непустой stuck и возвращается ДО подачи компенсирующих заявок.
                            # То есть после ЛЮБОГО отказа брокера (а Inactive — обычное
                            # оформление отказа у IBKR) книга остаётся разорванной: на ролле
                            # это нога А целиком, вне рынка до утра. Прежний код её
                            # восстанавливал.
                            # СЧЁТ РИСКОВ: оживание Inactive — событие ни разу не
                            # наблюдавшееся; голая нога после отказа — событие, которое
                            # наступает при каждом отказе. Защита от первого создавала второе.
                            # ЧТО ОСТАЁТСЯ ОТ ПРАВКИ: неопределённость НЕ замалчивается. Она
                            # уходит в запись отдельным признаком и печатается оператору —
                            # знание сохранено, а решение «останавливать ли контур» больше не
                            # принимается в отменялке заявок за все вызывающие пути.
                            print(f'ВНИМАНИЕ: заявка {oid}: была Inactive, отмену подтвердил '
                                  f'КЛИЕНТ, а не шлюз; отчётов об исполнении нет. Книга '
                                  f'восстанавливается, но заявку следует проверить вручную '
                                  f'на открытии площадки (О-5)')
                            return dict(terminal=True, cancelled=True, отмена_клиентская=True,
