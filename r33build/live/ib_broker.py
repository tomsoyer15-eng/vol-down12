...
# ОШИБКИ КОДА — ИЗ ОДНОЙ ТОЧКИ (state.CODE_ERRORS, сведено рецензией 19.08). ЛОВИМ ЧЕРЕЗ
# МОДУЛЬ (`except _STce.CODE_ERRORS`), а не через связанный при импорте алиас: `CODE_ERRORS
# = _STce.CODE_ERRORS` связывает ОБЪЕКТ-кортеж в момент импорта, и подмена единственной
# точки в state.py уже ничего не меняет ни здесь, ни в worm_anchor — то есть «одна точка»
# была бы словом, а не механизмом, и парная мутация правила оказалась бы пустой (найдено
# рецензией 19.08 экспериментом: setattr(state, 'CODE_ERRORS', …) не менял поведения).
import state as _STce

# ПОРОГ ЧАСОВОГО «НЕ ПОСЧИТАНО» (рецензия 19.08): всё, что по модулю не меньше этого, —
# UNSET_DOUBLE или его родня, а не величина маржи. Берём с запасом на порядок ниже
# 1.797e308, чтобы поймать и половинные/суммарные формы, оставаясь заведомо выше любого
# мыслимого требования по счёту (мировой ВВП ~1e14).
UNSET_DOUBLE_MIN = 1e300
...
class IBBroker:
    ...
    def unit_ref(self, instrument, cls, at_close=False):
        ...
        if _is_fund:
            ...
            if px is None:
                ...
                # а свежее — законно.
                # НИЖНЯЯ ГРАНИЦА НУЖНА И БЕЗ at_close (разбор /code-review 21.08): правка
                # №5 45-го круга сняла плоский пятидневный допуск через min_prev, но подала
                # его только на замыкающей ветке, и при at_close=False обе границы были
                # None — допуск включался снова, на той же дате 29.12.2026, только уже при
                # проверке ПЛАНА перехода.
                # СЧИТАЕТСЯ ЗДЕСЬ, А НЕ ВЫШЕ (разбор /code-review 22.08): вынесенный к
                # началу метода вызов заставлял ходить в биржевой календарь ВСЕХ, включая
                # ветки с живой котировкой и ветки ES/ZN, которые эту величину не читают, —
                # и с 2027 года, где таблица праздников ЕС кончается, предполёт запирал
                # аварийный выход Е→Ф на значении, которое сам же выбрасывал.
    ...
    def _venue_prev(self, is_fund, today):
        ...
        import feed as _FDv
        if is_fund:
            # ГОД НАЗАД, А НЕ ВПЕРЁД (рецензия 20.08). prev_session шагает НАЗАД, поэтому
            # праздники следующего года недостижимы по построению, а нужны праздники
            # ПРЕДЫДУЩЕГО: на первой сессии января предыдущая европейская сессия лежит в
            # декабре, где таблица держит 24, 25, 28 и 31 декабря. Со старым (year, year+1)
            # 4 января 2027 «предыдущей сессией» объявлялся бы 31 декабря — день, который
            # сама же таблица считает нерабочим: closes(expected_prev=…) поднял бы
            # [STALE_BAR], то есть ровно отказ №8, переехавший на стык годов. Отсутствие
            # прошлого года в таблице — законно (первый год эксплуатации), поэтому откат
            # мягкий; отсутствие ТЕКУЩЕГО — кричит calendar_horizon.
            try:
                _hol = _FDv.eu_holidays(today.year - 1, today.year)
    ...
    def _o3e_ok(self, cushion):
        """Норматив О-3-Е — ОДИН на весь слой; здесь только доступ к нему из адаптера."""
        import daily as _DLo
        return _DLo.o3e_ok(cushion)

    def _o3e_gate(self, cushion, done_all, why):
        """ВОРОТА НОРМАТИВА О-3-Е ДЛЯ ВЕТВЕЙ, ГДЕ ДЕНЬГИ НЕ ДВИГАЮТСЯ.

        Разбор /code-review 22.08 снял отсюда признак аварийности, и это отказ от моей же
        вчерашней правки. Причина в комментарии выше по методу, написанном 38-м кругом:
        ОТРИЦАТЕЛЬНАЯ СУММА ПРИРАЩЕНИЙ — НЕ ДОКАЗАТЕЛЬСТВО. Каждая целевая заявка
        просматривается против ВСЁ ЕЩЁ СУЩЕСТВУЮЩЕЙ исходной книги, и неттинг с активами,
        которые только будут проданы, даёт минусы, исчезающие после продажи. Значит в ветке
        `_worst <= 0` доказательства разгрузки нет; его даёт независимый файловый замер
        (_release_by_measure) — вот законная дверь аварийного выхода, открытая 42-м и 43-м
        кругами. Пропуская там аварийность мимо замера, я убрал и её, и жёсткий пол 1,0:
        заявки, ТРЕБУЮЩИЕ маржи, уходили в рынок при запасе 0,05x.

        Осталось одно исключение и одна ветка. Плана нет, потому что покупать больше нечего
        (это знает вызывающий, у него st['done'] и executed_usd) — тогда отказ не бережёт
        ни доллара, а через три POSTPONED даёт MIXED на ЦЕЛОЙ книге. Всё прочее судит
        норматив.
        """
        if self._o3e_ok(cushion):
            return True
        if done_all:
            self._preview_pass_why = (
                f'{why}: запас {float(cushion):.2f}x ниже норматива, но покупать больше '
                f'нечего — отказ дал бы MIXED на неразорванной книге')
            return True
        return False
    ...
    def preview(self, orders=None, emergency=False, done_all=False):
        ...
        self._preview_pass_why = ''            # причина ПРОПУСКА вопреки запасу (см. ниже)

        def _no(why):
            # ЧЕРЕЗ МЕТОД, ЧТОБЫ У ПРАВИЛА БЫЛА ТОЧКА МУТАЦИИ (рецензия 19.08): замыкание
            # подменить нечем, и заявленная защита «каждый отказ называет причину» имела
            # НУЛЕВОЕ покрытие — мутация в пустую строку оставляла батарею зелёной.
            return self._preview_no(why)
        ...
        if not orders:
            # БЕЗ ПЛАНА РЕШАЕТ НОРМАТИВ О-3-Е, А НЕ ЕДИНИЦА (СОРОК ПЯТЫЙ КРУГ, №2, P0).
            # Здесь стояло: ниже 1,0 — отказ, иначе разрешение. Порог 1,0 не значит ничего:
            # норматив О-3-Е требует 1,40, и книга с ЖИВЫМ запасом 1,20 получала «да».
            # Дорогой путь — не гипотетический: при полностью исполненном, но ещё не
            # завершённом resume pv_remainder честно отдаёт ПУСТОЙ план, вызывающий зовёт
            # preview БЕЗ плана, и переход получает COMPLETE при фактическом запасе ниже
            # норматива. Финальный gross() это не ловит: он сверяет МОДЕЛЬНОЕ плечо, а не
            # живое требование маржи у брокера. После закрытия европейского окна
            # автоматический О-3-Е книгу уже не сократит — она уходит в ночь пробитой.
            import daily as _DLc
            if self._o3e_ok(_c):
                return True
            # УЖЕСТОЧЕНИЕ НЕ СМЕЕТ ЗАПЕРЕТЬ ДВА ЗАКОННЫХ ПУТИ (разбор /code-review 45-го
            # круга, угол «от противоположного знака»). Порог 1,40 без плана я поставил
            # безусловным — и этим закрыл ровно то, что 42-й и 43-й круги открывали:
            #
            # (1) АВАРИЙНЫЙ ВЫХОД Е->Ф. Признак emergency доходит сюда и был проигнорирован.
            #     Выход из фондов ОСВОБОЖДАЕТ маржу; запрещать его из-за того, что маржи
            #     сейчас мало, — это запрет лечения по симптому болезни. Третий рецидив
            #     семьи в одном файле, и оба прежних лечения стоят этажом ниже.
            # (2) ЗАВЕРШЕНИЕ УЖЕ ИСПОЛНЕННОГО. Плана нет ровно потому, что покупать больше
            #     нечего: pv_remainder пуст при полностью отработанных лотах. Отказ здесь
            #     не экономит ни доллара — денег не двигают, — а через три POSTPONED даёт
            #     MIXED на ЦЕЛОЙ книге, то есть создаёт разрыв там, где его не было.
            #
            # Незаконный COMPLETE (P0 №2 того же круга) остаётся закрытым: путь «плана нет,
            # ничего не исполнено, аварии нет» — единственный, ради которого порог заводился,
            # — по-прежнему отвергается. Пропуск не молчит: причина уходит в §7 через
            # _preview_pass_why, иначе послабление стало бы невидимым.
            if self._o3e_gate(_c, done_all, why='плана нет'):
                return True
            return _no(f'плана нет, а живой запас счёта {float(_c):.2f}x ниже норматива '
                       f'О-3-Е {_DLc.O3E_MIN} — завершать переход нечем')
        try:
            ...
            for _inst, _qty in orders:
                ...
                # ЧАСОВОЙ IBKR «НЕ ПОСЧИТАНО» — КОНЕЧНОЕ ЧИСЛО (рецензия 19.08). TWS шлёт
                # UNSET_DOUBLE = 1.7976931348623157E308 (максимум double, запись Java с
                # заглавной E), а фильтр ib_insync сравнивает со строкой Python
                # '…e+308' — строки не равны, значит часовой доходит до нас ОБЫЧНЫМ
                # значением и проходит проверку конечности. Дальше он суммируется как
                # маржа: одна такая заявка даёт «запас 0.00x», две — переполнение в inf,
                # и законный переход (включая аварийный выход Е→Ф) уходит в ABORT со
                # словом «маржа» там, где шлюз просто не считал.
                if abs(_im) >= UNSET_DOUBLE_MIN:
                    return _no(f'{_inst}: шлюз вернул часового «не посчитано» '
                               f'({_im:.3e}) вместо маржи — предпросмотр недоказуем')
                _ims.append(_im)
                _need += _im
            ...
            if not _ims:
                # ПЛАН БЕЗ ИСПОЛНИМЫХ ЗАЯВОК (рецензия 19.08): все количества округлились в
                # ноль — whatIf не вызывался ни разу, и доказывать нечего. Прежде этот путь
                # уходил вниз и получал уверенную причину «разгрузка замером НЕ
                # подтверждена», хотя _release_by_measure не звался вовсе; резюмируемый
                # переход у самого конца (pv_remainder отдаёт дробные остатки) читал бы в
                # инциденте ссылку на замер, которого никто не спрашивал.
                # ЗДЕСЬ ИСКЛЮЧЕНИЙ НЕТ: план ЕСТЬ, и то, что его количества округлились
                # в ноль контрактов, означает незавершённую покупку цели, а не законченную
                # работу. Признак done_all сюда не доходит по построению (вызывающий
                # считает его только при ПУСТОМ остатке), а аварийность не заменяет
                # доказательства — её дверь ниже, через замер.
                if self._o3e_ok(_c):
                    return True
                return _no(f'план не содержит исполнимых заявок (все количества округлились '
                           f'в ноль), а текущий запас {float(_c):.2f}x ниже норматива '
                           f'О-3-Е {_DLp.O3E_MIN}')
            if _worst <= 0:
                ...
                # УСЛОВИЕ `all(x <= 0)` УБРАНО КАК ТОЖДЕСТВО (рецензия 19.08): _pos_need —
                # сумма ПОЛОЖИТЕЛЬНЫХ приращений, она неотрицательна, поэтому _worst <= 0
                # при непустом _ims уже означает, что положительных нет. Лишний конъюнкт
                # создавал видимость второй проверки там, где её нет.
                if self._release_by_measure(orders):
                    return True
                if self._o3e_ok(_c):
                    return True
            ...
            # ЗДЕСЬ ИСКЛЮЧЕНИЙ НЕТ И БЫТЬ НЕ ДОЛЖНО: заявки есть, они ТРЕБУЮТ маржи, и
            # разрешение прямо двигает деньги. Ниже стоит и жёсткий пол 1,0.
            if self._o3e_ok(_nlv / _worst):
                return True
    ...
    def _preview_no(self, why):
        """ОТКАЗ ПРЕДПРОСМОТРА С НАЗВАННОЙ ПРИЧИНОЙ — одной точкой (44-й круг, №14;
        вынесено в метод рецензией 19.08 ради парной мутации). Причина уходит в журнал
        перехода и в текст инцидента: «маржа не прошла» и «шлюз не ответил» — разные
        состояния, и оператор обязан их различать."""
    ...
    def _release_by_measure(self, orders):
        ...
        ПРИЧИНА КОПИТСЯ ОТДЕЛЬНО (_release_why, рецензия 20.08): у _preview_why обязан быть
        ОДИН писатель — _preview_no. Прежде помощник писал в него напрямую, и выходило два
        вреда сразу: (а) вызывающий тут же затирал конкретную причину общей «разгрузка
        замером НЕ подтверждена», то есть дефект №14 выживал ВНУТРИ функции, которая его
        чинила; (б) при здоровом запасе preview возвращал TRUE, оставив в поле причину
        ОТКАЗА — состояние расходилось с ответом.
        ...
        """
        # ОШИБКА КОДА НЕ ГЛОТАЕТСЯ И ЗДЕСЬ (рецензия 19.08). Этот помощник вызывается ИЗ
        # preview, то есть ВНУТРИ его try: широкий except перехватывал TypeError и прочее
        # раньше, чем сработает `except _STce.CODE_ERRORS: raise` снаружи, — и уверенная
        # причина «разгрузка замером НЕ подтверждена» одевала ошибку кода в маржинальный
        # вердикт. Ровно инцидент 19.08, воспроизведённый в правке, которая его закрывала.
        self._release_why = ''
        try:
            ...
            raise
        except Exception as _exlm:
            # ПРИЧИНА НАЗЫВАЕТСЯ: Incident «замера маржи нет» и отказ по существу — разные
            # состояния, и оператор обязан их различать (первое лечится first_connect).
            self._release_why = f'замер маржи недоступен ({type(_exlm).__name__}: {_exlm})'
            return False
        if not _lm:
            return False
        ...
        try:
            ...
            raise                              # та же дверь, что и выше (рецензия 19.08)
        except Exception as _exas:
            self._release_why = f'сводка счёта недоступна ({type(_exas).__name__}: {_exas})'
            return False
        have = None
        for _v in _vals:
