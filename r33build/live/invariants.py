        ...
        if kind == 'правила45: допуск бара снимается известным календарём':
            ...
            try:
                ...
                out['таблицы_по_корням_покрывают_канон'] = (
                    ...
                    and set(_CT46.FUT_LEGS) == set(_CT46.FUT_ROOTS))
                ...
                def _alien46():
                    """Считает ли state имя YMU26 посторонним. ОДНА функция на обе стороны
                    пробы (до подмены канона и после): прежде тот же try/except стоял двумя
                    копиями, и в каждой исполнялось ровно одно плечо — успех в одной,
                    исключение в другой, а вторые плечи не брались НИКОГДА."""
                    try:
                        _ST46.book_from_broker(_DL46.Book, {'YMU26': 1.0}, 'F')
                        return False            # чужим не считает
                    except ValueError as _e46:
                        return 'посторонние' in str(_e46)

                # ДО подмены YMU26 обязан быть чужим, иначе _alien46()==False после подмены
                # ничего не доказывает (угол «от отрицания»).
                _alien_before46 = _alien46()
                ...
                try:
                    ...
                    _follow = (_TR46.is_fut_name('YMU26')
                               ...
                               and _alien46() is False)     # state пошёл за каноном
                finally:
                    _CT46.FUT_ROOTS = _keep_roots46
                out['корни_чужой_до_подмены'] = _alien_before46 is True
                out['корни_одна_точка'] = (not _copies) and _follow and _alien_before46
                out['корни_копии'] = _copies       # в отчёт: где именно завелась копия
                # СРАВНЕНИЕ С СОХРАНЁННЫМ, А НЕ С ЛИТЕРАЛОМ (восьмой прогон): зашитый
                # канон был ПЯТОЙ его копией и покраснел бы на законном добавлении корня —
                # ровно то, за что этот же круг забраковал текстовый сторож.
                out['корни_канон_восстановлен'] = _CT46.FUT_ROOTS == _keep_roots46
