...
# реестр/замер по причине СРЕДЫ, а не кода: живой замер снят на DUR833748, а переменная
# могла нести другой счёт. Правило 5 требует, чтобы стенды не зависели от машинного
# состояния — переменная счёта такое же состояние, как каталог книги. selfcheck её уже
# снимает; голый прогон invariants.py не снимал.
for _v0 in ('ADDFUT_BOOK_PATH', 'ADDFUT_DIR', 'ADDFUT_SIGNALS', 'ADDFUT_ACCOUNT'):
...
# ПУТЬ ОТДЕЛЬНОЙ КОНСТАНТОЙ, КАК У ОБОЛОЧКИ: иначе стенд читает файл по жёсткому
# пути, и парной мутации некуда встать — проверка осталась бы без убийцы (8в-2).
BACKUP_PUSH_SH = ROOT.parent / 'tools' / 'backup_push.sh'
...
def _a45nopl(beh):
    ...
    # ИСКЛЮЧЕНИЯ РАЗЛИЧАЮТСЯ ПО ТОМУ, ДВИГАЮТСЯ ЛИ ДЕНЬГИ (разбор /code-review 21.08).
    # Плана нет и работы не было — аварийность бессильна, иначе она открывает COMPLETE на
    # пустом месте при любом запасе. Плана нет, потому что всё куплено, — завершаем.
    # План есть, но все количества округлились в ноль — аварийный выход проходит.
...
def _dref_probe(br):
    """ПРОБНИК КЭША ДОХОДНОСТИ — ОДНО ТЕЛО НА ДВУХ СУДЕЙ (разбор /code-review 21.08).

    Правило наблюдают два стенда: адаптерный (его судит run_adapter вместе с мутациями
    адаптера) и случай RUN_CASES (его судит run_run). Разными обязаны быть ВЫЗЫВАЮЩИЕ, а
    не тело: две копии уже разъехались на третьей проверке, и любая правка правила
    оставляла бы одну из них зелёной — ровно то, ради чего в этом же круге извлекали
    transition.consume_partial.

    Возвращает (закреплённое читается один раз, живое не залипает, живое не пишет кэш).
    """
    import feed as _FDp
    import pandas as _pdp
    _n = {'i': 0}
    _orig = _FDp.yield_pct
    try:
        def _fake(ib_, today, expected_prev=None):
            _n['i'] += 1
            return (4.0 + 0.5 * _n['i'], None)
        _FDp.yield_pct = _fake
        _t = _pdp.Timestamp('2026-08-21')
        _p = _pdp.Timestamp('2026-08-20')
        br._dref_cache = None
        ...
        return _pinned, (_n['i'] == 2 and _l1 != _l2), (br._dref_cache is None)
    finally:
        _FDp.yield_pct = _orig
...
def _a45dref(beh):
    ...
    gross(), а unit_ref зовут и мимо него: величина часовой давности выглядела бы исправной
    проверкой. Судья тот же, что и у мутации, — набор адаптера; тело пробника общее.
    """
    br, ib, _ = _adapter(beh)
    _pinned, _live, _clean = _dref_probe(br)
    return _pinned and _live and _clean
...
@ainv('недобор исполнения сообщается как недобор, а не как полное',
      needs=lambda b: b == 'partial')
def _a_partial(beh):
    """СПЛОШНОЙ АУДИТ 27.08.2026, находка №18 реестра §5в: СЦЕНАРИЙ БЕЗ СУДЬИ.

    Сценарий 'partial' был зарегистрирован в ADDFUT_CASES и исправно прокручивался, но его
    не отбирал НИ ОДИН предикат needs — проверено механически: из 27 сценариев адаптера
    судей не имел ровно один. run_adapter отчитывался «сценариев брокера: 27», и это
    выглядело покрытием. Класс «стенд без судьи» записан в CLAUDE.md (8в-5) — вот его
    живой представитель.

    ЧТО ЗДЕСЬ ЦЕНА ОШИБКИ. Стаб исполняет ПОЛОВИНУ заказанного (ib_stub: done = want*0.5).
    Если адаптер сообщит заказанное вместо исполненного, книга разойдётся со счётом молча:
    контур будет считать ногу набранной, тогда как у брокера её половина. Это денежный
    путь, и до сегодняшнего дня его не проверял никто.

    ТРИ ТОЧКИ, И ВСЕ ТРИ РАЗНЫЕ. Недобор не округляется вверх (иначе книга завышена), не
    схлопывается в ноль (иначе контур повторит заявку поверх исполненного — запрет правила
    7), и совпадает с фактической позицией у брокера.
    """
    br, ib, rows = _adapter(beh)
    инстр = rows[0]['instrument']
    rec = br.place(инстр, 4)
    _исполнено = float(rec.get('filled') or 0.0)
    _поз = sum(float(v) for v in ib._pos.values())
    return (abs(_исполнено - 2.0) < 1e-9        # ровно столько, сколько исполнено
            and abs(_исполнено - 4.0) > 1e-9    # не заказанное
            and _исполнено != 0                 # и не «ничего не произошло»
            and abs(_поз - _исполнено) < 1e-9)  # книга брокера сходится с отчётом


@ainv('отсутствие стороны стакана (-1) не превращается в цену',
      needs=lambda b: b == 'normal')
def _a_px_minus_one(beh):
    """СПЛОШНОЙ АУДИТ 27.08.2026, находка №5 реестра §5в.

    IBKR обозначает ОТСУТСТВИЕ стороны стакана значением -1, а не пустотой. Проверка
    годности стояла ПОСЛЕ усреднения (`v > 0`), поэтому спасала last и close, но не mid:
    при bid=-1 и ask=6000 получалось 2999.5 — величина положительная, правдоподобная и
    ВДВОЕ ЗАНИЖЕННАЯ. Дальше она уходит в unit_ref (полоса вокруг половины настоящего
    номинала, из-за чего верный план перехода отвергается как «вне рыночной полосы») и в
    px_order строки §7.

    ЧЕТЫРЕ ТОЧКИ, И ДВЕ ИЗ НИХ РАЗРЕШАЮЩИЕ: испорченная сторона отбрасывается, целый
    стакан считается, обе испорченные тоже отбрасываются, а -1 в last не мешает годному
    стакану. Без разрешающих стенд зелен и для кода, который не берёт mid никогда.
    """
    class _Тикер:
        def __init__(s, bid, ask, last=None, close=5000.0):
            s.bid, s.ask, s.last, s.close, s.marketDataType = bid, ask, last, close, 3

    br, ib, rows = _adapter(beh)
    ib.sleep = lambda *a, **k: None

    def ref(тик):
        ib.reqMktData = lambda *a, **k: тик
        return br._quote_ref('ESU26')[0]

    return (ref(_Тикер(-1, 6000.0)) == 5000.0            # сторона потеряна -> откат на close
            and ref(_Тикер(5990.0, 6000.0)) == 5995.0    # целый стакан считается
            and ref(_Тикер(-1, -1)) == 5000.0            # обе стороны потеряны
            and ref(_Тикер(5990.0, 6000.0, last=-1)) == 5995.0)


@ainv('дробное количество на отказном пути даёт ДОМЕННЫЙ отказ, а не ошибку кода',
      needs=lambda b: b in ('reject', 'disconnect'))
def _a_qty_fmt(beh):
    """СПЛОШНОЙ АУДИТ 27.08.2026, находка №4 реестра §5в.

    В сообщениях об отказе стояло `{qty:+d}`, а sell_units/buy_units всегда передают
    float. Значит на ВСЕХ трёх отказных путях place() вместо BrokerError поднимался
    ValueError «Unknown format code 'd'»: задуманный отказ не возникал НИКОГДА. Хуже
    диагностики то, что слой CODE_ERRORS видел ошибку кода там, где должен был увидеть
    доменный вердикт, — а различать их проект учился отдельно (44-й и 45-й круги).

    Проверяются ОБА отказных сценария и оба конца: тип исключения обязан быть доменным, а
    количество обязано ДОЙТИ до текста — иначе стенд зелен и для кода, который просто убрал
    количество из сообщения.
    """
    import ib_broker as IBB
    br, ib, rows = _adapter(beh)
    try:
        br.sell_units(rows[0]['instrument'], 2.5)
        return False                     # отказ обязан быть: оба сценария нештатные
    except IBB.BrokerError as ex:
        return '-2.5' in str(ex)
    except Exception:
        return False                     # ValueError и всё прочее — это и есть дефект
...
RUN_CASES = (
             # ПРАВИЛА КРУГА — В НАЧАЛЕ СПИСКА (разбор /code-review 21.08). Мутационный
             # прогон зовёт run_run(stop_on_first=True) и останавливается на ПЕРВОМ
             # несогласии; пока эти одиннадцать случаев стояли в хвосте, каждая мутация,
             # чей единственный наблюдатель здесь, сперва прогоняла все сорок два
             # предшествующих случая — полные сессии, шелловые подпроцессы, git и WORM.
             # Порядок на результат не влияет, на время — влияет в разы.
...
'наблюдение', 'торговля', 'незамкнутая предыдущая',
             ...
             'автопилот: сторож окружения различает свои и чужие ADDFUT_*',
             'правила45: реестр перехода узнаёт живые имена с сериями',
             'правила45: незамкнутая книга не запирает АВАРИЙНЫЙ выход',
             'правила45: утраченный журнал §7 не начинается заново',
             'правила45: журнал §7 пишется атомарно и переживает утрату терминатора',
             'автопилот: выгрузка копий сообщает об отказе кодом возврата',
             'автопилот: уборщик отметок разбирает ВСЕ дни месяца',
             'автопилот: возраст замера маржи предупреждает ЗАРАНЕЕ',
             ...
             'автопилот: горизонт календаря спрашивается про ОБА маршрута',
)
...
""" % dict(tmp=tmp, sh=str(AUTOPILOT_SH))
    if kind == 'автопилот: пустая книга Е не считается слепотой':
        ...
        return out
    if kind == 'автопилот: выгрузка копий сообщает об отказе кодом возврата':
        # СПЛОШНОЙ АУДИТ 27.08.2026, находка №11 реестра §5в. Последней командой
        # backup_push.sh был `echo`, поэтому статус ВСЕГДА 0 — при любом исходе. Автопилот
        # сбрасывает счётчик неудач при успехе, значит счётчик обнулялся каждое замыкание и
        # порог «три отказа подряд = тревога» был НЕДОСТИЖИМ: механизм существовал только
        # на бумаге. Измерено на живой машине: 16 строк «drive ПРОПУЩЕН» в журнале
        # автопилота и НИ ОДНОГО файла push-fails за всё время работы.
        #
        # СТЕНД НЕ ХОДИТ В СЕТЬ: git подменяется заглушкой через PATH, HOME уводится во
        # временный каталог. Проверяются только КОДЫ, потому что именно код читает
        # автопилот; тексты в журнале уже различимы и без стенда.
        _sh11 = BACKUP_PUSH_SH
        out = dict(case=kind, raised=False, error='', ok=False)
        try:
            _d11 = tempfile.mkdtemp(prefix='addfut-bp-')
            os.makedirs(os.path.join(_d11, '.addfut'), exist_ok=True)
            _bin = os.path.join(_d11, 'bin')
            os.makedirs(_bin, exist_ok=True)
            # ЗАГЛУШКИ: git — чтобы стенд не ходил в сеть; rclone — чтобы «успех» был
            # НАСТОЯЩИМ успехом. Первая редакция стенда заглушки rclone не имела, и путь,
            # объявленный успешным, на деле уходил в ветку «нет rclone» (код 2): стенд
            # поймал это сам, показав успех_ноль=False. Скрипт ищет rclone по $HOME/bin,
            # поэтому заглушка кладётся именно туда, а не в PATH.
            os.makedirs(os.path.join(_d11, 'bin'), exist_ok=True)
            for _имя, _тело in (
                    ('git', '#!/bin/sh\nexit 0\n'),
                    ('tar', '#!/bin/sh\nexit 0\n')):
                _st = os.path.join(_bin, _имя)
                with open(_st, 'w', encoding='utf-8') as _f:
                    _f.write(_тело)
                os.chmod(_st, 0o755)
            _rc_stub = os.path.join(_d11, 'bin', 'rclone')
            with open(_rc_stub, 'w', encoding='utf-8') as _f:
                _f.write('#!/bin/sh\ncase "$1" in listremotes) echo "vol-down12-drive:";; '
                         '*) exit 0;; esac\n')
            os.chmod(_rc_stub, 0o755)
            _исх = _sh11.read_text(encoding='utf-8')

            def _прогон(правка):
                _f11 = os.path.join(_d11, 'bp.sh')
                Path(_f11).write_text(правка(_исх), encoding='utf-8')
                _env = dict(os.environ, HOME=_d11,
                            PATH=_bin + os.pathsep + os.environ.get('PATH', ''))
                return subprocess.run(['/bin/bash', _f11], capture_output=True, text=True,
                                      timeout=300, env=_env).returncode

            out['нет_каталога_проекта'] = _прогон(
                lambda s: s.replace('cd /home/alex/claude-projects/vol-down12 ||',
                                    'cd /нет-такого-каталога ||', 1)) == 3
            out['нет_rclone'] = _прогон(
                lambda s: s.replace('RCLONE=', 'RCLONE=/нет-такого-rclone #', 1)) == 2
            # РАЗРЕШАЮЩАЯ ТОЧКА: успех обязан остаться нулём, иначе тревога поднималась бы
            # каждое замыкание и контур встал бы на третьи сутки на ровном месте.
            ...
            out['error'] = f'{type(ex).__name__}: {ex}'
        return out
    if kind == 'автопилот: горизонт календаря спрашивается про ОБА маршрута':
        # СПЛОШНОЙ АУДИТ 27.08.2026, находка №23 реестра §5в. Вопрос задавался про
        # ДЕЙСТВУЮЩИЙ маршрут: активен Ф, его таблица праздников продлена до 2028 — значит
        # предупреждение молчало и молчало бы всегда. А таблица маршрута Е кончается 2026
        # годом: измерено, что calendar_horizon('E') отдаёт False уже с 2026-10-03.
        # ЦЕНА МОЛЧАНИЯ: маршрут Е — аварийный выход. В 2027-м переход Ф->Е откажет
        # FeedError ровно тогда, когда нужен, и предупреждения за 90 дней не будет НИ
        # ОДНОГО, потому что вопрос про Е никто не задал.
        # СТЕНД ИСПОЛНЯЕТ ТЕКСТ ИЗ БОЕВОГО ФАЙЛА и подставляет ПОДДЕЛЬНЫЙ feed: иначе
        # проверялась бы сегодняшняя дата, на которой оба календаря ещё покрыты, — то есть
        # угол «траектория, а не снимок» остался бы незакрытым.
        import os as _os23
        ...
        _d23 = None
        try:
            _txt = AUTOPILOT_SH.read_text(encoding='utf-8')
            _m = '_h=$(cd "$LIVE" && "$PY" -c "'
            out['вставка_вырезана'] = _txt.count(_m) == 1
            _i = _txt.index(_m) + len(_m)
            _src = _txt[_i:_txt.index('" 2>&1)', _i)].replace('\\$', '$')
            _d23 = _tf23.mkdtemp(prefix='addfut-cal23-')

            def _вердикт(f_ok, e_ok):
                _fake = ("def calendar_horizon(route='F', today=None, days=90):\n"
                         "    return ({'F': %r, 'E': %r}[route],\n"
                         "            'календарь маршрута %%s не покрывает горизонт' %% route)\n"
                         % (f_ok, e_ok))
                open(_os23.path.join(_d23, 'feed.py'), 'w', encoding='utf-8').write(_fake)
                _r = subprocess.run([sys.executable, '-c', _src], cwd=_d23,
                                    capture_output=True, text=True, timeout=120)
                return (_r.stdout or _r.stderr).strip()

            _оба = _вердикт(True, True)
            _плохо_e = _вердикт(True, False)
            _плохо_f = _вердикт(False, True)
            # РАЗРЕШАЮЩАЯ: когда оба покрыты, строки быть не должно — иначе предупреждение
            # звучит каждый день и его перестают читать.
            out['оба_покрыты_молчит'] = _оба.startswith('ADDFUT-VERDICT OK')
            # ГЛАВНОЕ: неактивный маршрут Е обязан быть замечен.
            out['плохой_E_замечен'] = (_плохо_e.startswith('ADDFUT-VERDICT NO')
                                       and 'E' in _плохо_e)
            out['плохой_F_замечен'] = (_плохо_f.startswith('ADDFUT-VERDICT NO')
                                       and 'F' in _плохо_f)
            out['ok'] = all([out['вставка_вырезана'], out['оба_покрыты_молчит'],
                             out['плохой_E_замечен'], out['плохой_F_замечен']])
        except Exception as ex:
            out['raised'] = True
            out['error'] = f'{type(ex).__name__}: {ex}'
        finally:
            if _d23:
                _sh23.rmtree(_d23, ignore_errors=True)
        return out
    if kind == 'автопилот: потерянный маршрут не подменяется молча на Ф':
        ...
        try:
            ...
            out['error'] = f'{type(ex).__name__}: {ex}'
        return out
    if kind == 'автопилот: отказ уборщика архивов не выдаётся за отказ снимка':
        # СПЛОШНОЙ АУДИТ 27.08.2026, находка №22 реестра §5в. Конвейер ротации стоял
        # ПОСЛЕДНЕЙ командой backup_state, а под `set -o pipefail` его код становился кодом
        # функции. Вызывающий читает это как «копия НЕ снята»: пишет ALARM-backup, не
        # ставит closed-<день>, и КАЖДЫЙ следующий тик повторяет замыкание и снимок WORM.
        # ИЗМЕРЕНО: пустой каталог копий даёт код 2, неудаляемый архив при 70 накопленных —
        # 123. Второй случай ДАТИРУЕМЫЙ: архивов сейчас 11, ротация впервые тронет файлы
        # примерно после 60 замыканий — в середине ноября.
        # ЭТО УГОЛ «ТРАЕКТОРИЯ, А НЕ СНИМОК»: на сегодняшнем состоянии ветка молчит, и
        # проверять её надо на будущем — что стенд и делает, накапливая 70 архивов.
        import os as _os22
        ...
        try:
            _txt = AUTOPILOT_SH.read_text(encoding='utf-8')
            _m1 = '    # СТАТУС УБОРЩИКА НЕ ЕСТЬ СТАТУС СНИМКА'
            out['блок_вырезан'] = _txt.count(_m1) == 1
            _i = _txt.index(_m1)
            _блок = _txt[_i:_txt.index('    return 0\n}', _i) + len('    return 0\n')]

            def _прогон(файлов, только_чтение):
                _d = _tf22.mkdtemp(prefix='addfut-rot-')
                for _k in range(файлов):
                    open(_os22.path.join(
                        _d, f'addfut-2026-08-{_k % 28 + 1:02d}-1200{_k:02d}.tgz'), 'w').write('x')
                if только_чтение:
                    _os22.chmod(_d, 0o555)
                _prog = ('set -u -o pipefail\nlog() { echo "ЖУРНАЛ: $*"; }\n'
                         f'bdir="{_d}"\nf() {{\n{_блок}}}\nf\n')
                _r = subprocess.run(['/bin/bash', '-c', _prog], capture_output=True,
                                    text=True, timeout=120)
                if только_чтение:
                    _os22.chmod(_d, 0o755)
                _sh22.rmtree(_d, ignore_errors=True)
                return _r.returncode, ('ротация архивов состояния не выполнена' in _r.stdout)

            _к0, _п0 = _прогон(0, False)
            _к11, _п11 = _прогон(11, False)
            _к70, _п70 = _прогон(70, False)
            _кro, _пro = _прогон(70, True)
            # СНИМОК СОСТОЯЛСЯ — КОД НОЛЬ ВО ВСЕХ ЧЕТЫРЁХ: отказ уборщика не имеет права
            # останавливать замыкание дня.
            out['код_всегда_ноль'] = (_к0, _к11, _к70, _кro) == (0, 0, 0, 0)
            # НО МОЛЧАТЬ УБОРЩИК НЕ ДОЛЖЕН: непрошедшая уборка означает рост каталога без
            # предела, и об этом обязана быть строка.
            out['неудача_не_молчит'] = _п0 and _пro
            # И НЕ ДОЛЖЕН КРИЧАТЬ ЗРЯ — иначе строку перестанут читать.
            out['удача_молчит'] = (not _п11) and (not _п70)
            out['ok'] = all([out['блок_вырезан'], out['код_всегда_ноль'],
                             out['неудача_не_молчит'], out['удача_молчит']])
        except Exception as ex:
            out['raised'] = True
            out['error'] = f'{type(ex).__name__}: {ex}'
        return out
    if kind == 'автопилот: возраст замера маржи предупреждает ЗАРАНЕЕ':
        # СПЛОШНОЙ АУДИТ 27.08.2026, находка №14 реестра §5в. Замер маржи живёт 35 дней
        # (transition._meta_age_ok); старше — Incident, который selfcheck ловит широким
        # перехватом и после истёкшего срока превращает в chk(False), то есть в
        # «ВЫПУСК НЕ СОСТОЯЛСЯ». Нынешний замер снят 18.08 — с 23.09 выпуск встал бы САМ
        # СОБОЙ, и до того дня об этом не говорило НИЧТО. Отказ датируемый и тихий, а
        # освежить замер по требованию нельзя: whatIf на бумажном шлюзе днём маржи не
        # отдаёт, оператору нужно заранее известное утро.
        #
        # ЭТО УГОЛ «ТРАЕКТОРИЯ, А НЕ СНИМОК» (правило 8а(а)): проверка, молчащая на
        # сегодняшнем состоянии, не испытана вовсе. Поэтому стенд гоняет БУДУЩИЕ даты, а не
        # только текущую, и исполняет ТЕКСТ ИЗ БОЕВОГО ФАЙЛА, а не свою копию правила.
        import json as _js14
        import os as _os14
        ...
        _d14 = None
        try:
            _txt14 = AUTOPILOT_SH.read_text(encoding='utf-8')
            _mk14 = '_mg=$(cd "$LIVE" && "$PY" -c "'
            out['снимок_вырезан'] = _txt14.count(_mk14) == 1
            _i14 = _txt14.index(_mk14) + len(_mk14)
            _src14 = _txt14[_i14:_txt14.index('" 2>&1)', _i14)]
            _d14 = _tf14.mkdtemp(prefix='addfut-mg-')

            def _вердикт(дата):
                _js14.dump({'_meta': {'date': f'{дата} 08:54 UTC'}, 'ESU26': 1},
                           open(_os14.path.join(_d14, 'margins_live.json'), 'w'))
                _r = subprocess.run([sys.executable, '-c', _src14], cwd=_d14,
                                    capture_output=True, text=True, timeout=120)
                _o = (_r.stdout or _r.stderr).strip()
                return _o.split()[1] if _o.startswith('ADDFUT-VERDICT') else '??'

            import datetime as _dt14
            _сегодня = _dt14.datetime.now(_dt14.timezone.utc).date()
            # СВЕЖИЙ замер молчит — иначе предупреждение стало бы шумом и его перестали
            # бы читать; это разрешающая половина.
            out['свежий_молчит'] = _вердикт((_сегодня - _dt14.timedelta(days=2)).isoformat()) == 'OK'
            # ЗА 8 ДНЕЙ ДО СТЕНЫ предупреждение обязано быть — это и есть заблаговременность.
            out['за_восемь_дней_предупреждает'] = _вердикт(
                (_сегодня - _dt14.timedelta(days=27)).isoformat()) == 'NO'
            # ПОСЛЕ СТЕНЫ тоже, разумеется.
            out['после_предела_предупреждает'] = _вердикт(
                (_сегодня - _dt14.timedelta(days=40)).isoformat()) == 'NO'
            # НЕЧИТАЕМЫЙ ЗАМЕР — НЕ МОЛЧАНИЕ: иначе пропажа файла выглядела бы как «всё
            # хорошо», тот же класс, что молчаливый код возврата выгрузки.
            open(_os14.path.join(_d14, 'margins_live.json'), 'w').write('не json')
            _r14 = subprocess.run([sys.executable, '-c', _src14], cwd=_d14,
                                  capture_output=True, text=True, timeout=120)
            _o14 = (_r14.stdout or _r14.stderr).strip()
            out['нечитаемый_не_молчит'] = _o14.startswith('ADDFUT-VERDICT NO')
            out['ok'] = all([out['снимок_вырезан'], out['свежий_молчит'],
                             out['за_восемь_дней_предупреждает'],
                             out['после_предела_предупреждает'],
                             out['нечитаемый_не_молчит']])
        except Exception as ex:
            out['raised'] = True
            out['error'] = f'{type(ex).__name__}: {ex}'
        finally:
            if _d14:
                _sh14.rmtree(_d14, ignore_errors=True)
        return out
    if kind == 'автопилот: уборщик отметок разбирает ВСЕ дни месяца':
        # СПЛОШНОЙ АУДИТ 27.08.2026, находка №12. В уборщике стояло `${_mk##*-2}` — срез по
        # САМОМУ ДЛИННОМУ префиксу, кончающемуся на «-2». Для traded-2026-08-27 это
        # оставляло «27», шаблон даты такое не принимал, и КАЖДАЯ отметка с днём 20-29
        # пропускалась — треть входа. Уборщик не падал, он просто не работал и не сообщал
        # об этом ничего.
        # СТЕНД ИСПОЛНЯЕТ БОЕВОЙ РАЗБОР, А НЕ ПЕРЕСКАЗ: строка с разбором вырезается из
        # autopilot.sh, иначе проверялась бы моя копия правила.
        _txt12 = AUTOPILOT_SH.read_text(encoding='utf-8')
        _mark = '_base=${_mk##*/}; _d=${_base#*-}'
        out = dict(case=kind, raised=False, error='', ok=False)
        try:
            out['разбор_вырезан'] = _txt12.count(_mark) == 1
            _prog = ('set -u\n'
                     'for _mk in %s; do\n'
                     '  %s\n'
                     '  case "$_d" in [0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]) '
                     'echo "ДА $_d";; *) echo "НЕТ $_d";; esac\n'
                     'done\n') % (
                '/x/traded-2026-08-27 /x/closed-2026-08-15 /x/traded-2026-12-25 '
                '/x/closed-2026-11-20 /x/traded-2026-08-03 /x/traded-мусор', _mark)
            _r12 = subprocess.run(['/bin/bash', '-c', _prog], capture_output=True,
                                  text=True, timeout=60)
            _строки = _r12.stdout.split()
            # ДНИ 20-29 — ТО, ЧТО ПРОПУСКАЛОСЬ; дни 03 и 15 — то, что работало и обязано
            # работать дальше; мусорное имя обязано по-прежнему отвергаться.
            out['дни_20_29_разбираются'] = all(
                f'ДА {d}' in _r12.stdout for d in ('2026-08-27', '2026-12-25', '2026-11-20'))
            out['прочие_дни_разбираются'] = all(
                f'ДА {d}' in _r12.stdout for d in ('2026-08-15', '2026-08-03'))
            out['чужое_имя_отвергнуто'] = 'НЕТ мусор' in _r12.stdout
            out['ok'] = all([out['разбор_вырезан'], out['дни_20_29_разбираются'],
                             out['прочие_дни_разбираются'], out['чужое_имя_отвергнуто']])
        except Exception as ex:
            out['raised'] = True
            out['error'] = f'{type(ex).__name__}: {ex}'
        return out
    if kind == 'автопилот: сторож окружения различает свои и чужие ADDFUT_*':
        # СЛОЙ 1 РЕЕСТРА 5б, шаг 3. env_guard — единственная функция ядра оболочки, чей
        # отказ останавливает торговлю ЦЕЛИКОМ, и до 26.08.2026 у неё не было ни одного
        # стенда и ни одной мутации. Цена пробела измерена в тот же день: зонды аудита
        # подключили боевой файл с ADDFUT_DIR, сторож отработал верно — и остановил ЖИВОЙ
        # контур, потому что ST=$HOME/.addfut задан жёстко и тревога легла в боевой
        # каталог (§12, инцидент 26.08).
        #
        # ШЕСТЬ ИСХОДОВ СРАЗУ, И ТРИ ИЗ НИХ — РАЗРЕШАЮЩИЕ. Одних запретов мало: стенд из
        # одних «поднял тревогу» зелен и для сторожа, который запрещает ВСЁ, — а такой
        # сторож не пустил бы автопилот в торговлю ни разу (ADDFUT_ACCOUNT он ставит себе
        # сам, ADDFUT_REALTIME_MD включает подписку в бою). Это угол «от противоположного
        # знака»: у каждого ужесточения спрашивается, какой законный путь оно закрыло.
        #
        # ИЗОЛЯЦИЯ — ЧЕРЕЗ HOME, А НЕ ЧЕРЕЗ ADDFUT_*: последнее здесь физически невозможно,
        # потому что ADDFUT_* и есть проверяемый вход. Окружение строится с нуля (env -i),
        # иначе переменные батареи попадут в измеряемый прогон и стенд будет мерить их.
        _sh_env = str(AUTOPILOT_SH)

        def _guard(prep):
            _h = tempfile.mkdtemp(prefix='addfut-envguard-')
            os.makedirs(os.path.join(_h, '.addfut'), exist_ok=True)
            _prog = ('exit() { return 0; }\n'
                     'source "$SH" __стенд__ >/dev/null 2>&1\n'
                     'unset -f exit\n'
                     'eval "$PREP"\n'
                     'if env_guard; then echo "КОД=0"; else echo "КОД=1"; fi\n'
                     'ls "$ST" | sed "s/^/ФАЙЛ=/"\n')
            _r = subprocess.run(
                ['env', '-i', f'HOME={_h}', f'PATH={os.environ.get("PATH", "")}',
                 'TERM=dumb', f'SH={_sh_env}', f'PREP={prep}', 'bash', '-c', _prog],
                capture_output=True, text=True, cwd=str(ROOT), timeout=120)
            return _r.stdout, _h

        # ИСКЛЮЧЕНИЕ ЛОВИТСЯ, КАК У СОСЕДНИХ СЛУЧАЕВ. Без этого падение подпроцесса
        # (timeout, нет bash, нет места под временный дом) обрушило бы ВСЮ батарею вместо
        # отказа одного случая, а судья `not r['raised']` не увидел бы даже словаря.
        try:
            _o1, _ = _guard(':')
            out['чистое_окружение_пропущено'] = ('КОД=0' in _o1 and 'ФАЙЛ=ALARM' not in _o1)
            _o2, _ = _guard('export ADDFUT_ACCOUNT=DUR833748')
            out['свой_пин_счёта_пропущен'] = ('КОД=0' in _o2 and 'ФАЙЛ=ALARM' not in _o2)
            _o3, _ = _guard('export ADDFUT_REALTIME_MD=1')
            out['боевой_переключатель_пропущен'] = ('КОД=0' in _o3 and 'ФАЙЛ=ALARM' not in _o3)
            # ЧУЖАЯ ПЕРЕМЕННАЯ — ЗАПРЕТ И ДОЛГОВЕЧНАЯ ДАТИРОВАННАЯ ТРЕВОГА. Без даты в имени
            # тревога затиралась бы следующим тиком (двадцать третий круг, №22 и №7).
            _o4, _ = _guard('export ADDFUT_LIVE_OK=1')
            out['чужая_запрещена'] = ('КОД=1' in _o4 and 'ФАЙЛ=ALARM-env-' in _o4)
            _o5, _ = _guard('export ADDFUT_DIR=/tmp/подмена')
            out['подмена_путей_запрещена'] = ('КОД=1' in _o5 and 'ФАЙЛ=ALARM-env-' in _o5)
            # ШЕСТОЙ ИСХОД — ИЗМЕРЕННЫЙ ФАКТ О САМОЙ ФУНКЦИИ, А НЕ ТРЕБОВАНИЕ К НЕЙ. Второй
            # сторож (строки «переопределение ADDFUT_* — пути состояния раздвоены») в БОЕВОМ
            # пути НЕДОСТИЖИМ: cron отдаёт переменные окружением, то есть ЭКСПОРТИРОВАННЫМИ,
            # `env` их видит, и первый сторож возвращает 1 раньше. Достижим он ровно тогда,
            # когда файл ПОДКЛЮЧАЮТ из оболочки с НЕэкспортированной переменной — то есть на
            # стенде. Стенд закрепляет это как есть: сторож жив и охраняет стенды, а не бой.
            # Убрать его нельзя не рассуждением, а только отдельным решением: удаление сняло бы
            # единственную защиту подключаемого контекста.
            _o6, _ = _guard('ADDFUT_DIR=/tmp/подмена')
            out['второй_сторож_достижим_только_подключением'] = (
                'КОД=1' in _o6 and 'ФАЙЛ=ALARM-env.txt' in _o6
                and 'ФАЙЛ=ALARM-env-' not in _o6.replace('ФАЙЛ=ALARM-env.txt', ''))
            out['ok'] = all([out['чистое_окружение_пропущено'], out['свой_пин_счёта_пропущен'],
                             out['боевой_переключатель_пропущен'], out['чужая_запрещена'],
                             out['подмена_путей_запрещена'],
                             out['второй_сторож_достижим_только_подключением']])
        except Exception as ex:
            out['raised'] = True
            out['error'] = f'{type(ex).__name__}: {ex}'
...
def _rules45_case(kind):
    if kind == 'правила45: оценка плана заявок не оставляет временных каталогов':
        # СПЛОШНОЙ АУДИТ 27.08.2026, находка №25 реестра §5в. plan_orders создавала
        # временный каталог голым mkdtemp и не убирала его, а зовётся она из БОЕВОГО
        # предполёта (preflight_margin_orders) и дважды из selfcheck — то есть ВНЕ процесса
        # батареи, где уборка по atexit есть.
        # ИЗМЕРЕНО НА ЖИВОЙ МАШИНЕ: 3591 каталог /tmp/addfut-*, из них 199 с этим
        # префиксом; /tmp система чистит лишь через 30 дней. Переполнение /tmp
        # останавливает автопилот — тот же класс, из-за которого убирает за собой и
        # измеритель покрытия.
        # СТЕНД СЧИТАЕТ КАТАЛОГИ ДО И ПОСЛЕ, а не читает исходник: утечку видно только по
        # следам, и «в коде написан контекст» — не то же самое, что «каталог исчез».
        import glob as _gl25
        out = dict(case=kind, raised=False, error='', ok=False)
        try:
            import transition as _T25
            _до = len(_gl25.glob('/tmp/addfut-plancount-*'))
            # ЗОНД ДОСТИЖИМОСТИ: вызов обязан ДОЙТИ до создания каталога, иначе стенд
            # доказывает, что ничего не делалось. Проверяем это возвратом пары чисел.
            _res = _T25.plan_orders([], {}, 1e9)
            out['вызов_дошёл'] = isinstance(_res, tuple) and len(_res) == 2
            _после = len(_gl25.glob('/tmp/addfut-plancount-*'))
            out['прироста_нет'] = (_после == _до)
            out['ok'] = out['вызов_дошёл'] and out['прироста_нет']
        except Exception as ex:
            out['raised'] = True
            out['error'] = f'{type(ex).__name__}: {ex}'
        return out
    if kind == 'правила45: пустой route.txt — отсутствие маршрута, а не маршрут «»':
        # СПЛОШНОЙ АУДИТ 27.08.2026, находка №20 реестра §5в. active_route проверяла
        # СУЩЕСТВОВАНИЕ файла, а не содержимое: файл нулевой длины давал маршрут '' —
        # значение не None, поэтому громкий отказ «маршрут неизвестен» не срабатывал.
        # ЦЕНА — ЛОЖНЫЙ ДИАГНОЗ В САМОМ ДОРОГОМ МЕСТЕ: дальше по пустому маршруту строится
        # путь книги (book_path('') -> book-.json), load возвращает None, и оператору
        # сообщают «книга отсутствует — якорь не о чем писать». При потере МАРШРУТА его
        # посылают чинить КНИГУ. Пустой route.txt здесь не гипотеза: он уже дважды
        # приводил к тревогам (ALARM-route-2026-08-15 и -16).
        # Оболочка ту же ситуацию ловила верно (`[ -s "$ROUTE_F" ]`), то есть правило
        # разъехалось между двумя реализациями одного решения.
        import os as _os20
        import tempfile as _tf20
        out = dict(case=kind, raised=False, error='', ok=False)
        _keep20 = _os20.environ.get('ADDFUT_LOCK_DIR')
        try:
            _d20 = _tf20.mkdtemp(prefix='addfut-route-')
            _os20.environ['ADDFUT_LOCK_DIR'] = _d20
            # БЕЗ ПЕРЕЗАГРУЗКИ МОДУЛЯ, И ЭТО НЕ МЕЛОЧЬ. Первая редакция стенда звала
            # importlib.reload(state) — и тем самым ВОССТАНАВЛИВАЛА подменённую функцию,
            # то есть отменяла парную мутацию: стенд оставался зелёным под мутацией и не
            # наблюдал ничего. Перезагрузка не нужна вовсе: lock_dir читает
            # ADDFUT_LOCK_DIR из окружения при КАЖДОМ вызове (state.py:98).
            import state as _ST20
            _p20 = _os20.path.join(_d20, 'route.txt')

            def _исход(содержимое):
                if содержимое is None:
                    if _os20.path.exists(_p20):
                        _os20.unlink(_p20)
                else:
                    open(_p20, 'w', encoding='utf-8').write(содержимое)
                try:
                    return _ST20.active_route()
                except RuntimeError:
                    return 'ОТКАЗ'

            out['нет_файла_отказ'] = _исход(None) == 'ОТКАЗ'
            out['пустой_отказ'] = _исход('') == 'ОТКАЗ'
            out['пробелы_отказ'] = _исход('   \n') == 'ОТКАЗ'
            # РАЗРЕШАЮЩИЕ ТОЧКИ: без них стенд зелен и для функции, отвергающей ВСЁ, —
            # а это остановка контура на ровном месте.
            out['маршрут_F_проходит'] = _исход('F') == 'F'
            out['маршрут_E_проходит'] = _исход('E\n') == 'E'
            out['ok'] = all([out['нет_файла_отказ'], out['пустой_отказ'],
                             out['пробелы_отказ'], out['маршрут_F_проходит'],
                             out['маршрут_E_проходит']])
        except Exception as ex:
            out['raised'] = True
            out['error'] = f'{type(ex).__name__}: {ex}'
        finally:
    ...
    if kind == 'правила45: журнал §7 пишется атомарно и переживает утрату терминатора':
        ...
        # №9: файл, у которого последняя строка ПОЛНА, но не завершена переводом строки,
        # read и verify считают ЗДОРОВЫМ, а следующий append дописывает встык и затирает
        # row_hash предыдущей строки — цепочка разрушается НЕОБРАТИМО. Писатель, ради
        # целостности которого цепочка заведена, сам её и ломал.
        # №10: журнал был ЕДИНСТВЕННЫМ долговечным артефактом без temp+fsync+rename (книга,
        # намерение и якорь давно атомарны). Обрыв записи запирал контур целиком: отказывали
        # read, verify, append, снимок WORM и замыкание дня, без автоматического выхода.
        import os as _os9
        import tempfile as _tf9
        import journal as _J9
        out = dict(case=kind, raised=False, error='', ok=False)
        try:
            _d9 = _tf9.mkdtemp(prefix='addfut-j79-')
            _p9 = _os9.path.join(_d9, 'journal-F.csv')

            def _итог(n):
                _J9.append(_p9, dict(date=f'2026-08-{25 + n}', leg='', instrument='ИТОГ',
                                     qty=0, px_order='-', px_fill='', commission='',
                                     reason='', nav='', leverage='', roll_spread_near='',
                                     roll_spread_far='', note=f'итог сессии {n}: строк 0'))

            _итог(1); _итог(2)
            out['обычная_запись'] = (len(_J9.read(_p9)) == 2 and _J9.verify(_p9) == 2)
            _os9.chmod(_p9, 0o664)
            # УТРАТА ТЕРМИНАТОРА: прежде следующий append разрушал цепочку.
            _b9 = open(_p9, 'rb').read()
            open(_p9, 'wb').write(_b9.rstrip(b'\r\n'))
            _итог(3)
            out['терминатор_восстановлен'] = (len(_J9.read(_p9)) == 3 and _J9.verify(_p9) == 3)
            # АТОМАРНОСТЬ наблюдаема по следам: временных файлов после записи не остаётся,
            # а режим доступа не меняется (иначе первая же запись выглядела бы как
            # вмешательство в машинное состояние для lstat-снимка selfcheck).
            out['временных_не_осталось'] = not [f for f in _os9.listdir(_d9)
                                                if f.endswith('.tmp')]
            out['режим_сохранён'] = (_os9.stat(_p9).st_mode & 0o777) == 0o664
            out['ok'] = all([out['обычная_запись'], out['терминатор_восстановлен'],
                             out['временных_не_осталось'], out['режим_сохранён']])
        except Exception as ex:
            out['raised'] = True
            out['error'] = f'{type(ex).__name__}: {ex}'
        return out
    if kind == 'правила45: утраченный журнал §7 не начинается заново':
        # СПЛОШНОЙ АУДИТ 27.08.2026, находка №8 реестра §5в. Ворота целостности журнала были
        # СКОПИРОВАНЫ ДВАЖДЫ в одной функции и отсутствовали на третьем пути записи —
        # «сессия исполнена прошлым запуском, состояние принимается по намерению». Тот путь
        # зовёт J.append и делает return ВЫШЕ обеих копий.
        # ИЗМЕРЕНО: append в отсутствующий журнал создаёт заголовок и первую строку с
        # prev_hash = GENESIS, после чего verify() отдаёт 1 и не жалуется. Утраченная
        # история начинается заново, следующая сессия видит валидную цепочку, WORM заверяет
        # её как подлинную — «отмывание истории», закрытое 43-м кругом на других путях.
        import os as _os8
        import tempfile as _tf8
        import daily as _DL8
        import journal as _J8
        out = dict(case=kind, raised=False, error='', ok=False)
        try:
            _bk = _DL8.Book(n_e=26, n_b=0, unit_is_mes=True, d_fix=7.93, prev_close_lev=1.0,
                            prev_st_eq=True, prev_st_bd=False, ser_a='Z26', ser_b='Z26',
                            es_held=2, roll_pending=False, last_session='2026-08-26',
                            close_provisional=False)
            _p8 = _os8.path.join(_tf8.mkdtemp(prefix='addfut-j78-'), 'journal.csv')

            def _держит(sess):
                try:
                    _DL8._j7_gate(_p8, _bk, sess)
                    return False
                except RuntimeError:
                    return True

            # ЗАПРЕЩАЮЩЕЕ: журнал утрачен, а книга торговала.
            out['утрата_при_торговавшей_книге_отказ'] = _держит(11)
            # РАЗРЕШАЮЩЕЕ: новая установка — журнала нет и сессий не было.
            out['новая_установка_проходит'] = not _держит(0)
            _J8.append(_p8, dict(date='2026-08-26', leg='', instrument='ИТОГ', qty=0,
                                 px_order='-', px_fill='', commission='', reason='',
                                 nav='', leverage='', roll_spread_near='',
                                 roll_spread_far='', note='итог сессии 11: строк 0'))
            # РАЗРЕШАЮЩЕЕ: целый журнал, закрытый итогом.
            out['целый_журнал_проходит'] = not _держит(11)
            _J8.append(_p8, dict(date='2026-08-27', leg='А', instrument='ESZ26', qty=1,
                                 px_order='7700', px_fill='7700', commission='2',
                                 reason='', nav='990000', leverage='1.0',
                                 roll_spread_near='', roll_spread_far='', note=''))
            # ЗАПРЕЩАЮЩЕЕ: сессия не закрыта итогом — строки исполнения могли потеряться.
            out['незакрытая_сессия_отказ'] = _держит(12)
            out['ok'] = all([out['утрата_при_торговавшей_книге_отказ'],
                             out['новая_установка_проходит'],
                             out['целый_журнал_проходит'],
                             out['незакрытая_сессия_отказ']])
        except Exception as ex:
            out['raised'] = True
            out['error'] = f'{type(ex).__name__}: {ex}'
        return out
    if kind == 'правила45: незамкнутая книга не запирает АВАРИЙНЫЙ выход':
        # СПЛОШНОЙ АУДИТ 27.08.2026, находка №7 реестра §5в. У сторожа close_provisional не
        # было исключения для аварии, хотя у соседнего сторожа строкой ниже оно есть.
        # СЧИТАНО ПО КОДУ, А НЕ ПРИКИНУТО: окно перехода 02:00-10:30 Чикаго
        # (feed.common_window), ворота замыкания Е 10:35 (feed.e_close_gate), торговое окно
        # с 08:45. Ежедневная сессия сохраняет книгу с close_provisional=True, значит с
        # 08:45 сторож отказывает, а в 10:30 закрывается окно — замыкание в 10:35 уже
        # поздно. Аварийный выход был недоступен с 08:45 до 02:00 следующего дня, то есть
        # всю американскую сессию: ровно тогда, когда случается маржинальный стресс.
        import daily as _DL7
        import state as _ST7
        import transition as _T7
        out = dict(case=kind, raised=False, error='', ok=False)
        _keep = (_ST7.load, _ST7.load_intent)
        try:
            _bk = _DL7.Book(n_e=26, n_b=0, unit_is_mes=True, d_fix=7.93, prev_close_lev=1.0,
                            prev_st_eq=True, prev_st_bd=False, ser_a='Z26', ser_b='Z26',
                            es_held=2, roll_pending=False, last_session='2026-08-27',
                            close_provisional=True)
            _ST7.load = lambda p, c=None: (_bk, None, None)
            _ST7.load_intent = lambda p: None

            def _зовём(ав):
                try:
                    _T7._preflight_handover('F', 'E', _dst_names=['CSPX', 'CBU0'],
                                            emergency=ав)
                    return ''
                except Exception as ex:
                    return str(ex)
            ...
            _без = _зовём(False)
            _с = _зовём(True)
            # ОБЕ СТОРОНЫ. Без аварии сторож обязан ДЕРЖАТЬ: иначе правка сняла бы защиту
            # вовсе, и незамкнутая книга уезжала бы с маршрутом в обычный день.
            out['без_аварии_держит'] = 'close_provisional' in _без
            out['при_аварии_пропускает'] = 'close_provisional' not in _с
            out['ok'] = out['без_аварии_держит'] and out['при_аварии_пропускает']
        except Exception as ex:
            out['raised'] = True
            out['error'] = f'{type(ex).__name__}: {ex}'
        finally:
            _ST7.load, _ST7.load_intent = _keep
        return out
    if kind == 'правила45: реестр перехода узнаёт живые имена с сериями':
        # СПЛОШНОЙ АУДИТ 27.08.2026, находка №6 реестра §5в — самая дорогая из денежных.
        # В проекте ДВА реестра: закреплённый хэшем whitelist направлений (голые корни ES,
        # MES, ZN, CSPX, CBU0) и живой реестр con_id (имена с серией ESZ26, MESZ26).
        # net_positions отдаёт ВТОРЫЕ, сверка шла по ПЕРВЫМ и по точному имени — значит
        # любая живая позиция объявлялась неизвестной реестру, и переход был заперт в ОБЕ
        # стороны, включая аварийный выход Е->Ф при маржинальном стрессе.
        #
        # СТЕНД БЕРЁТ ИМЕНА ИЗ ЖИВОГО РЕЕСТРА, А НЕ ИЗ СВОЕГО СПИСКА: свой список разошёлся
        # бы с боевым при первом же ролле, и стенд доказывал бы вчерашний мир. Это ровно
        # тот класс, из-за которого дефект и дожил: фикстура подставляла обеим сторонам
        # один голый instruments.csv и описывала мир, которого на счёте нет.
        import csv as _csv6
        import transition as _T6
        out = dict(case=kind, raised=False, error='', ok=False)
        try:
            _reg = _T6.load_registry(str(ROOT / 'instruments.csv'))
            _живые = [r['instrument'] for r in
                      _csv6.DictReader(open(ROOT / 'live' / 'instruments_live.csv',
                                            encoding='utf-8'))]
            _фьюч = [n for n in _живые if _T6.fut_series(n)]
            out['живых_имён_с_серией'] = len(_фьюч)
            out['все_серии_опознаны'] = bool(_фьюч) and all(
                _T6.reg_of(_reg, n) is not None for n in _фьюч)
            # РАЗРЕШАЮЩЕЕ И ЗАПРЕЩАЮЩЕЕ РЯДОМ: без второго стенд зелен и для реестра,
            # который принимает ЧТО УГОДНО, — а это снимает единственную защиту от чужого
            # инструмента на счёте.
            out['голые_корни_опознаны'] = all(
                _T6.reg_of(_reg, n) is not None for n in ('ES', 'MES', 'ZN', 'CSPX', 'CBU0'))
            out['чужое_отвергнуто'] = all(
                _T6.reg_of(_reg, n) is None
                for n in ('ЧУЖОЙ', 'НЕИЗВЕСТНЫЙ:12345', 'SPY', 'ESZ26X'))
            # Класс и группа обязаны браться у КОРНЯ, иначе пара «ES->CSPX» рассыплется.
            out['класс_от_корня'] = (_T6.reg_of(_reg, 'ESZ26')['sec_type'] == 'FUT'
                                     and _T6.reg_of(_reg, 'ESZ26')['pair_group'] ==
                                     _T6.reg_of(_reg, 'CSPX')['pair_group'])
            out['ok'] = all([out['все_серии_опознаны'], out['голые_корни_опознаны'],
                             out['чужое_отвергнуто'], out['класс_от_корня']])
        except Exception as ex:
            out['raised'] = True
            out['error'] = f'{type(ex).__name__}: {ex}'
        return out
    ...
    # Форма ответа — как у соседних помощников (_autopilot_case, _worm_case): четыре ключа.
    # Поля исполнителя (placed/positions/saved/lev/journal) здесь были скопированы из формы
    # отказа _session_run и ни одним утверждением не читаются — все они отбирают случай по
    # точному r['case'], — зато обещали читателю заявки и книги, которых стенд не касается.
    out = dict(case=kind, raised=False, error='', ok=False)
    try:
        if kind == 'правила45: допуск бара снимается известным календарём':
            ...
            # АВАРИЙНОСТЬ НЕ ОТМЫЧКА К COMPLETE НА ПУСТОМ МЕСТЕ (разбор /code-review 21.08):
            # когда плана нет И ничего не исполнено, признак аварии обязан быть бессилен —
            # иначе он открывает «переход завершён, хотя ничего не делалось», причём при
            # ЛЮБОМ запасе, вплоть до 0,05x.
            _e_ok, _e_why = _pv(_low, emergency=True)
            out['авария_не_отмычка'] = _e_ok is False
            _e2_ok, _ = _pv(0.05, emergency=True)
            out['авария_не_отмычка_и_на_дне'] = _e2_ok is False
            _d_ok, _d_why = _pv(_low, done_all=True)
            out['исполненное_завершается'] = _d_ok is True and 'нечего' in _d_why
            out['пропуск_назван'] = bool(_d_why)
            # А ТАМ, ГДЕ ПЛАН ЕСТЬ, НО ВСЕ КОЛИЧЕСТВА ОКРУГЛИЛИСЬ В НОЛЬ, аварийный выход
            # обязан проходить: денег шаг не двигает, а отказ через три POSTPONED даёт MIXED.
            ...
            try:
                ...
                # И ЯВНАЯ ВЕТКА ТОЖЕ (честные ворота 1: её raise не исполнялся никем —
                # стенд кормил относительными путями только ветку окружения).
                try:
                    _ST45.book_lock_dir(Path('book-F.json'))
                    out['относительный_явный_отвергнут'] = False
                except ValueError:
                    out['относительный_явный_отвергнут'] = True
                # ИСТОЧНИК В СООБЩЕНИИ НАЗВАН ПРАВДИВО (седьмой прогон, №13). Боевые
                # вызывающие передают book_path(route), то есть саму ADDFUT_BOOK_PATH, и
                # сообщение «аргумент book_lock_dir» указывало оператору на величину,
                # которой на его уровне нет. Обе стороны: путь ИЗ окружения и путь, ему не
                # равный.
                _os45.environ['ADDFUT_BOOK_PATH'] = 'rel/book-F.json'
                def _msg45(_a):
                    try:
                        _ST45.book_lock_dir(_a)
                        return ''
                    except ValueError as _e45s:
                        return str(_e45s)
                out['источник_окружения_назван'] = _msg45('rel/book-F.json').startswith(
                    'ADDFUT_BOOK_PATH =')
                out['источник_аргумента_назван'] = _msg45('другой/book-F.json').startswith(
                    'аргумент book_lock_dir =')
                # ТРЕТИЙ ВХОД — ЗАКОННЫЙ (ворота 8в-1): обе пробы выше относительные и
                # падают всегда, поэтому успешное плечо зонда не исполнялось. Заодно это
                # утверждение по существу: абсолютный путь обязан приниматься.
                out['абсолютный_аргумент_принят'] = _msg45('/tmp/книга/book-F.json') == ''
            ...
            out['ok'] = all([out['голый_и_явный_совпали'], out['замок_у_книги'],
                             ...
                             out['относительный_явный_отвергнут'],
                             out['окружение_сильнее'], out.get('замок_берётся') is True,
                             out['источник_окружения_назван'],
                             out['источник_аргумента_назван'],
                             out['абсолютный_аргумент_принят']])
            ...
            # БОЕВАЯ ФУНКЦИЯ, А НЕ ПЕРЕСКАЗ (разбор /code-review 21.08): стенд повторял
            # выражение отбора у себя, поэтому всё, что main() делает ПОСЛЕ него, не
            # наблюдалось ничем. Отбор вынесен в diagnose.classify — его и зовём.
            def _causes(body):
                return [c for c, _t in _DG45.classify(body)]
            _c1 = _causes('\n'.join([_kap, _zdor]))
            ...
            # ТЕЛА С КОДОМ — ТЕ, ГДЕ ПРОЗА ОДНА ДАЁТ НЕВЕРНЫЙ ОТВЕТ (ворота 3). Оба взяты
            # у производителей: успешный срез пишет СОКРАЩЕНА заглавными, а к строке
            # пробоя приклеивается хвост «после среза НЕИЗВЕСТЕН». Без кода перечень слов
            # отвечает «не классифицирована» и «исход заявки» соответственно.
            _m = _DG45.CAUSE_MARK
            _c5 = _causes(f'{_m} O3E-ПРОБИТ\nзапас О-3-Е ПОСЛЕ исполнений 2.10x '
                          f'(порог 1.4). КНИГА СОКРАЩЕНА В ТУ ЖЕ СЕССИЮ по нормативу §8')
            out['код_сильнее_прозы'] = any('ниже норматива О-3-Е' in c for c in _c5)
            _c6 = _causes(f'{_m} O3E-НЕИЗВЕСТЕН\nзапас О-3-Е при живой книге НЕИЗВЕСТЕН '
                          f'(брокер не вернул требование)')
            ...
            # ВЕТКА СЧЁТА ИСПОЛНЯЕТСЯ, А НЕ ЧИСЛИТСЯ (разбор /code-review 22.08): без
            # заданного пина она не бралась НИ РАЗУ за всю батарею, то есть защита «замер
            # без счёта не заверяется молча» была ненаблюдаема, а мутация, удаляющая её,
            # выжила бы. Пин задаётся здесь явно и снимается в finally.
            _keepa = _os45.environ.get('ADDFUT_ACCOUNT')
            try:
                _os45.environ['ADDFUT_ACCOUNT'] = 'DU000001'
                _ok_meta = dict(series=['ESU26', 'ZNU26'],
                                con_ids={'ESU26': '1', 'ZNU26': '2'})
                out['чужой_счёт_виден'] = 'на счёте DU999999' in _pair(
                    _fut, {'_meta': dict(_ok_meta, account='DU999999')})
                out['свой_счёт_молчит'] = (_pair(
            ...
            out['ok'] = all([out['согласованное_молчит'], out['фонды_не_мешают'],
                             ...
                             out['чужой_счёт_виден'], out['свой_счёт_молчит'],
                             out['замер_без_счёта_виден']])
            ...
            (out['закреплённое_читается_раз'],
             out['живое_не_залипает'],
        ...
        elif kind == 'правила45: ворота покрытия наблюдают сами себя':
            # ПРАВИЛО 4 ДЛЯ ФИЛЬТРОВ ВОРОТ (разбор /code-review 22.08): три фильтра шума
            # превращают бывший отказ в молчание, и каждый уже был дефектен в первый день
            # (затопление составными; мёртвый канал infra; снятая однобокость). Мутацию на
            # tools-скрипт не повесить — наблюдателем становится батарея: синтетический
            ...
            _m.write_text('def f(x):\n'
                          ...
                          'except:\n'
                          '    r = 3\n'
                          # УЗКИЙ перехват (седьмой прогон, №9): фикстура состояла из одних
                          # перехватов-всех, поэтому мутация «_is_catchall всегда истинно»
                          # была НЕНАБЛЮДАЕМА — а она глушит канал однобокости для ВСЕХ
                          # except-строк репозитория разом.
                          'try:\n'
                          '    u = 1\n'
                          'except ValueError:\n'
                          '    u = 2\n'
                          # ASSERT С СООБЩЕНИЕМ и ЦЕПОЧКА СРАВНЕНИЙ (седьмой прогон, №8):
                          # оба плеча ленивости добавлены шестым прогоном и не имели в
                          # фикстуре ни одного примера — удаление любого из них оставляло
                          # самопроверку зелёной.
                          'assert y, (\'сообщение\'\n'
                          '           \' продолжение\')\n'
                          'w = (1 <\n'
                          '     2 <\n'
                          '     3)\n', encoding='utf-8')
            # ОСВОБОЖДЕНИЕ ПО СОБСТВЕННОМУ ОБЪЯВЛЕНИЮ — ПОД НАБЛЮДЕНИЕМ (27.08.2026,
            # находка №24 сплошного аудита). Файл может объявить у себя ADDFUT_ПОКРЫТИЕ и
            # уйти в справочный ярус, то есть выйти из-под ворот. Механизм осмыслен —
            # объявление живёт рядом с кодом, — но у САМОГО механизма не было ни стенда, ни
            # мутации: подмена _own_run на «всегда строка» освободила бы ВЕСЬ контур молча,
            # и ворота остались бы зелёными. Ровно тот класс, ради которого ворота и
            # заведены, только внутри них самих.
            # Проверяются обе стороны на подложных файлах: боевой без объявления обязан
            # остаться боевым, с объявлением — уйти в справку и НАЗВАТЬ причину.
            _mb = _d / 'boevoi.py'
            _mb.write_text('x = 1\n', encoding='utf-8')
            _ms = _d / 'spravka.py'
            _ms.write_text("ADDFUT_ПОКРЫТИЕ = 'оператор вручную'\nx = 1\n", encoding='utf-8')
            _mp = _d / 'pustoe.py'
            _mp.write_text("ADDFUT_ПОКРЫТИЕ = ''\nx = 1\n", encoding='utf-8')
            # ВРЕМЕННЫЙ КАТАЛОГ ВНОСИТСЯ В КОНТУР НА ВРЕМЯ ПРОВЕРКИ. Без этого файлы лежат
            # ВНЕ контура, и _tier_of отдаёт «справочно» всем троим по совершенно другой
            # причине — первая редакция стенда так и получила три False и приняла бы их за
            # опровержение правки. Проверять надо ту ветку, о которой речь.
            _keepC = list(_bc.CONTOUR)
            _bc.CONTOUR = list(_bc.CONTOUR) + [_d]
            try:
                _tб, _пб = _bc._tier_of(str(_mb), {})
                _tс, _пс = _bc._tier_of(str(_ms), {})
                _tп, _пп = _bc._tier_of(str(_mp), {})
            finally:
                _bc.CONTOUR = _keepC
            out['без_объявления_боевой'] = _tб == 'боевой'
            out['с_объявлением_справочно'] = (_tс == 'справочно' and 'оператор вручную' in _пс)
            # ПУСТОЕ ОБЪЯВЛЕНИЕ НЕ ОСВОБОЖДАЕТ: пустая строка — это не объяснение покрытия,
            # а способ выйти из-под ворот, не сказав ничего.
            out['пустое_объявление_не_освобождает'] = _tп == 'боевой'
            ...
            # БЕЗ ТАВТОЛОГИИ (седьмой прогон, №7): конъюнкт `8 in _ex45` не мог быть ложным
            # НИКОГДА — credit_spans начинает с set(executed) и только объединяет, семя не
            # удаляется. Утверждение выглядело вдвое сильнее, чем было.
            out['мёртвая_ветвь_не_зачтена'] = (9 not in _ex45)
            # СЕМЯ — МЕНЬШАЯ строка, ПРОВЕРЯЕТСЯ БОЛЬШАЯ (седьмой прогон, №7): прежде семя
            # было {7}, а проверялось `6 in`, и это верно для ОБОИХ плеч — min({6,7})==6,
            # то есть мутация «всё лениво» выживала. Теперь неленивый спан обязан отдать 7.
            _ex46 = _bc.credit_spans({6}, _spans)
            out['неленивое_склеивается_потреблением'] = (7 in _ex46)
            out['узкий_перехват_не_опознан'] = (27 not in _catch)
            out['assert_с_сообщением_ленив'] = any(
                _sp == {29, 30} and _lz for _sp, _lz in _spans)
            out['цепочка_сравнений_ленива'] = any(
                _sp == {31, 32, 33} and _lz for _sp, _lz in _spans)
            _exe = _bc.executable(str(_m))
            out['вселенная_исполняемая'] = (6 in _exe and 5 not in _exe)
            # РЕШАТЕЛИ ЯРУСА — ПОД НАБЛЮДЕНИЕМ (восьмой прогон /code-review). _tier_of и
            # _one_sided_is_fatal решают, упадут ли ворота вообще, и не имели ни стенда, ни
            # мутации: опечатка `in` вместо `not in` делала ворота тождественно зелёными, и
            # ни батарея, ни мутации, ни сами ворота этого не увидели бы.
            _R46 = str(_bc.ROOT)
            out['ярус_боевой_без_загрузки'] = (
                _bc._tier_of(_R46 + '/r33build/live/margin_check.py', {})[0] == 'боевой')
            out['ярус_справочный_вне_пакета'] = (
                _bc._tier_of(_R46 + '/tools/release.py', {})[0] == 'справочно')
            out['однобокость_стенда_не_отказ'] = (
                _bc._one_sided_is_fatal(_R46 + '/r33build/live/invariants.py') is False
                and _bc._one_sided_is_fatal(_R46 + '/r33build/live/state.py') is True)
            out['ok'] = all([out['тела_не_затапливаются'],
                             ...
                             out['узкий_перехват_не_опознан'],
                             out['assert_с_сообщением_ленив'],
                             out['цепочка_сравнений_ленива'],
                             out['ярус_боевой_без_загрузки'],
                             out['ярус_справочный_вне_пакета'],
                             out['однобокость_стенда_не_отказ'],
                             out['вселенная_исполняемая'],
                             # ОСВОБОЖДЕНИЕ ПО СОБСТВЕННОМУ ОБЪЯВЛЕНИЮ — В ПРИГОВОР, А НЕ
                             # РЯДОМ С НИМ (27.08.2026). Первая редакция этих трёх проверок
                             # вычисляла их и НЕ включала в ok: они были ложны, а случай
                             # оставался зелёным. Это ровно класс «вычислено, но не судится»
                             # — тот самый, что закрывался сегодня в находке №17.
                             out['без_объявления_боевой'],
                             out['с_объявлением_справочно'],
                             out['пустое_объявление_не_освобождает']])
            ...
            try:
                ...
                _cids = {r['instrument']: r['con_id']
                         ...
                         if _TR47f.is_fut_name(r['instrument'])}
                ...
                def _bookF():
                    ...
                    # prev_close_lev ВОЗВРАЩЁН (седьмой прогон, №15). В шестом я снял его
                    # с обоснованием «поле не читает никто на исполняемом пути» — это НЕВЕРНО:
                    # daily.step сверяет с ним кап (§1), session.py читает трижды. Хуже того,
                    # фикстура стала противоречить себе: selfcheck выпуска по-прежнему сеет
                    # 1.99, и строка ИТОГ этого же случая пишет leverage='1.99', а книга
                    # уходила с умолчанием 0.0 — то есть направление «у самого капа» не
                    # проверялось вовсе, и ничто не краснело.
                    return _DL47.Book(d_fix=7.9, n_e=0, n_b=1, unit_is_mes=True,
                                      prev_st_eq=True, prev_st_bd=True, ser_a='U26',
                                      ser_b='U26', es_held=0, prev_close_lev=1.99,
                ...
                class _Br:
                    ...
                    def __init__(_b, ok=True, frac=False):
                        # frac — ДРОБНЫЙ ОСТАТОК ФЬЮЧЕРСА ПОСЛЕ ЗАКРЫТИЯ (седьмой прогон,
                        # ворота 8в-1). Дробь СОЗДАЁТСЯ ИСПОЛНЕНИЕМ, а не лежит с начала:
                        # первая редакция фикстуры сеяла 1.25 в исходную книгу и упиралась
                        # в РАННИЙ барьер «книга не соответствует плану» — проверялось не то
                        # место. Настоящий источник дроби — частичный фил при закрытии.
                    ...
                    def sell_units(_b, i, u):
                        ...
                        _b.np[i] = _b.np.get(i, 0) - int(u) + (0.25 if _b.frac else 0)
                        return (f's{_b.n}', u)
                ...
                def _restore_mr47(_target, _out):
                    """Вернуть служебный каталог МР. ОТДЕЛЬНОЙ функцией (ворота 8в-1):
                    отказное плечо стояло инлайном в finally и не исполнялось НИ РАЗУ —
                    именно оно и было мёртвым в первой редакции (out['ok']=False, затираемое
                    строкой ниже). Здесь у правила одна точка, и оба плеча берутся пробой."""
                    try:
                        _M47.set_state_dir_for_tests(_target)
                        return True
                    except Exception as _exmr:
                        _out['error'] = (_out.get('error') or '') + \
                            f' | каталог МР не восстановлен: {_exmr}'
                        return False

                def _reset47():
                    """Полный сброс между сценариями — ОДНОЙ функцией (седьмой прогон):
                    блок был скопирован дословно, а сценариев стало три."""
                    for _f in (_sp47, _sx):
                        if _f.exists():
                            _f.unlink()
                    _jx.write_text('asof,event,detail\n'
                ...
                # ПУТЬ ОТКАТА d_fix ИСПОЛНЯЕТСЯ ПО-НАСТОЯЩЕМУ (слой 5, шаг 3). Правило
                # «нога Б жива» вынесено и наблюдаемо мутацией, но его МЕСТО ВЫЗОВА батарея
                # не исполняла: предполёт клал значение в _PREFLIGHT_DFIX, и вся иерархия
                # источников до отката не доходила. Кэш очищается — у стаба нет .ib, живая
                # доходность недоступна, и управление доходит до старой книги Ф.
                _TR47._PREFLIGHT_DFIX.clear()
                _b1 = _Br(ok=True)
                ...
                # ПРИЧИНА ПРОВЕРЯЕТСЯ ПЕРЕНОСОМ, А НЕ ТЕКСТОМ УМОЛЧАНИЯ (седьмой прогон,
                # №11). Здесь пришпиливалась строка 'О-3-Е' — а она приходит из УМОЛЧАНИЯ
                # оркестратора, срабатывающего лишь потому, что стаб не задаёт своей
                # причины. Такой якорь краснел на переформулировке сообщения оператору
                # (поведение то же) и молчал бы, начни брокер возвращать свою причину — а в
                # бою это обычный случай: из четырнадцати причин отказа предпросмотра в
                # ib_broker лишь три содержат 'О-3-Е'. Проверяем СВОЙСТВО: причина брокера
                # доходит до вердикта неискажённой.
                _reset47()
                _b2w = _Br(ok=False)
                _b2w._preview_why = 'СТЕНД-ПРИЧИНА: шлюз не вернул начальную маржу'
                _r2w = _TR47.execute(_b2w, str(_sp47), 1e6, _L, **_кв)
                out['причина_брокера_доходит'] = (
                    'СТЕНД-ПРИЧИНА' in str(_r2w.get('why', ''))
                    and _r2w.get('status') == 'POSTPONED')
                # ДРОБНЫЙ ОСТАТОК ФЬЮЧЕРСА НЕ ПРОХОДИТ В COMPLETE (ворота 8в-1: тело цикла
                # запрета не исполнялось батареей НИ РАЗУ — правило было наблюдаемо только
                # поштучно, на самой fractional_fut, но не на своём месте в оркестраторе).
                _reset47()
                _b3 = _Br(ok=True, frac=True)
                try:
                    _r3 = _TR47.execute(_b3, str(_sp47), 1e6, _L, **_кв)
                except Exception as _ex3:      # запрет может прийти и исключением
                    _r3 = {'status': type(_ex3).__name__, 'why': str(_ex3)}
                out['дробный_остаток_не_complete'] = (
                    _r3.get('status') != 'COMPLETE'
                    and 'дробная фьючерсная' in str(_r3))
                # ОТКАЗНОЕ ПЛЕЧО ВОССТАНОВЛЕНИЯ КАТАЛОГА МР (ворота 8в-1). Невосстановленный
                # каталог оставляет mr_engine._STATE на удалённом /tmp ВСЕМ последующим
                # случаям батареи — класс, уже затиравший боевую метку выкладки. Плечо
                # исполняется только при отказе, поэтому отказ подставляется.
                _keep_scfg = _M47.set_state_dir_for_tests
                _probe_out = {}
                try:
                    _M47.set_state_dir_for_tests = lambda *_a, **_k: (
                        _ for _ in ()).throw(OSError('каталог занят (проба)'))
                    _bad_restore = _restore_mr47(_keep_mrdir, _probe_out)
                finally:
                    _M47.set_state_dir_for_tests = _keep_scfg
                out['отказ_восстановления_виден'] = (
                    _bad_restore is False
                    and 'каталог МР не восстановлен' in _probe_out.get('error', ''))
            finally:
                ...
                for _k, _v in _keep_env.items():
                    if _v is None:
                        ...
                        _os45.environ[_k] = _v
                # СВОИМ КЛЮЧОМ, А НЕ out['ok'] (седьмой прогон /code-review, №1). Здесь
                # стояло out['ok'] = False, и оно было МЁРТВЫМ: строка ниже присваивает
                # out['ok'] безусловно и затирает его. До этой правки тот же отказ
                # выбрасывался наружу и красил случай через raised — то есть правка,
                # задуманная как аккуратность, превратила КРАСНЫЙ в ЗЕЛЁНЫЙ. Цена ошибки
                # не теоретическая: невосстановленный каталог МР оставляет
                # mr_engine._STATE указывающим на удалённый /tmp ВСЕМ последующим случаям
                # батареи — ровно тот класс, что уже затёр боевую метку выкладки.
                out['каталог_мр_восстановлен'] = _restore_mr47(_keep_mrdir, out)
            out['ok'] = all([out['завершается'], out['предпросмотр_звался_с_планом'],
                             out['продажа_и_покупка_состоялись'],
                             out['отказ_откладывает'],
                             out['отказал_именно_предпросмотр'],
                             out['каталог_мр_восстановлен'],
                             out['причина_брокера_доходит'],
                             out['дробный_остаток_не_complete'],
                             out['отказ_восстановления_виден']])
            ...
            import contracts as _CT46, daily as _DL46
            ...
            try:
                ...
                # ОДНА ТОЧКА КОРНЕЙ — ДВУМЯ НЕЗАВИСИМЫМИ СПОСОБАМИ (седьмой прогон, №4).
                # Прежний сторож считал ВХОЖДЕНИЯ ТЕКСТА "('ES', 'MES', 'ZN')" в исходнике
                # feed и требовал ровно два. Он краснел на ВЕРНОМ исправлении (импорт вместо
                # копии даёт ноль вхождений) и зеленел на настоящем разъезде (комментарий с
                # той же строкой держит счёт, пока боевая строка уезжает в ('ES','MES','ZB')),
                # а любая смена кавычек или переносов делала его красным навсегда — то есть
                # он блокировал КАЖДЫЙ выпуск, пока не тронешь сам сторож.
                # (а) СИНТАКСИСОМ, А НЕ ТЕКСТОМ: ни в одном боевом модуле нет литерального
                #     кортежа корней. AST безразличен к кавычкам, порядку, переносам и
                #     комментариям — тем самым отличиям, на которых текстовый счёт врал.
                import ast as _ast46
                _roots_set = set(_CT46.FUT_ROOTS)

                def _scan46(_path46):
                    """Литеральные кортежи корней в файле. ОТДЕЛЬНОЙ функцией с
                    положительным контролем ниже: на чистом дереве список пуст ВСЕГДА,
                    поэтому строка добавления не исполнялась ни разу — сломанный обход
                    (например, сравнение списка с множеством) молча давал бы «копий нет»."""
                    _hits46 = []
                    for _nd46 in _ast46.walk(_ast46.parse(Path(_path46).read_text('utf-8'))):
                        if (isinstance(_nd46, _ast46.Tuple)
                                and all(isinstance(_e, _ast46.Constant)
                                        and isinstance(_e.value, str) for _e in _nd46.elts)
                                and {_e.value for _e in _nd46.elts} == _roots_set):
                            _hits46.append(f'{Path(_path46).name}:{_nd46.lineno}')
                    return _hits46

                # ОБХОД — ПО ВСЕМУ ПАКЕТУ, А НЕ ПО ТРЁМ МОДУЛЯМ (восьмой прогон, №4).
                # Прежний список файлов был тем же «списком имён»: живые копии кортежа
                # лежали в selfcheck_v192.py, contour_test.py и mutation.py и не назывались.
                # Дом канона (contracts.py) исключён явно — там кортеж и обязан быть.
                _copies = [h for _f46 in sorted(Path(_CT46.__file__).parent.parent.rglob('*.py'))
                           if _f46.name != 'contracts.py'
                           for h in _scan46(_f46)]
                # ТАБЛИЦЫ, КЛЮЧУЕМЫЕ КОРНЕМ, ОБЯЗАНЫ ПОКРЫВАТЬ КАНОН (восьмой прогон, №4).
                # AST-обход видит кортеж и не видит КЛЮЧИ СЛОВАРЯ, а именно так живут
                # маржа по корню, модельный множитель и множитель журнала: добавили корень
                # в FUT_ROOTS, забыли в FUT_MARGIN — предполёт считает маржу без него.
                # Проверяется не форма записи, а само свойство — полнота покрытия.
                import journal as _J46
                out['таблицы_по_корням_покрывают_канон'] = (
                    set(_TR46.FUT_MARGIN) == set(_CT46.FUT_ROOTS)
                    and set(_FD46.MODEL_MULT) == set(_CT46.FUT_ROOTS)
                    and set(_CT46.FUT_ROOTS) <= set(_J46.MULT))
                # Конъюнкт set(FUT_LEGS)==set(FUT_ROOTS) снят (девятый прогон): он не мог
                # быть ложным — contracts выводит одно из другого, и проверка сторожила
                # собственную строку вывода.
                # ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ: обход обязан НАХОДИТЬ копию, иначе его молчание
                # ничего не значит. Порядок и кавычки нарочно иные, чем в каноне.
                _fx46 = Path(_tf45.mkdtemp(prefix='addfut-roots46-')) / 'копия.py'
                _fx46.write_text('x = ("ZN",\n     "ES", "MES")\n', encoding='utf-8')
                out['обход_находит_копию'] = len(_scan46(_fx46)) == 1
                # (б) ПОВЕДЕНИЕМ: подменяем канон и требуем, чтобы за ним пошли. Ловит то,
                #     чего не видит AST: from-импорт связывает кортеж в момент импорта, и
                #     потребитель с виду «читает канон», а на деле держит свой снимок.
                def _вердикт46():
                    """ЧТО ИМЕННО отвечает state на YMU26 — текстом отказа, а не «да/нет».

                    Девятый прогон: прежняя форма возвращала bool «посторонняя ли позиция»,
                    и после того как соседняя правка научила check_one_series отвергать
                    корень без ноги, проба стала зелёной ПО ЧУЖОЙ ПРИЧИНЕ — «не посторонняя»
                    оказывалось истинным из-за совсем другого отказа. Проверяем СМЕНУ
                    вердикта: до подмены канона имя чужое, после — уже своё, но без
                    модельной единицы. Одна точка присваивания, мёртвых плеч нет.
                    """
                    _m = ''
                    try:
                        _ST46.book_from_broker(_DL46.Book, {'YMU26': 1.0}, 'F')
                    except ValueError as _e46:
                        _m = str(_e46)
                    return _m

                # ДО подмены YMU26 обязан быть чужим, иначе _alien46()==False после подмены
                # ничего не доказывает (угол «от отрицания»).
                _до46 = _вердикт46()
                _keep_roots46 = _CT46.FUT_ROOTS
                try:
                    _CT46.FUT_ROOTS = tuple(_keep_roots46) + ('YM',)
                    _follow = (_TR46.is_fut_name('YMU26')
                               and _TR46.fut_series('YMU26') == 'U26'
                               and _TR46._bare_dst(['YM']) == ['YM']
                               and 'посторонние' not in _вердикт46())  # канон дошёл
                finally:
                    _CT46.FUT_ROOTS = _keep_roots46
                out['корни_чужой_до_подмены'] = 'посторонние' in _до46
                out['корни_одна_точка'] = (not _copies) and _follow and ('посторонние' in _до46)
                out['корни_копии'] = _copies       # в отчёт: где именно завелась копия
                # Ключ «канон восстановлен» снят (девятый прогон): сравнение с
                # _keep_roots46 стало тождеством — finally присваивает ровно этот объект
                # двумя строками выше. Восстановление гарантирует finally, а не проверка.
                ...
                out['серийная_цель_проходит'] = ('БЕЗ поставочной серии' not in _r7
                                                 and 'd_fix' in _r7)
                # ЦЕЛЬ БЕЗ НОГИ Б (ворота 8в-1): признак _zn_target вычисляется
                # генератором, и батарея давала ему только цели, СОДЕРЖАЩИЕ ZN — сторона
                # «ни одного ZN» не бралась ни разу, то есть требование свежей d_fix
                # включалось всегда и его отключение было ненаблюдаемо.
                _r8 = _pf(_dst_names=('ESU26',))
                # ЦЕЛЬ БЕЗ ZN не включает ЗАПРЕТ ПО НОГЕ Б (сторож 1188), но по-прежнему
                # упирается в ОБЩЕЕ требование d_fix (сторож 1202) — он не смотрит на состав
                # цели. Пришпиливаем ИМЕННО это различие: моё первое утверждение («без ноги Б
                # d_fix не требуется») было ДОГАДКОЙ и оказалось ложным — сторожа два.
                out['цель_без_ноги_б_минует_запрет_ноги'] = (
                    'нога Б открывается заново' not in _r8
                    and 'd_fix восстановить нечем' in _r8)
                # ЗАПРЕТ ПО НОГЕ Б НАБЛЮДАЕТСЯ ТОЛЬКО С ЗАКРЫТОЙ КАЛИТКОЙ. Батарея держит
                # ADDFUT_DFIX_TEST=1, и сторож 1188 пропускается ВСЕГДА — поэтому цель с ZN и
                # без ZN давали одинаковый отказ (общий сторож d_fix), а разница по _zn_target
                # была ненаблюдаема. Здесь калитка на время закрывается.
                _keep_dfix46 = _os45.environ.get('ADDFUT_DFIX_TEST')
                try:
                ...
                out['цель_без_ноги_б_минует_его_и_с_калиткой'] = (
                    'нога Б открывается заново' not in _r10)
                # КОНТРАКТ ИМЁН ЦЕЛИ — ОБА НЕЗАКОННЫХ ВХОДА (ворота 8в-1: тело raise новой
                # защиты батарея не исполняла ни разу; правило 4 — у защиты обязан быть
                # наблюдатель, а не только мой разовый зонд).
                def _dst46(_d):
                    try:
                        _TR46._check_dst_names(_d)
                        return 'пропущено'
                    except TypeError as _e46t:
                        return 'контракт' if 'нужен список или кортеж' in str(_e46t) else 'чужой'
                out['имена_none_отвергнуты'] = _dst46(None) == 'контракт'
                out['имена_строкой_отвергнуты'] = _dst46('ZNU26') == 'контракт'
                out['законный_кортеж_проходит_вход'] = _dst46(('ZNU26',)) == 'пропущено'
                out['итератор_отвергнут'] = _dst46(iter(['ZNU26'])) == 'контракт'
                out['множество_отвергнуто'] = _dst46({'ZNU26'}) == 'контракт'
                # ПРЕДПОЛЁТ ЗОВЁТ ИМЕННО ЭТУ ФУНКЦИЮ, А НЕ СВОЮ КОПИЮ. Проверяется
                # подменой общей точки: если предполёт правило пересказывает, подмена до
                # него не дойдёт и он отвергнет None по-прежнему. (Первая редакция этой
                # строки была тавтологией — повторным вызовом _dst46; поймано при чтении.)
                # ПРЯМОЕ НАБЛЮДЕНИЕ ТОЧКИ ВЫЗОВА. Без него подмена ниже зелена и когда
                # правило ВЫРЕЗАНО из предполёта совсем: «сообщения нет» — общий ответ и на
                # «правило подменено», и на «правило не зовётся». Пара обязана быть.
                _pf_live46 = ''
                try:
                    _TR46._preflight_handover('E', 'F', _dst_names=None)
                except Exception as _e46l:
                    _pf_live46 = str(_e46l)
                out['предполёт_отвергает_none'] = 'нужен список или кортеж' in _pf_live46
                _keep_chk46 = _TR46._check_dst_names
                try:
                    _TR46._check_dst_names = lambda _d: None      # правило выключено
                    # ОДНА ТОЧКА ПРИСВОЕНИЯ, БЕЗ РАЗБОРА ИСХОДОВ: предполёт без фикстурной
                    # книги падает ВСЕГДА, поэтому плечи «без исключения» и «прошёл вход»
                    # были недостижимы — три мёртвые строки в зонде, который сам же и
                    # написан против мёртвого кода.
                    _pf_none46 = ''
                    try:
                        _TR46._preflight_handover('E', 'F', _dst_names=None)
                    except Exception as _e46p:
                        _pf_none46 = str(_e46p)
                finally:
                    _TR46._check_dst_names = _keep_chk46
                out['контракт_зовётся_предполётом'] = 'нужен список или кортеж' not in _pf_none46
                # РЕЕСТР ПОКРЫВАЕТ БЛИЖАЙШИЙ РОЛЛ (ворота 8в-1: строка требования серий у
                # всех корней не исполнялась батареей — ветка берётся только когда срок
                # ролла попал в горизонт). Обе стороны: реестр с целевой серией и без неё.
                import ib_stub as _IBS46x
                _bh46 = _DL46.Book(ser_a='U26', ser_b='U26')
                _keep_reg46 = _os45.environ.get('ADDFUT_REGISTRY')
                try:
                    _os45.environ['ADDFUT_REGISTRY'] = str(_IBS46x.fixture_registry(_bd))
                    _ok46, _msg46 = _FD46.registry_horizon(_bh46, today='2026-08-20')
                    out['горизонт_реестра_считается'] = isinstance(_ok46, bool) and bool(_msg46)
                finally:
                ...
                out['дробный_фьючерс_ловится'] = _TR46.fractional_fut({'ZNU26': 2.5}) == ['ZNU26']
                out['дробная_доля_фонда_законна'] = _TR46.fractional_fut({'CSPX': 0.3}) == []
                out['целый_фьючерс_проходит'] = _TR46.fractional_fut({'MESU26': 6.0}) == []
                # РАЗБОР ПОЗИЦИЙ ПО НОГАМ — ВСЕ ТРИ ВЕТКИ (седьмой прогон, ворота 8в-1).
                # Ворота показали, что ветки MES/ES/ZN в book_from_broker не исполнялись
                # батареей НИ РАЗУ: книга из фактических позиций строится только после
                # перехода, и стенд туда не заходил. Здесь исполняются все три сразу,
                # с проверкой упаковки сетки (2 ES = 20 MES, плюс 6 MES) и обеих серий.
                _bk46 = _ST46.book_from_broker(_DL46.Book,
                                               {'MESU26': 6, 'ESU26': 2, 'ZNU26': 10}, 'F')
                out['книга_из_позиций_разбирает_ноги'] = (
                    _bk46.n_e == 26 and _bk46.es_held == 2 and _bk46.n_b == 10
                    and _bk46.ser_a == 'U26' and _bk46.ser_b == 'U26')
                # ЯВНО ПЕРЕДАННАЯ СЕРИЯ НЕ ПЕРЕТИРАЕТСЯ ПОЗИЦИЯМИ (ворота 8в-1: левое плечо
                # `ser_b or ...` недостижимо, пока стенд не пользуется этим параметром —
                # у ноги Б один корень, поэтому само по себе оно не короткозамкнётся).
                _bk46b = _ST46.book_from_broker(_DL46.Book, {'ZNU26': 10}, 'F', ser_b='Z26')
                out['явная_серия_не_перетирается'] = _bk46b.ser_b == 'Z26'
                # КОРЕНЬ НОГИ А БЕЗ ИЗВЕСТНОГО МНОЖИТЕЛЯ — ОТКАЗ (восьмой прогон): ветка не
                # исполнялась, а она стоит ровно на пути четвёртого корня, ради которого
                # канон и сводили в одну точку.
                _keep_legs46 = _CT46.FUT_LEGS
                try:
                    _CT46.FUT_LEGS = dict(_keep_legs46, YM='А')
                    _CT46.FUT_ROOTS = tuple(_CT46.FUT_LEGS)
                    # ОДНА ТОЧКА ПРИСВОЕНИЯ: плечо «принят» недостижимо (корень ноги А
                    # без множителя обязан отвергаться всегда), и оставлять его значит
                    # заводить мёртвую строку в зонде против мёртвого кода.
                    _мн46 = ''
                    try:
                        _ST46.book_from_broker(_DL46.Book, {'YMU26': 1}, 'F')
                    except ValueError as _e46m:
                        _мн46 = str(_e46m)
                finally:
                    _CT46.FUT_LEGS = _keep_legs46
                    _CT46.FUT_ROOTS = _keep_roots46
                out['корень_ноги_а_без_множителя_отвергнут'] = (
                    'модельной единицы для него в книге нет' in _мн46)
                # И ТА ЖЕ ПРОБА ДЛЯ НОГИ Б (девятый прогон): прежняя редакция отвергала
                # неизвестный корень только на ноге А, а на Б молча суммировала — с
                # четвёртым корнем ZB книга давала n_b=17.
                _keep_legs46b = _CT46.FUT_LEGS
                _мнБ46 = ''
                try:
                    _CT46.FUT_LEGS = dict(_keep_legs46b, ZB='Б')
                    _CT46.FUT_ROOTS = tuple(_CT46.FUT_LEGS)
                    try:
                        _ST46.book_from_broker(_DL46.Book, {'ZNU26': 10, 'ZBU26': 7}, 'F')
                    except ValueError as _e46b:
                        _мнБ46 = str(_e46b)
                finally:
                    _CT46.FUT_LEGS = _keep_legs46b
                    _CT46.FUT_ROOTS = _keep_roots46
                out['корень_ноги_б_без_единицы_отвергнут'] = (
                    'модельной единицы для него в книге нет' in _мнБ46)
                # ТРЕТЬЯ НОГА — ОТКАЗ, А НЕ ГОЛЫЙ KeyError (девятый прогон): вселенная ног
                # в check_one_series зашита {'А','Б'}, и корень с любой иной ногой ронял
                # трассировку внутри hand_over_book, то есть уже ПОСЛЕ перевода денег.
                _мнВ46 = ''
                try:
                    _CT46.FUT_LEGS = dict(_keep_legs46b, ZB='В')
                    _CT46.FUT_ROOTS = tuple(_CT46.FUT_LEGS)
                    try:
                        _ST46.book_from_broker(_DL46.Book, {'ZBU26': 7}, 'F')
                    except ValueError as _e46v:
                        # ТОЛЬКО ValueError: голый KeyError — это и есть дефект, ради
                        # которого проба заведена, и ловить его тут значило бы завести
                        # плечо, которое на исправном коде не исполняется никогда.
                        # Незаявленный KeyError уронит случай, и это верный исход.
                        _мнВ46 = str(_e46v)
                finally:
                    _CT46.FUT_LEGS = _keep_legs46b
                    _CT46.FUT_ROOTS = _keep_roots46
                # СЛОЙ 5, ШАГ 3 (правило 8б). Сторож «цель перехода в УХОДЯЩЕЙ серии» —
                # это P0 сорок четвёртого круга №3, и измерение 25.08 показало, что БАТАРЕЯ
                # НЕ ИСПОЛНЯЕТ ЕГО НИ РАЗУ: ни накопление списка просроченных целей, ни оба
                # его отказа. Защита была закрыта кодом и не наблюдаема ничем. Ветка берётся
                # только когда срок ролла целевой серии уже наступил, поэтому день задаётся
                # подменой биржевого календаря, а не ожиданием нужной даты.
                import pandas as _pd46
                _keep_today46 = _FD46.exchange_today
                try:
                    _FD46.exchange_today = lambda: _pd46.Timestamp('2026-08-26')
                    _r_ух = _pf(_dst_names=('ESU26', 'ZNU26'))
                    _FD46.exchange_today = lambda: _pd46.Timestamp('2026-08-20')
                    _r_св = _pf(_dst_names=('ESU26', 'ZNU26'))
                    _FD46.exchange_today = lambda: (_ for _ in ()).throw(
                        ValueError('календарь недоступен (проба)'))
                    _r_нет = _pf(_dst_names=('ESU26', 'ZNU26'))
                finally:
                    _FD46.exchange_today = _keep_today46
                # СЛОЙ 5, ШАГ 3 (продолжение). ЛИЧНОСТЬ СЧЁТА В ПРЕДПОЛЁТЕ — три отказа,
                # ни один из которых батарея не исполняла. Денежный путь назван в самом
                # коде: маржа и машинная книга счёта A, позиции/NLV/заявки счёта B; после
                # COMPLETE ежедневный контур A получает состояние B. Ветка включается лишь
                # при переданном брокере, а стенды слоя зовут предполёт без него — потому
                # все три плеча и молчали.
                class _БрП46:                       # брокер с настраиваемой личностью
                    def __init__(_b, acct): _b.account = acct
                _keep_acc46 = _os45.environ.get('ADDFUT_ACCOUNT')
                try:
                    _os45.environ.pop('ADDFUT_ACCOUNT', None)
                    _r_бп = _pf(_dst_names=('ESU26',), _broker_p=_БрП46('DU1'))
                    _os45.environ['ADDFUT_ACCOUNT'] = 'DU1'
                    _r_бб = _pf(_dst_names=('ESU26',), _broker_p=_БрП46(None))
                    _r_чс = _pf(_dst_names=('ESU26',), _broker_p=_БрП46('DU2'))
                    _r_сс = _pf(_dst_names=('ESU26',), _broker_p=_БрП46('DU1'))
                finally:
                    # СНЯТЬ, ПОТОМ ВЕРНУТЬ, ЕСЛИ БЫЛО: развилка if/else держала бы одно
                    # плечо неисполнимым — в среде батареи ADDFUT_ACCOUNT не задан.
                    # БЕЗ ВЕТВЛЕНИЯ ВООБЩЕ: в среде батареи ADDFUT_ACCOUNT не задан,
                    # поэтому ЛЮБАЯ развилка восстановления держит одно плечо неисполнимым —
                    # ворота ловили меня на этом трижды за день, и каждый раз я писал новую
                    # развилку вместо безусловной формы. update пустым словарём исполняется
                    # всегда и возвращает значение только когда оно было.
                    _os45.environ.pop('ADDFUT_ACCOUNT', None)
                    _os45.environ.update({'ADDFUT_ACCOUNT': _keep_acc46} if _keep_acc46 else {})
                out['счёт_не_пинован_запрещает'] = 'не пинован' in _r_бп
                out['брокер_без_счёта_запрещает'] = 'не сообщает счёт' in _r_бб
                out['чужой_счёт_запрещает'] = 'а машинный пин' in _r_чс
                out['свой_счёт_проходит_личность'] = ('не пинован' not in _r_сс
                                                      and 'не сообщает счёт' not in _r_сс
                                                      and 'машинный пин' not in _r_сс)
                # СЛОЙ 5, ШАГ 3. Правило «нога Б жива» решает, можно ли откатиться к
                # d_fix СТАРОЙ книги Ф: при живом ZN откат означал бы месяцы оценки трежерис
                # по чужой дюрации (22-й круг №9, 33-й круг №3). Батарея эту строку не
                # исполняла ни разу — и перевод её на канон батчем 21 держался на зонде.
                # СЛОЙ 5, ШАГ 3. ОШИБКА КОДА НЕ ПЕРЕОДЕВАЕТСЯ В ПРЕДПОЛЁТНЫЙ ВЕРДИКТ —
                # запрет 45-го круга. Батарея не исполняла ни плечо перевозбуждения, ни
                # соседнее «снимок недоступен»: стенды звали предполёт без брокера, и до
                # согласованного снимка дело не доходило. Оба конца сразу: опечатка в коде
                # обязана прийти СВОИМ типом с трассировкой, а честная недоступность
                # снимка — доменным отказом.
                class _БрСн46:
                    account = 'DU1'
                    def __init__(_b, боль): _b.боль = боль
                    def open_orders(_b):
                        if _b.боль == 'код':
                            raise TypeError('опечатка в коде предполёта (проба)')
                        raise RuntimeError('шлюз не отвечает (проба)')
                    def net_positions(_b): return {}
                _keep_acc47 = _os45.environ.get('ADDFUT_ACCOUNT')
                try:
                    _os45.environ['ADDFUT_ACCOUNT'] = 'DU1'
                    # ОДНА ТОЧКА ПРИСВОЕНИЯ, ОДНО ПЛЕЧО: разбор исходов по нескольким
                    # except держит недостижимые ветви — предполёт на этом входе падает
                    # ВСЕГДА, и «без исключения» с «переодета» не берутся никогда.
                    # Спрашиваем ТИП, а не классифицируем заранее.
                    _к46 = ''
                    try:
                        _TR46._preflight_handover('E', 'F', _dst_names=('ESU26',),
                                                  _broker_p=_БрСн46('код'))
                    except BaseException as _e47:
                        _к46 = type(_e47).__name__
                    _r_сн = _pf(_dst_names=('ESU26',), _broker_p=_БрСн46('связь'))
                finally:
                    _os45.environ.pop('ADDFUT_ACCOUNT', None)
                ...
                out['нога_б_жива_видит_фонд'] = _TR46.нога_б_жива({'CBU0': 5}) is True
                out['нулевая_нога_б_не_жива'] = _TR46.нога_б_жива({'ZNU26': 0}) is False
                out['нога_а_не_считается_ногой_б'] = _TR46.нога_б_жива({'ESU26': 26}) is False
                out['уходящая_серия_отвергнута'] = 'УХОДЯЩЕЙ серии' in _r_ух
                out['свежая_серия_проходит_календарь'] = 'УХОДЯЩЕЙ серии' not in _r_св
                out['календарь_недоступен_запрещает'] = 'календарь ролла недоступен' in _r_нет
                out['третья_нога_отвергнута_разбором'] = (
                    'которой книга не знает' in _мнВ46)
            ...
            out['ok'] = all([out['плановый_остановлен'],
                             ...
                             out['серии_реестра_требуют'],
                             out['корни_одна_точка'],
                             out['корни_чужой_до_подмены'],
                             out['таблицы_по_корням_покрывают_канон'],
                             out['дробный_фьючерс_ловится'],
                             out['дробная_доля_фонда_законна'],
                             out['целый_фьючерс_проходит'],
                             out['книга_из_позиций_разбирает_ноги'],
                             out['явная_серия_не_перетирается'],
                             out['корень_ноги_а_без_множителя_отвергнут'],
                             out['корень_ноги_б_без_единицы_отвергнут'],
                             out['третья_нога_отвергнута_разбором'],
                             out['счёт_не_пинован_запрещает'],
                             ...
                             out['нога_б_жива_видит_фонд'],
                             out['нулевая_нога_б_не_жива'],
                             out['нога_а_не_считается_ногой_б'],
                             out['уходящая_серия_отвергнута'],
                             out['свежая_серия_проходит_календарь'],
                             out['календарь_недоступен_запрещает'],
                             out['обход_находит_копию'],
                             out['имена_none_отвергнуты'],
                             out['имена_строкой_отвергнуты'],
                             out['законный_кортеж_проходит_вход'],
                             out['итератор_отвергнут'],
                             out['множество_отвергнуто'],
                             out['контракт_зовётся_предполётом'],
                             out['предполёт_отвергает_none'],
                             out['цель_без_ноги_б_минует_запрет_ноги'],
                             out['цель_с_ногой_б_ловится_своим_сторожем'],
                             out['цель_без_ноги_б_минует_его_и_с_калиткой'],
                             out['горизонт_реестра_считается'],
            ...
            def _run(cushion, pos, pos_raises=False, cushion_raises=False):
                ...
                class _B:
                    ...
                    def net_positions(self):
                        ...
                        return pos
                _brm.IBBroker = _B
                # НОРМАТИВ БЕРЁТСЯ У НОРМАТИВА (разбор /code-review 21.08): стенд судил
                # боевой текст пробы по СВОЕЙ копии числа, поэтому изменение О-3-Е (и его
                # мутация) не сдвигало бы ни одну из семи проверяемых границ.
                import daily as _DLreal
                _dlm = _ty45.ModuleType('daily')
                _dlm.O3E_MIN = _DLreal.O3E_MIN
                _dlm.o3e_ok = _DLreal.o3e_ok
                _keep = {k: _sy.modules.get(k) for k in ('ib_insync', 'ib_broker', 'daily')}
                # ТЕЛО ПРОБЫ НАЧИНАЕТСЯ С sys.path.insert(0, '.') — восстанавливаем и ПУТЬ,
                # иначе каждый прогон стенда кладёт лишнюю '.' в начало пути импорта, и
                # текущий каталог оказывается первым для всех последующих стендов батареи
                # и всего мутационного прогона (разбор /code-review 21.08).
                _keep_path = list(_sy.path)
...
def _worm_case(kind):
    ...
    try:
        ...
        if kind == 'worm: расхождение пары заверяется, а не роняет замыкание':
            ...
            # Под tmp случая, а не отдельным mkdtemp: atexit-трекер уберёт вместе со
            # всем состоянием, и при kill -9 хвостов в /tmp не остаётся (угол эффективности).
            rdp = Path(tmp) / 'git-pair'
            rdp.mkdir()
            ...
            return out
        if kind == 'worm: брокер отвечает некорректно':
            # СЛОЙ 6, АУДИТ 26.08. Слой числился «закрыт», а измерение дало 25 односторонних
            # ветвей: молчали ОТКАЗНЫЕ плечи, и все за одним допущением фикстуры — СТАБ
            # ВСЕГДА ДАЁТ КОРРЕКТНЫЕ ДАННЫЕ СВОЕГО СЧЁТА. Здесь данные нарочно негодные.
            # gross() — денежный путь: по его плечу работает кап §1, и неизвестность обязана
            # быть отказом, а не тихой подстановкой (NaN проходит ОБА сравнения с капом).
            import ib_broker as _IB6
            class _Бр6(_IB6.IBBroker):
                def __init__(_s, поз, nlv):
                    _s._поз, _s._nlv = поз, nlv
                    _s._meta, _s._dref_cache, _s._since = {}, None, {}
                def net_positions(_s): return dict(_s._поз)
                def net_liquidation(_s): return _s._nlv
                def unit_ref(_s, name, cls, at_close=False): return None
            def _отказ6(поз, nlv):
                _м6 = ''
                try:
                    _Бр6(поз, nlv).gross()
                except BaseException as _e6:
                    # ОДНО ПЛЕЧО, СПРОСИТЬ ТИП (пятый случай этого класса за аудит):
                    # классификация исходов заранее держит недостижимые ветви — здесь все
                    # отказы приходят BrokerError, и «чужое» не берётся никогда.
                    _м6 = f'{type(_e6).__name__}: {_e6}'
                return _м6
            # unit_ref НА ЗАКРЫТИИ: ДВА ОТКАЗА, СТЕРЕГУЩИЕ МОДЕЛЬНУЮ ЕДИНИЦУ НОГИ А.
            # Единица ноги А — 500 x SPY и считается по ЗАКРЫТИЮ SPY. Недостоверный SPY или
            # разъехавшийся базис ES/10 к SPY означают, что плечо непроверяемо, — и это
            # отказ, а не тихая подстановка. Ни одна ветка не исполнялась: фикстура всегда
            # давала согласованную пару цен.
            # ПЕРВАЯ РЕДАКЦИЯ ЗОНДА НЕ ДОХОДИЛА ДО ПРОВЕРЯЕМОГО МЕСТА: unit_ref сперва
            # сверяет контракт с реестром, и заглушка сверку не проходила — четыре
            # утверждения были ложны. Обходится ИМЕННО сверка (contracts.mismatches), а не
            # проверяемая логика; иначе зонд доказывал бы работу собственной заглушки.
            import feed as _FD6, contracts as _CT6
            class _ИбК6:
                def qualifyContracts(_s, *a, **k): return list(a)
            class _БрК6(_IB6.IBBroker):
                def __init__(_s): _s.ib, _s._meta, _s._dref_cache = _ИбК6(), {}, None
                def _venue_prev(_s, *a, **k): return None
            def _единица6(spy, es=7750.0):
                _kc, _km = _FD6.closes, _CT6.mismatches
                _м = ''
                try:
                    _FD6.closes = lambda ib, c, d, expected_prev=None: (
                        (spy, d, None, None) if getattr(c, 'symbol', '') == 'SPY'
                        else (es, d, None, None))
                    _CT6.mismatches = lambda c, row: []
                    try:
                        _БрК6().unit_ref('ESU26', 'FUT', at_close=True)
                    except BaseException as _eк:
                        _м = f'{type(_eк).__name__}: {_eк}'
                finally:
                    _FD6.closes, _CT6.mismatches = _kc, _km
                return _м
            out['spy_недостоверен_отказ'] = 'закрытие SPY недостоверно' in _единица6(float('nan'))
            out['нулевой_spy_отказ'] = 'закрытие SPY недостоверно' in _единица6(0.0)
            # es_to_unit(7750) = 775; при SPY=500 базис +55% — далеко за предел.
            out['базис_разъехался_отказ'] = 'базис ES/10 к SPY' in _единица6(500.0)
            # ОБРАТНАЯ СТОРОНА: согласованная пара отказа не вызывает, иначе «нашёл беду»
            # было бы истинным всегда (угол «от отрицания»).
            out['согласованная_пара_проходит'] = (_единица6(775.0) == '')
            # ЧАСЫ ПАРЫ: ПУСК И НЕИЗМЕРИМОСТЬ. Тайм-аут 15 минут — денежная защита
            # (ожидание встречной ноги), и её молчаливое отключение опаснее лишнего отказа.
            # Ни первичная инициализация словаря, ни отказ «часы не пущены» не исполнялись.
            class _БрЧ6(_IB6.IBBroker):
                def __init__(_s): _s._since = None
            _брч = _БрЧ6()
            _неизм = ''
            try:
                _брч.minutes_since('ES/ZN')
            except BaseException as _eч:
                _неизм = f'{type(_eч).__name__}: {_eч}'
            out['часы_не_пущены_отказ'] = 'не пущены' in _неизм
            _брч.mark_pair('ES/ZN')                     # первичная инициализация словаря
            out['пуск_часов_измерим'] = (0.0 <= _брч.minutes_since('ES/ZN') < 1.0)
            _брч.mark_pair('MES/ZN')                    # словарь уже есть — вторая сторона
            out['вторая_пара_не_сбрасывает_первую'] = (
                _брч.minutes_since('ES/ZN') is not None
                and 0.0 <= _брч.minutes_since('MES/ZN') < 1.0)
            # ФИЛЬТРЫ todays_executions — МЕХАНИЗМ, КОТОРЫМ КОНТУР ОТЛИЧАЕТ СВОИ СДЕЛКИ
            # ОТ ПОСТОРОННИХ, и он не проверялся НИ РАЗУ: стаб отдавал только свои
            # исполнения. Правило проекта прямое — исполнение доказывается permId-отчётами,
            # значит цена ошибки здесь в том, что ЧУЖАЯ сделка будет засчитана как наша.
            class _Исп6:
                def __init__(_s, acct, ref, pid): _s.acctNumber, _s.orderRef, _s.permId = acct, ref, pid
            class _Фил6:
                def __init__(_s, ex): _s.execution = ex
            class _Иб6:
                def __init__(_s, сп): _s._сп = сп
                def fills(_s): return list(_s._сп)
            class _БрФ6(_IB6.IBBroker):
                def __init__(_s, сп, acct):
                    _s.ib, _s.account, _s._meta = _Иб6(сп), acct, {}
                def _exec_barrier(_s): return None
            _сп6 = [_Фил6(_Исп6('DU1', 'ADDFUT', 11)),      # своё — обязано попасть
                    _Фил6(None),                            # пустой отчёт — пропуск
                    _Фил6(_Исп6('DU2', 'ADDFUT', 22)),      # ЧУЖОЙ СЧЁТ — пропуск
                    _Фил6(_Исп6('DU1', 'ЧУЖОЙ', 33))]       # чужой orderRef — пропуск
            _взято6 = _БрФ6(_сп6, 'DU1').todays_executions()
            out['своё_исполнение_взято'] = (11 in _взято6)
            out['чужой_счёт_отсечён'] = (22 not in _взято6)
            out['чужой_orderref_отсечён'] = (33 not in _взято6)
            out['пустой_отчёт_не_роняет'] = (len(_взято6) == 1)
            out['nlv_недостоверен_отказ'] = 'NLV недостоверен' in _отказ6({'ZNU26': 1}, 0.0)
            out['nan_nlv_отказ'] = 'NLV недостоверен' in _отказ6({'ZNU26': 1}, float('nan'))
            out['посторонняя_позиция_отказ'] = 'посторонняя позиция' in _отказ6(
                {'НЕИЗВЕСТНЫЙ-X': 1}, 1e6)
            out['единица_неизвестна_отказ'] = 'долларовая единица неизвестна' in _отказ6(
                {'ZNU26': 1}, 1e6)
            # ОБРАТНАЯ СТОРОНА (угол «от отрицания»): нулевая позиция пропускается, а не
            # объявляется дефектом, — иначе «нашёл беду» было бы истинным всегда.
            out['нулевая_позиция_пропускается'] = _отказ6({'ZNU26': 0}, 1e6) == ''
            out['ok'] = all([out['spy_недостоверен_отказ'], out['нулевой_spy_отказ'],
                             out['базис_разъехался_отказ'],
                             out['согласованная_пара_проходит'],
                             out['часы_не_пущены_отказ'], out['пуск_часов_измерим'],
                             out['вторая_пара_не_сбрасывает_первую'],
                             out['своё_исполнение_взято'], out['чужой_счёт_отсечён'],
                             out['чужой_orderref_отсечён'], out['пустой_отчёт_не_роняет'],
                             out['nlv_недостоверен_отказ'], out['nan_nlv_отказ'],
                             out['посторонняя_позиция_отказ'],
                             out['единица_неизвестна_отказ'],
                             out['нулевая_позиция_пропускается']])
        if kind == 'worm: повреждённый архив опознаётся':
            # СЛОЙ 7, АУДИТ 26.08. Измерение показало: ВСЕ 78 непокрытых строк слоя сидят
            # за ОДНИМ допущением — батарея никогда не строит ПОВРЕЖДЁННЫЙ архив. Фикстуры
            # создают согласованные пары «архив-якорь», поэтому вся защита от рассогласования
            # молчала, а смысл WORM-якоря ровно в её работе: заверенное прошлое обязано быть
            # доказуемо восстановимо. Доказано было лишь то, что на исправной паре он не
            # ругается. Здесь два варианта порчи, не зависящие от живого состояния.
            import tarfile as _tar7, io as _io7
            _d7 = Path(tempfile.mkdtemp(prefix='addfut-worm7-'))
            import worm_anchor as _WA7
            # (1) АРХИВ НЕ ЧИТАЕТСЯ: подсунут не tar, а мусор.
            _мусор = _d7 / 'мусор.tgz'
            _мусор.write_bytes(b'\x00 not a tar at all \x00')
            _r1 = _WA7.attested_missing_from_archive(str(_мусор))
            out['нечитаемый_архив_назван'] = any('не читается' in _x for _x in _r1)
            # (2) В АРХИВЕ ССЫЛКА ВМЕСТО БАЙТОВ: после потери машины восстановить нечем.
            _ссыл = _d7 / 'ссылка.tgz'
            with _tar7.open(_ссыл, 'w:gz') as _tf7:
                _ti = _tar7.TarInfo('attested/registry.csv')
                _ti.type = _tar7.SYMTYPE
                _ti.linkname = '/куда-то/наружу.csv'
                _tf7.addfile(_ti)
            _r2 = _WA7.attested_missing_from_archive(str(_ссыл))
            out['ссылка_вместо_байтов_названа'] = any('ССЫЛКА' in _x for _x in _r2)
            # ОБРАТНАЯ СТОРОНА: исправный архив с теми же именами претензий не вызывает —
            # иначе «нашёл порчу» было бы истинным всегда (угол «от отрицания»).
            _цел = _d7 / 'целый.tgz'
            with _tar7.open(_цел, 'w:gz') as _tf8:
                _b = b'x,y\n1,2\n'
                _ti2 = _tar7.TarInfo('attested/registry.csv'); _ti2.size = len(_b)
                _tf8.addfile(_ti2, _io7.BytesIO(_b))
            _r3 = _WA7.attested_missing_from_archive(str(_цел))
            out['целый_архив_без_претензий_по_форме'] = not any(
                ('ССЫЛКА' in _x or 'не читается' in _x) for _x in _r3)
            out['ok'] = all([out['нечитаемый_архив_назван'],
                             out['ссылка_вместо_байтов_названа'],
                             out['целый_архив_без_претензий_по_форме']])
...
@rinv('выгрузка копий сообщает об отказе кодом возврата',
      needs=lambda r: r['case'] == 'автопилот: выгрузка копий сообщает об отказе кодом возврата')
def _l11push(r):
...
@rinv('горизонт календаря спрашивается про ОБА маршрута',
      needs=lambda r: r['case'] == 'автопилот: горизонт календаря спрашивается про ОБА маршрута')
def _l23cal(r):
    """Находка №23. Неактивный маршрут — это аварийный выход; вопрос только про активный
    делал проверку неспособной сработать по существу."""
    return not r['raised'] and r['ok'] is True
...
@rinv('отказ уборщика архивов не останавливает замыкание, но не молчит',
      needs=lambda r: r['case'] == 'автопилот: отказ уборщика архивов не выдаётся за отказ снимка')
def _l22rot(r):
    """Находка №22. Четыре точки: код ноль во всех состояниях, неудача уборки говорит, удача
    молчит. Без последней строку перестанут читать, и предупреждение обесценится."""
    return not r['raised'] and r['ok'] is True


@rinv('возраст замера маржи предупреждает заранее, а свежий молчит',
      needs=lambda r: r['case'] == 'автопилот: возраст замера маржи предупреждает ЗАРАНЕЕ')
def _l14mg(r):
    """Находка №14. Пять точек, и «свежий молчит» — разрешающая: предупреждение, звучащее
    каждый день, перестают читать, и заблаговременность пропадает вместе с ним."""
    return not r['raised'] and r['ok'] is True


@rinv('уборщик отметок разбирает ВСЕ дни месяца, но не чужие имена',
      needs=lambda r: r['case'] == 'автопилот: уборщик отметок разбирает ВСЕ дни месяца')
def _l12mark(r):
    """Находка №12. Дни 20-29 пропускались молча — треть входа. Чужое имя обязано
    по-прежнему отвергаться, иначе уборщик начнёт трогать не своё."""
    return not r['raised'] and r['ok'] is True


@rinv('сторож окружения различает СВОИ и ЧУЖИЕ ADDFUT_*',
      needs=lambda r: r['case'] == 'автопилот: сторож окружения различает свои и чужие ADDFUT_*')
def _l1envguard(r):
    """СЛОЙ 1 РЕЕСТРА 5б (аудит 26.08.2026). Шесть исходов, из них ТРИ разрешающих:
    без них стенд зелен и для сторожа, запрещающего всё, — а такой не пустил бы автопилот
    в торговлю ни разу. Шестой исход закрепляет ИЗМЕРЕННЫЙ факт: второй сторож недостижим
    в боевом пути и охраняет только подключаемый контекст."""
    return not r['raised'] and r['ok'] is True
...
@rinv('оценка плана заявок не оставляет временных каталогов',
      needs=lambda r: r['case'] == 'правила45: оценка плана заявок не оставляет временных каталогов')
def _l25tmp(r):
    """Находка №25. Утечка в БОЕВОМ пути: переполнение /tmp останавливает автопилот, а
    система чистит его лишь через 30 дней. Стенд считает следы, а не читает исходник."""
    return not r['raised'] and r['ok'] is True


@rinv('пустой route.txt даёт отказ, а не пустой маршрут',
      needs=lambda r: r['case'] == 'правила45: пустой route.txt — отсутствие маршрута, а не маршрут «»')
def _l20route(r):
    """Находка №20. Две разрешающие точки обязательны: функция, отвергающая всё, остановила
    бы контур на ровном месте — а отказ здесь запрещает и торговлю, и замыкание."""
    return not r['raised'] and r['ok'] is True


@rinv('журнал §7 атомарен и переживает утрату терминатора',
      needs=lambda r: r['case'] == 'правила45: журнал §7 пишется атомарно и переживает утрату терминатора')
def _l9j7(r):
    """Находки №9 и №10. Обычная запись обязана остаться обычной — иначе стенд зелен и для
    правки, сломавшей журнал ради атомарности."""
    return not r['raised'] and r['ok'] is True


@rinv('утраченный журнал §7 не начинается заново, целый проходит',
      needs=lambda r: r['case'] == 'правила45: утраченный журнал §7 не начинается заново')
def _l8j7(r):
    """Находка №8. Четыре точки, две запрещающие и две РАЗРЕШАЮЩИЕ: ворота, отвергающие
    всё, остановили бы новую установку и обычный день, то есть контур целиком."""
    return not r['raised'] and r['ok'] is True


@rinv('незамкнутая книга не запирает аварийный выход, но держит обычный',
      needs=lambda r: r['case'] == 'правила45: незамкнутая книга не запирает АВАРИЙНЫЙ выход')
def _l7prov(r):
    """Находка №7. Обе стороны обязательны: снять сторож целиком так же неверно, как
    запирать им аварию, — незамкнутая книга не несёт плеча закрытия, и в обычный день она
    уезжать с маршрутом не должна."""
    return not r['raised'] and r['ok'] is True


@rinv('реестр перехода узнаёт живые имена с сериями, но не чужое',
      needs=lambda r: r['case'] == 'правила45: реестр перехода узнаёт живые имена с сериями')
def _l6reg(r):
    """Находка №6 сплошного аудита. Четыре точки, и две из них РАЗРЕШАЮЩИЕ: без них стенд
    зелен и для реестра, принимающего что угодно, — а это снимает единственную защиту от
    чужого инструмента на счёте."""
    return not r['raised'] and r['ok'] is True
...
def _r45lock(r):
    ...
    return (not r['raised'] and r['ok'] is True and r.get('голый_и_явный_совпали') is True
            ...
            # №13: сообщение об отказе обязано называть ИСТИННЫЙ источник пути
            and r.get('источник_окружения_назван') is True
            and r.get('источник_аргумента_назван') is True)
...
def _r45dg(r):
    ...
    return (not r['raised'] and r['ok'] is True
            ...
            and r.get('код_сильнее_прозы') is True)
...
def _r45w(r):
    ...
    return (not r['raised'] and r['ok'] is True and r.get('согласованное_молчит') is True
            ...
            and r.get('замер_без_счёта_виден') is True
            and r.get('свой_счёт_молчит') is True)
...
def _r46reg(r):
    ...
    return (not r['raised'] and r['ok'] is True
            ...
            # СЕДЬМОЙ ПРОГОН, №4: судья обязан спрашивать и «одну точку корней» — иначе
            # замена текстового сторожа поведенческим осталась бы вне ворот выпуска.
            and r.get('корни_одна_точка') is True
            and r.get('корни_чужой_до_подмены') is True
            and not r.get('корни_копии')
            and r.get('таблицы_по_корням_покрывают_канон') is True
            # №5: правило целочисленности — оба конца, иначе парная мутация не наблюдается
            and r.get('дробный_фьючерс_ловится') is True
            and r.get('дробная_доля_фонда_законна') is True
            and r.get('книга_из_позиций_разбирает_ноги') is True
            # Правило 4: у контракта имён цели должен быть наблюдатель в батарее
            and r.get('имена_none_отвергнуты') is True
            and r.get('имена_строкой_отвергнуты') is True
            and r.get('обход_находит_копию') is True
            and r.get('предполёт_отвергает_none') is True)



@rinv('негодные данные брокера останавливают счёт плеча, годные — нет',
      needs=lambda r: r['case'] == 'worm: брокер отвечает некорректно')
def _r48gross(r):
    """Слой 6, аудит 26.08: слой числился «закрыт», а отказные плечи gross() не брались ни
    разу — все за допущением «стаб всегда даёт корректные данные своего счёта». Это
    денежный путь: по плечу gross работает кап §1, а NaN проходит ОБА сравнения с капом,
    то есть неизвестность обязана быть отказом, а не тихой подстановкой."""
    return (not r['raised'] and r['ok'] is True
            and r.get('nan_nlv_отказ') is True
            and r.get('посторонняя_позиция_отказ') is True
            and r.get('нулевая_позиция_пропускается') is True
            # Отсечение ЧУЖИХ исполнений — правило 7 проекта: исход доказывается
            # permId-отчётами, и засчитать чужую сделку как свою нельзя.
            and r.get('чужой_счёт_отсечён') is True
            and r.get('чужой_orderref_отсечён') is True
            and r.get('своё_исполнение_взято') is True
            # Тайм-аут пары — денежная защита: неизмеримость обязана быть отказом
            and r.get('часы_не_пущены_отказ') is True
            and r.get('пуск_часов_измерим') is True
            # Модельная единица ноги А: недостоверность обязана быть отказом
            and r.get('spy_недостоверен_отказ') is True
            and r.get('базис_разъехался_отказ') is True
            and r.get('согласованная_пара_проходит') is True)


@rinv('повреждённый архив опознаётся якорем, а исправный не оговаривается',
      needs=lambda r: r['case'] == 'worm: повреждённый архив опознаётся')
def _r48worm(r):
    """Слой 7, аудит 26.08: ВСЕ непокрытые строки слоя сидели за одним допущением —
    батарея не строит повреждённый архив. Смысл WORM-якоря именно в поимке рассогласования,
    а доказано было лишь, что на исправной паре он не ругается. Судья спрашивает обе
    стороны прямо: порча названа, исправное не оговорено."""
    return (not r['raised'] and r['ok'] is True
            and r.get('нечитаемый_архив_назван') is True
            and r.get('ссылка_вместо_байтов_названа') is True
            and r.get('целый_архив_без_претензий_по_форме') is True)
...
def _r47orch(r):
    ...
    return (not r['raised'] and r['ok'] is True
            ...
            # №1: невосстановленный каталог МР отравляет ВСЕ последующие случаи батареи —
            # судья обязан спрашивать об этом прямо, а не через общий ok.
            and r.get('каталог_мр_восстановлен') is True
            # Ворота 8в-1: тело запрета COMPLETE при дробном остатке и перенос причины
            # брокера — обе ветки до седьмого прогона батареей не исполнялись.
            and r.get('дробный_остаток_не_complete') is True
            and r.get('причина_брокера_доходит') is True
            and r.get('отказ_восстановления_виден') is True)
...
if BRANCH_COVER_OK:
    ...
    def _r46gate(r):
        ...
        return (not r['raised'] and r['ok'] is True
                ...
                # №8 и №9: оба плеча ленивости и УЗКИЙ перехват — судья спрашивает прямо,
                # иначе их удаление тонет в общем ok вместе с двумя десятками ключей.
                and r.get('узкий_перехват_не_опознан') is True
                and r.get('assert_с_сообщением_ленив') is True
                and r.get('цепочка_сравнений_ленива') is True
                # Решатели яруса — прямо, а не через общий ok: они решают сам факт отказа
                and r.get('ярус_боевой_без_загрузки') is True
                and r.get('однобокость_стенда_не_отказ') is True)
