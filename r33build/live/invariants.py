...
def _tr_run(case):
    ...
    try:
        if case == 'разрешение поднимается второй записью':
            ...
            keep05 = _M05._STATE['dir']; пин05 = _M05._PINNED['journal']
            ...
            keep04 = _M04._STATE['dir']
            _M04.set_state_dir_for_tests(str(td04 / 'install'))
            _пин04 = _M04._PINNED['journal']
            try:
                СВОЙ = Path(__file__).resolve().parent.parent
                for метка, nlv, ключ in (('ниже', 998_642.01, 'ниже'), ('выше', 5_000_000.0, 'выше')):
                    j04 = td04 / f'mr-{метка}.csv'
                    j04.write_text('asof,event,detail\n', encoding='utf-8')
                    _M04._PINNED['journal'] = None
                    _M04.test_configure(str(j04))
                    d04 = _M04.run(str(СВОЙ / 'mr_inputs_retro.csv'), str(СВОЙ / 'mr_tariff.csv'),
                                   str(td04 / f's-{метка}.csv'), str(j04),
                                   '2026-09-04', nlv, None, False, write=True)
                    события = [l.strip() for l in
                               j04.read_text(encoding='utf-8').splitlines()[1:] if l.strip()]
                    out[f'{ключ}_решение'] = d04
                    out[f'{ключ}_сигналов'] = sum(1 for s in события if ',SWITCH_SIGNAL,E|' in s)
                # ТРЕТЬЯ ПРОВЕРКА — СТОРОЖ ПОВТОРА: при уже ожидающем сигнале второй не
                # пишется, иначе бюджет переключений тратился бы каждым запуском.
                j04b = td04 / 'mr-повтор.csv'
                j04b.write_text('asof,event,detail\n2026-09-01,SWITCH_SIGNAL,E|s0\n',
                                encoding='utf-8')
                _M04._PINNED['journal'] = None
                _M04.test_configure(str(j04b))
                _M04.run(str(СВОЙ / 'mr_inputs_retro.csv'), str(СВОЙ / 'mr_tariff.csv'),
                         str(td04 / 's-повтор.csv'), str(j04b), '2026-09-04', 998_642.01,
                         None, False, write=True)
                out['повтор_сигналов'] = sum(
                    1 for s in j04b.read_text(encoding='utf-8').splitlines()[1:]
                    if ',SWITCH_SIGNAL,E|' in s)
            finally:
                _M04.set_state_dir_for_tests(keep04)
                _M04._PINNED['journal'] = _пин04
...
def _tr_grant_max(r):
    ...
    return (r['грант_после_первой'] == 387026.0 and r['грант_после_второй'] == 450000.0
...
def _tr_porog8(r):
    ...
    return (r['ниже_решение'] == _M.SWITCH_E and r['ниже_сигналов'] == 1
            and r['выше_сигналов'] == 0 and r['повтор_сигналов'] == 1)
