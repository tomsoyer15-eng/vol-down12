    ...
    try:
        if kind == 'правила45: допуск бара снимается известным календарём':
            ...
            try:
                ...
                _b2._preview_why = 'СТЕНД-ПРИЧИНА: шлюз не вернул начальную маржу'
                _reset47()
                _b2w = _Br(ok=False)
            ...
            try:
                ...
                _copies = [h for _mod46 in (_FD46, _ST46, _TR46)
                           for h in _scan46(_mod46.__file__)]
                ...
                out['корни_канон_восстановлен'] = _CT46.FUT_ROOTS == ('MES', 'ES', 'ZN')
                ...
                def _dst46(_d):
                    try:
                        ...
                        return 'контракт' if 'имена целей потеряны' in str(_e46t) else 'чужой'
                ...
                out['предполёт_отвергает_none'] = 'имена целей потеряны' in _pf_live46
                ...
                out['контракт_зовётся_предполётом'] = 'имена целей потеряны' not in _pf_none46
