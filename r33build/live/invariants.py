...
def _вне_пакета(путь, чем):
    """Стенду нужен файл, которого В ПАКЕТЕ НЕТ. Вернуть причину пропуска или None.

    ЗАЧЕМ (27.08.2026, разбор провалившегося выпуска). Два стенда этого дня читают файлы
    рабочего дерева: `tools/backup_push.sh` (в манифест не входит — это инструмент проекта,
    а не пакет) и `live/instruments_live.csv` (живой реестр, машинное состояние, в пакет не
    кладётся сознательно). В рабочем дереве оба на месте, и батарея зелёная; в распакованном
    архиве их нет, батарея падает FileNotFoundError — и выпуск не состоялся, хотя код исправен.
    ПОЧЕМУ ПРОПУСК, А НЕ ЗАГЛУШКА: подсунуть стенду фикстуру вместо живого файла значит
    доказывать вчерашний мир — ровно тот класс, из-за которого дефект №6 и дожил до аудита.
    ПОЧЕМУ ПРОПУСК ГРОМКИЙ: молчаливый skip неотличим от прохождения, а стенд, замолкший
    незаметно, хуже отсутствующего (правило 8в-2). Пропуск печатается и попадает в результат.
    ПОЧЕМУ ЭТО НЕ ЛАЗЕЙКА: в рабочем дереве, где идёт выпуск и где файлы есть, стенд
    работает в полную силу — пропуск возможен только там, где проверять физически нечего.
    """
    if Path(путь).exists():
        return None
    _п = f'вне рабочего дерева: {чем} ({путь}) — проверять нечего'
    print(f'[ПРОПУСК] {_п}')
    return _п
...
@ainv('неопределённость отмены Inactive называется, но не рвёт восстановление',
      needs=lambda b: b == 'reject')
def _a_inactive_cancel(beh):
    """КОНТРАКТ ПЕРЕПИСАН 28.08.2026 (рецензия, находка №2) — и это важнее, чем правка кода.

    ЧТО УТВЕРЖДАЛОСЬ ВЧЕРА: cancel_order при статусе Inactive обязан ПАДАТЬ, потому что
    'Cancelled' там пишет клиентская библиотека (ib_insync ib.py:699-706), а не шлюз, и
    заявку теоретически могут активировать на открытии площадки.
    ЧЕМ ЭТО ОБЕРНУЛОСЬ, ИЗМЕРЕНО: отказ попадает в список «неснятых» в _cancel_all,
    restore_to видит непустой stuck и уходит ДО подачи компенсирующих заявок. Inactive —
    обычное оформление ОТКАЗА у IBKR, значит после любого отказа книга оставалась
    разорванной: на ролле это нога А целиком, вне рынка до утра. Прежний код её
    восстанавливал. Защита от ненаблюдавшегося события создавала событие наблюдаемое.

    ЧТО УТВЕРЖДАЕТСЯ ТЕПЕРЬ — ДВЕ ВЕЩИ, И ВТОРАЯ НОВАЯ:
      (1) неопределённость НЕ теряется: в записи стоит признак `отмена_клиентская`, то есть
          знание сохранено и доступно вызывающему;
      (2) отмена НЕ рвёт восстановление: cancel_order возвращается без исключения, поэтому
          заявка не попадает в stuck и restore_to доходит до компенсирующих заявок.
    Второе — ровно то, чего вчерашний стенд не проверял и потому пропустил.

    ЗОНД ДОСТИЖИМОСТИ ОСТАЁТСЯ: заявка обязана лежать в openTrades со статусом Inactive,
    иначе проверяется запасная ветка «заявки нет», а не разбор Inactive.
    """
    import ib_broker as IBB
    br, ib, rows = _adapter(beh)
    try:
    ...
    try:
        зап = br.cancel_order(oid)
        рвётся = False
    except IBB.BrokerError:
        зап, рвётся = {}, True
    назвала = bool(зап.get('отмена_клиентская'))
    return (в_открытых and статус_до == 'Inactive' and not рвётся and назвала)
...
def _margins_fixture():
    ...
    # ДАТА — UTC, КАК У ЧИТАТЕЛЯ (02.09.2026, отвергнутый ночной выпуск). Ровно та правка,
    # которую 20.08 сделали в ib_stub.fixture_margins и не распространили на эту, вторую
    # фикстуру того же замера: `date.today()` берёт часы МАШИНЫ (+03), а _meta_age_ok
    # сравнивает с датой UTC — с полуночи до трёх ночи фикстура датируется БУДУЩИМ, возраст
    # выходит −1, замер объявляется устаревшим, и preview отвечает отказом на ОБЕ цели.
    # Стенд «разгрузка маржи подтверждается ИЗМЕРЕННОЙ маржой» краснел не по существу, а по
    # ЧАСУ ЗАПУСКА: батарея днём зелёная, выпуск в 00:06 отвергнут. Класс — «часы машины ≠
    # биржа» (§7 CLAUDE.md); проверено зондом: та же фикстура с датой UTC проходит.
    raw = {'_meta': {'date': str(_dt.datetime.now(_dt.timezone.utc).date()),
                     'account': 'DU000001',
...
""" % dict(tmp=tmp, sh=str(AUTOPILOT_SH))
    ...
    if kind == 'автопилот: выгрузка копий сообщает об отказе кодом возврата':
        ...
        out = dict(case=kind, raised=False, error='', ok=False)
        _проп = _вне_пакета(_sh11, 'скрипт выгрузки копий')
        if _проп:
            out['пропущен'] = _проп
            return out
        try:
...
def _rules45_case(kind):
    ...
    if kind == 'правила45: реестр перехода узнаёт живые имена с сериями':
        ...
        out = dict(case=kind, raised=False, error='', ok=False)
        _проп = _вне_пакета(ROOT / 'live' / 'instruments_live.csv', 'живой реестр con_id')
        if _проп:
            out['пропущен'] = _проп
            return out
        try:
...
def _l11push(r):
    ...
    # Вне рабочего дерева файла нет по построению — пропуск назван вслух в r.
    return bool(r.get('пропущен')) or (not r['raised'] and r['ok'] is True)
...
TR_CASES = ('план целых фьючерсов', 'план дробных долей фонда', 'дублированный источник',
            ...
            # 04.09.2026, ЖИВОЙ ОТКАЗ ПЕРЕВОДА ПИЛОТА: ниже порога §8 движок возвращал
            # REVIEW и НЕ выдавал сигнала, а исполнение требует одобрения КОНКРЕТНОГО
            # сигнала — маршрут Е был недостижим для любого счёта меньше 3 млн.
            'порог §8 выдаёт сигнал в Е',
            # 04.09.2026, ТОТ ЖЕ ЖИВОЙ ПЕРЕВОД: требование лимита плавает с ценой контракта,
            # а разрешение читалось ПЕРВОЕ — заниженная цифра запирала переход навсегда.
            'разрешение поднимается второй записью',
            # 15.09.2026, ЖИВОЙ ОБРЫВ: без сохранённых параметров возобновление
            # перехода недостижимо — tid привязан к плавающему капиталу.
            'параметры tid сохраняются и воспроизводят его')
...
def _tr_run(case):
    ...
    try:
        if case == 'параметры tid сохраняются и воспроизводят его':
            # Проверяется КОНТРАКТ функции, а не запись файла: из сохранённых параметров
            # обязан восстанавливаться ТОТ ЖЕ идентификатор. Сквозную проверку (реальный
            # прогон перехода -> файл прогресса -> tid) держит selfcheck; здесь — точка,
            # наблюдаемая парной мутацией.
            ноги_п = {'А': dict(src=[('ESZ26', 2, 384637.5), ('MESZ26', 6, 38463.75)],
                                dst=('CSPX', 820.63, 'ETF'))}
            кап_п = 980127.86
            пар = TRN.параметры_tid(кап_п, ноги_п)
            out['параметры'] = bool(пар) and 'capital' in пар and 'legs' in пар
            try:
                восст = {n: dict(src=[(i, u, uu) for i, u, uu in v['src']],
                                 dst=(v['dst'][0], v['dst'][1], v['dst'][2]))
                         for n, v in пар['legs'].items()}
                out['tid_исходный'] = TRN.transition_id('s1', 'F', 'E', кап_п,
                                                        TRN.plan_lots(ноги_п, кап_п))
                out['tid_восстановленный'] = TRN.transition_id(
                    's1', 'F', 'E', пар['capital'], TRN.plan_lots(восст, пар['capital']))
            except Exception as _ex_п:
                out['tid_исходный'] = 'ошибка'
                out['tid_восстановленный'] = repr(_ex_п)
        elif case == 'разрешение поднимается второй записью':
            import mr_engine as _M05
            import tempfile as _tf05
            td05 = Path(_tf05.mkdtemp(prefix='addfut-grant-'))
            # СНИМОК ПИНА — ЦЕЛИКОМ И ДО set_state_dir_for_tests (рецензия 04.09.2026):
            # он сам обнуляет _PINNED['journal'], а ключей там четыре — восстановление
            # одного оставляло бы ident/epoch/uuid от временного журнала, и следующий же
            # разбор живого журнала объявил бы подмену файла.
            keep05 = _M05._STATE['dir']; пин05 = dict(_M05._PINNED)
            _M05.set_state_dir_for_tests(str(td05 / 'install'))
            try:
                j05 = td05 / 'mr.csv'
                j05.write_text('asof,event,detail\n', encoding='utf-8')
                _M05._PINNED['journal'] = None
                _M05.test_configure(str(j05))
                _M05.append_event(str(j05), '2026-09-04', 'GRANULARITY_EXCEPTION', 's1|387026')
                out['грант_после_первой'] = _M05.find_grant(str(j05), '2026-09-04', 's1')
                _M05.append_event(str(j05), '2026-09-04', 'GRANULARITY_EXCEPTION', 's1|450000')
                out['грант_после_второй'] = _M05.find_grant(str(j05), '2026-09-04', 's1')
                # ЧУЖОЙ СИГНАЛ НЕ ПОДХВАТЫВАЕТСЯ: иначе «наибольшее» превратилось бы в
                # «наибольшее по всему журналу» и разрешение одного перехода расширяло бы
                # соседний.
                out['грант_чужого_sid'] = _M05.find_grant(str(j05), '2026-09-04', 's2')
                # ПОРЧА ЛОВИТСЯ И ВО ВТОРОЙ СТРОКЕ — прежний код до неё не доходил, потому
                # что возвращался на первой. Пишем ЗАКОННЫМ путём (append_event обновляет
                # digest), иначе отказ пришёл бы от сверки отпечатка, а не от разбора
                # лимита, и стенд мерил бы не то место.
                # МЕНЬШЕЕ ПОСЛЕ БОЛЬШЕГО НЕ ОПУСКАЕТ ПОТОЛОК — иначе «наибольшее»
                # неотличимо от «последнего», и мутация last-wins проходила бы незамеченной
                # (рецензия 04.09: именно так она и выжила).
                _M05.append_event(str(j05), '2026-09-04', 'GRANULARITY_EXCEPTION', 's1|200000')
                out['грант_после_меньшего'] = _M05.find_grant(str(j05), '2026-09-04', 's1')
                # ПОРЧА ЛОВИТСЯ И ВО ВТОРОЙ СТРОКЕ — прежний код до неё не доходил, потому
                # что возвращался на первой. Путь ЗАПИСИ теперь негодное значение не
                # принимает (рецензия 04.09), поэтому проверяем оба конца: запись отвергает,
                # а чтение уже лежащей строки называет причиной ЛИМИТ, а не отпечаток.
                try:
                    _M05.grant_granularity(str(j05), str(td05 / 's.csv'), '2026-09-04',
                                           's1', float('nan'))
                    out['запись_негодного'] = 'ПРОПУЩЕНА'
                except ValueError:
                    out['запись_негодного'] = 'отвергнута'
                _M05.append_event(str(j05), '2026-09-04', 'GRANULARITY_EXCEPTION', 's1|nan')
                try:
                    _M05.find_grant(str(j05), '2026-09-04', 's1')
                    out['порча_поймана'] = 'НЕТ'
                except _M05.JournalCorrupt as _ex05:
                    out['порча_поймана'] = 'лимит' in str(_ex05)
                # НЕЧИСЛО — ТОЖЕ ПОРЧА, А НЕ ГОЛЫЙ ValueError: transition ловит только
                # JournalCorrupt, и описка «450 000» уходила наружу трейсбеком.
                j05b = td05 / 'mr-nechislo.csv'
                j05b.write_text('asof,event,detail\n', encoding='utf-8')
                _M05._PINNED['journal'] = None
                _M05.test_configure(str(j05b))
                _M05.append_event(str(j05b), '2026-09-04', 'GRANULARITY_EXCEPTION', 's1|450 000')
                try:
                    _M05.find_grant(str(j05b), '2026-09-04', 's1')
                    out['нечисло'] = 'ПРОПУЩЕНО'
                except _M05.JournalCorrupt:
                    out['нечисло'] = 'JournalCorrupt'
                except ValueError:
                    out['нечисло'] = 'голый ValueError'
            finally:
                _M05.set_state_dir_for_tests(keep05)
                _M05._PINNED.update(пин05)
        elif case == 'порог §8 выдаёт сигнал в Е':
            # ОБЕ СТОРОНЫ ОДНОЙ РАЗВИЛКИ В ОДНОМ СЛУЧАЕ — иначе мутация «выдавать сигнал
            # ВСЕГДА» осталась бы ненаблюдаемой: её ловит именно верхняя половина (выше
            # порога сигнала быть не должно), а «не выдавать никогда» — нижняя.
            import mr_engine as _M04
            import tempfile as _tf04
            td04 = Path(_tf04.mkdtemp(prefix='addfut-mr8-'))
            keep04 = _M04._STATE['dir']; _пин04 = dict(_M04._PINNED)   # СНИМОК ДО обнуления
            _M04.set_state_dir_for_tests(str(td04 / 'install'))
            try:
                СВОЙ = Path(__file__).resolve().parent.parent
                # ВХОДЫ — СВОЯ ФИКСТУРА, А НЕ ЖИВОЙ mr_inputs_retro.csv (рецензия
                # 04.09.2026). Половина «выше порога» была зелена лишь потому, что в живом
                # файле оба квартала не подтверждены: заполнят их — и стенд покраснеет на
                # ПРАВИЛЬНОМ поведении движка. Здесь два подтверждённых смежных квартала с
                # разрывом внутри мёртвой зоны: квартальный сигнал не возникает НИКОГДА, и
                # любой записанный сигнал в Е приходит только от ветви порога §8.
                вх04 = td04 / 'inputs.csv'
                вх04.write_text(
                    'quarter_id,window_start,window_end,s_eq_pp,quote_base,basis_to_effr_pp,'
                    's_zn_pp,source_url,fix_date,status\n'
                    'Q1-2026,2026-01-01,2026-04-01,0.90,SOFR3M,0.00,0.00,'
                    'https://example.invalid/q1,2026-04-02,OK\n'
                    'Q2-2026,2026-04-01,2026-07-01,0.90,SOFR3M,0.00,0.00,'
                    'https://example.invalid/q2,2026-07-02,OK\n', encoding='utf-8')

                def прогон04(имя, nlv, засеять=''):
                    j = td04 / f'mr-{имя}.csv'
                    j.write_text('asof,event,detail\n' + засеять, encoding='utf-8')
                    _M04._PINNED['journal'] = None
                    _M04.test_configure(str(j))
                    d = _M04.run(str(вх04), str(СВОЙ / 'mr_tariff.csv'),
                                 str(td04 / f's-{имя}.csv'), str(j),
                                 '2026-09-04', nlv, None, False, write=True)
                    строки = [s.strip() for s in
                              j.read_text(encoding='utf-8').splitlines()[1:] if s.strip()]
                    # СЧИТАЮТСЯ СИГНАЛЫ ЭТОГО ПРОГОНА, А НЕ ВСЕ В ФАЙЛЕ: посеянная история
                    # (завершённый переход, встречный сигнал) сама содержит SWITCH_SIGNAL,
                    # и счёт «всего в файле» показывал бы её, а не поведение движка —
                    # поймано первым же запуском переписанного стенда.
                    новые = sum(1 for s in строки
                                if s.startswith('2026-09-04,') and ',SWITCH_SIGNAL,E|' in s)
                    return d, новые, len(строки)

                out['ниже_решение'], out['ниже_сигналов'], _ = прогон04('ниже', 998_642.01)
                out['выше_решение'], out['выше_сигналов'], _ = прогон04('выше', 5_000_000.0)
                # СТОРОЖ ПОВТОРА: при уже ожидающем сигнале второй не пишется.
                _, out['повтор_сигналов'], _ = прогон04(
                    'повтор', 998_642.01, '2026-09-01,SWITCH_SIGNAL,E|s0\n')
                # ВЕТВЬ «УЖЕ НА МАРШРУТЕ Е» — она и есть та половина предиката route == 'F',
                # которую не убивала ни одна мутация (рецензия 04.09): без неё движок гнал
                # бы счёт из Е в Е. Журнал с завершённым переходом в Е, капитал ниже порога.
                _на_Е = ('2026-01-05,SWITCH_SIGNAL,E|s0\n2026-01-05,OWNER_APPROVE,E|s0\n'
                         '2026-01-05,TRANSITION_OPEN,E|s0|t0\n'
                         '2026-01-05,TRANSITION_COMPLETE,E|s0|t0\n')
                out['на_Е_решение'], out['на_Е_сигналов'], _ = прогон04('на-Е', 998_642.01, _на_Е)
                # И ОБЯЗАТЕЛЬНО СУХОЙ ПРОГОН ТОГО ЖЕ СОСТОЯНИЯ. На пути ЗАПИСИ выпадение
                # проверки маршрута поглощается давней перепроверкой под замком («цель уже
                # действует»), то есть наблюдаемого следа не оставляет — измерено. А при
                # write=False перепроверки нет, и движок доложил бы оператору SWITCH_E на
                # счёте, который УЖЕ в Е. Именно этим плечом и убивается мутация, иначе
                # половина предиката остаётся ненаблюдаемой (рецензия 04.09).
                j04s = td04 / 'mr-на-Е-сухой.csv'
                j04s.write_text('asof,event,detail\n' + _на_Е, encoding='utf-8')
                _M04._PINNED['journal'] = None
                _M04.test_configure(str(j04s))
                out['на_Е_решение_сухой'] = _M04.run(
                    str(вх04), str(СВОЙ / 'mr_tariff.csv'), str(td04 / 's-на-Е-сухой.csv'),
                    str(j04s), '2026-09-04', 998_642.01, None, False, write=False)
                # ДВА СТОРОЖА ВЕТВИ, КОТОРЫЕ НЕ ИСПОЛНЯЛИСЬ НИ РАЗУ (ворота 8в-1): встречный
                # pending и исчерпанный бюджет. Оба обязаны давать REVIEW и НИЧЕГО не
                # писать — прежняя редакция дописывала REVIEW_SIGNAL каждым прогоном.
                _пара = '2026-01-05,SWITCH_SIGNAL,E|a\n2026-01-05,OWNER_APPROVE,E|a\n' \
                        '2026-01-05,TRANSITION_OPEN,E|a|t1\n2026-01-05,TRANSITION_COMPLETE,E|a|t1\n' \
                        '2026-02-05,SWITCH_SIGNAL,F|b\n2026-02-05,OWNER_APPROVE,F|b\n' \
                        '2026-02-05,TRANSITION_OPEN,F|b|t2\n2026-02-05,TRANSITION_COMPLETE,F|b|t2\n'
                out['бюджет_решение'], _, out['бюджет_строк'] = прогон04(
                    'бюджет', 998_642.01, _пара)
                out['бюджет_строк_ожидалось'] = len(
                    [s for s in _пара.splitlines() if s.strip()])
                out['встречный_решение'], _, out['встречный_строк'] = прогон04(
                    'встречный', 998_642.01, '2026-09-01,SWITCH_SIGNAL,F|s0\n')
            finally:
                _M04.set_state_dir_for_tests(keep04)
                _M04._PINNED.update(_пин04)
        elif case == 'журнал МР правлен на месте':
...
@tinv('параметры tid сохраняются в прогрессе и воспроизводят идентификатор',
      needs=lambda r: r['case'] == 'параметры tid сохраняются и воспроизводят его')
def _tr_params_tid(r):
    """15.09.2026, ЖИВОЙ ОБРЫВ ПЕРЕХОДА Ф->Е. transition_id считается от связки
    «сигнал+маршруты+КАПИТАЛ+план», капитал меняется непрерывно, и после обрыва следующий
    запуск получает ДРУГОЙ идентификатор: ни продолжить переход, ни закрыть его событием —
    оба требуют совпадения. Переход оставался открытым навсегда, с возможной непарной
    позицией на счёте; капитал в тот день искали ПЕРЕБОРОМ по хэшу 105 секунд.
    Проверяется то, что делает механизм механизмом: параметры непусты и восстановленный из
    них идентификатор совпадает с исходным."""
    return (not r['raised'] and r['параметры'] is True
            and r['tid_восстановленный'] == r['tid_исходный']
            and isinstance(r['tid_исходный'], str) and len(r['tid_исходный']) == 16)


@tinv('разрешение owner-cap поднимается второй записью, чужой sid не подхватывается',
      needs=lambda r: r['case'] == 'разрешение поднимается второй записью')
def _tr_grant_max(r):
    """04.09.2026, ЖИВОЙ ПЕРЕВОД. Требуемый лимит непарной дельты привязан к ЦЕНЕ контракта
    и плавает каждый день, а разрешение пишется заранее: ES вырос на 0,9% за сутки,
    требование стало 390 393 против записанных 387 026 — переход отказал. Поднять потолок
    было нечем: читалось ПЕРВОЕ разрешение, новый сигнал движок не выдаёт при ожидающем, а
    ожидающий снимается только исполненным или прерванным переходом. Одна заниженная цифра
    запирала переход навсегда.

    Проверяется вся тройка: первая запись действует; вторая, большая, поднимает потолок;
    чужой сигнал разрешения не получает (иначе «наибольшее» расширяло бы соседний переход);
    и порча ловится ВО ВТОРОЙ строке, до которой прежний код не доходил."""
    return (not r['raised']
            and r['грант_после_первой'] == 387026.0 and r['грант_после_второй'] == 450000.0
            # меньшее после большего потолок НЕ опускает — иначе «наибольшее» неотличимо
            # от «последнего», и мутация last-wins проходит незамеченной
            and r['грант_после_меньшего'] == 450000.0
            and r['грант_чужого_sid'] is None and r['порча_поймана'] is True
            and r['запись_негодного'] == 'отвергнута' and r['нечисло'] == 'JournalCorrupt')


@tinv('ниже порога §8 движок выдаёт сигнал в Е, выше — не выдаёт, повтор не плодит',
      needs=lambda r: r['case'] == 'порог §8 выдаёт сигнал в Е')
def _tr_porog8(r):
    """04.09.2026, ЖИВОЙ ОТКАЗ. Перевод пилота Ф->Е не состоялся: исполнение требует
    OWNER_APPROVE на КОНКРЕТНЫЙ сигнал, одобрять можно только ОЖИДАЮЩИЙ, а в состоянии
    «маршрут Ф, NLV ниже порога §8» движок возвращал REVIEW ДО генерации сигнала —
    квартальная ветвь стоит ниже возврата, аварийная ещё ниже и заперта условием HOLD.
    Начальный маршрут derive_state — 'F', меняет его только завершённый переход, поэтому
    маршрут Е был недостижим для ЛЮБОГО счёта ниже 3 млн: и для пилота, и для живого
    старта (~100 тыс.), ради которого §8 его и предписывает.

    Проверяется ВСЯ развилка, а не сработавшее плечо: ниже порога — ровно один сигнал в Е;
    выше порога — ни одного (иначе «выдавать всегда» прошло бы незамеченным); при уже
    ожидающем сигнале — по-прежнему один (бюджет переключений не тратится повторно)."""
    import mr_engine as _M
    return (not r['raised']
            and r['ниже_решение'] == _M.SWITCH_E and r['ниже_сигналов'] == 1
            # выше порога — ни решения в Е, ни записи: входы фикстуры лежат в мёртвой зоне,
            # поэтому единственным источником сигнала могла быть только ветвь порога
            and r['выше_решение'] != _M.SWITCH_E and r['выше_сигналов'] == 0
            and r['повтор_сигналов'] == 0          # повторного сигнала этот прогон не пишет
            # уже на маршруте Е — ветвь не срабатывает (половина предиката route == 'F')
            and r['на_Е_решение'] != _M.SWITCH_E and r['на_Е_сигналов'] == 0
            # сухой прогон на маршруте Е — то самое плечо, где выпадение проверки
            # маршрута видно: перепроверки под замком там нет
            and r['на_Е_решение_сухой'] != _M.SWITCH_E
            # оба сторожа дают REVIEW и НЕ пишут в журнал ни строки
            and r['бюджет_решение'] == _M.REVIEW
            and r['бюджет_строк'] == r['бюджет_строк_ожидалось']
            and r['встречный_решение'] == _M.REVIEW and r['встречный_строк'] == 1)
...
SIG_CASES = ('дописывается новый месяц', 'источник разошёлся с историей',
             ...
             'выплата между дозаписями месяцев')
...
def _sig_run(case):
    ...
    try:
        ...
        if case == 'порча промежуточного месяца после пропуска':
            ...
            ib.set_bars(bars)
        if case == 'выплата между дозаписями месяцев':
            # ЖИВОЙ ОТКАЗ КОНТУРА 01.09.2026. Первый прогон пишет сайдкар целиком
            # (поколение A). Затем выплата двигает ВСЮ историю на общий множитель — для
            # ADJUSTED_LAST это штатное поведение, — и следующий месяц дозаписывается уже
            # в поколении B. Прежний код замораживал его буквальным значением, сайдкар
            # становился двухпоколенным, и ТРЕТИЙ прогон падал по УРОВНЯМ: месячное
            # обновление сигнала не проходило больше никогда, торговли не было.
            SU.update(ib)
            os.environ.pop('ADDFUT_LEVELS_BOOTSTRAP', None)
            купон = 0.9964                    # один купон IEF — 0,36%, втрое больше допуска
            for k in bars:
                bars[k] = [(d, px * купон) for d, px in bars[k]]
            bars[spy_id].append(('2026-08-31', bars[spy_id][-1][1] * 1.01))
            bars[ief_id].append(('2026-08-31', bars[ief_id][-1][1]))
            ib.set_bars(bars)
            FD.exchange_today = lambda: pd.Timestamp('2026-09-13')
            SU.update(ib)                     # дозапись августа — в поколении B
...
@sginv('дозапись месяца после выплаты не ломает следующую сверку уровней',
       needs=lambda r: r['case'] == 'выплата между дозаписями месяцев')
def _sg_pokolenie(r):
    """01.09.2026, живой отказ: месяц, замороженный ПОСЛЕ выплаты, оставался в чужом
    поколении, и следующая сверка объявляла расхождение размером с купон (август +0,361%
    при допуске 0,1%). Для ежемесячного плательщика — отказ каждого месяца после первой
    дозаписи: сигнал не обновляется, автопилот не торгует."""
