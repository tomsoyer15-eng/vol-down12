    ...
    try:
        if kind == 'правила45: допуск бара снимается известным календарём':
            ...
            try:
                ...
                out['каталог_мр_восстановлен'] = True
                try:
                    ...
                    out['каталог_мр_восстановлен'] = False
            ...
            try:
                ...
                _copies = []
                for _mod46 in (_FD46, _ST46, _TR46):
                    _f46 = _mod46.__file__
                    for _nd46 in _ast46.walk(_ast46.parse(Path(_f46).read_text('utf-8'))):
                        if (isinstance(_nd46, _ast46.Tuple)
                            ...
                            _copies.append(f'{Path(_f46).name}:{_nd46.lineno}')
                ...
                # ОБРАТНАЯ СТОРОНА ТОЙ ЖЕ ПРОБЫ: до подмены YMU26 обязан быть чужим, иначе
                # _alien46()==False ничего не доказывает (угол «от отрицания»).
                _alien_before46 = None
                try:
                    _ST46.book_from_broker(_DL46.Book, {'YMU26': 1.0}, 'F')
                    _alien_before46 = False
                except ValueError as _e46b:
                    _alien_before46 = 'посторонние' in str(_e46b)
