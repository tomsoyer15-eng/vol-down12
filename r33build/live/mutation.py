...
def _run_mutations():
    ...
    def calendar_horizon_active_route_only():
        ...
        обрыв таблицы Е обнаружится в 2027-м, когда аварийный переход уже нужен."""
        return _mutate_autopilot(
            "for _r in ('F', 'E'):",
            "for _r in ('F',):",
            'addfut-mut-cal-', 'горизонт только активного маршрута')

    def lost_route_defaults_to_f():
        """Потерянный маршрут снова молча означает Ф — как было до разбора находки №21:
        traded_today читает СТАРУЮ книгу Ф, когда живой маршрут Е, и день объявляется
        отторгованным. Дальше close_after отдаёт ворота Ф вместо Е, а замыкание уходит с
        маршрутом NONE.

        Мутируется ПЕРВАЯ из пяти вставок — та, что стоит в traded_today: этого достаточно,
        чтобы решение принималось по чужой книге. Якорь включает строку комментария, потому
        что сама строка `route = ST.active_route()` встречается пять раз, а помощник
        мутации требует РОВНО одного места — и правильно требует."""
        return _mutate_autopilot(
            ...
            'addfut-mut-route-', 'потерянный маршрут молча становится Ф')
    ...
    return [('пин торгового счёта не требуется', pin_not_required),
            ...
            # МУТАЦИЯ СНИМАЕТСЯ ВНЕ РАБОЧЕГО ДЕРЕВА (27.08.2026, второй провал выпуска).
            # Она правит tools/backup_push.sh, которого в пакете НЕТ: инструмент проекта в
            # манифест не входит. Стенд в invariants.py я на прошлом заходе научил громко
            # пропускаться, а ЭТУ, парную ему мутацию — нет, и mutation.py падал
            # FileNotFoundError; selfcheck архива показывал только «мутаций без убийцы нет»,
            # потому что stderr он не печатает. Ровно тот класс, против которого заведено
...
def _signal_mutations():
    ...
    def pokolenie_off():
        """Новый месяц замораживается БУКВАЛЬНЫМ значением поставщика (как было до
        01.09.2026): после выплаты сайдкар становится двухпоколенным, и следующая сверка
        уровней объявляет расхождение размером с купон — сигнал не обновляется никогда."""
        orig = SU._поколение_сайдкара
        return orig, (lambda rows, sym, me: 1.0), '_поколение_сайдкара'
    ...
    return [('хвост только у последнего месяца', tail_last_only),
            ...
            ('уровень месяца замораживается в чужом поколении', pokolenie_off),
