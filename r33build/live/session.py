...
def _connect(client_id):
    ...
    ib = IB()
    # ОДИН ПОВТОР ПРИ ТАЙМАУТЕ ПРОГРЕВА (31.08.2026, карточка ALARM-trade-2026-08-31,
    # «чини» заказчика). Наблюдено живьём: первый тик после простоя поднимает шлюз, порт
    # отвечает раньше, чем шлюз готов отдать executions, — connect падает TimeoutError, и
    # день стоит до ручного разбора. Пятница 28.08 прошла тот же путь успешно — исход
    # решали секунды прогрева. Повторяется ИМЕННО ПОДКЛЮЧЕНИЕ и ровно один раз: заявок в
    # этот момент ещё не существует, правило 7 (не повторять заявку при неизвестном
    # исходе) не задевается. Второй таймаут — прежняя дорога: тревога и ручной разбор.
    try:
        ib.connect(HOST, PORT, clientId=client_id, timeout=30)
    except TimeoutError:
        print('шлюз не прогрелся за 30 с — единственный повтор подключения через 30 с')
        time.sleep(30)
        ib.connect(HOST, PORT, clientId=client_id, timeout=30)
...
def do_close(ib, route):
    ...
    with ST.hold_book_lock(ST.book_lock_dir(bp)):
        ...
        # НЕЗАВЕРШЁННОЕ НАМЕРЕНИЕ — БАРЬЕР И ДЛЯ ЗАМЫКАНИЯ (тридцать седьмой круг, №9).
        # Замыкатель проверял метку передачи, но НЕ intent. После аварии среза О-3-Е на диске
        # остаётся промежуточная сегодняшняя книга и намерение с НЕИЗВЕСТНЫМ исходом: снимок
        # позиций может в этот момент совпасть с ней (заявка ещё не разнеслась), и замыкание
        # спокойно фиксировало плечо, снимало backup и WORM-якорь, разрешая поставить
        # closed-*. Поздний fill менял книгу у брокера уже ПОСЛЕ заверения ложного состояния.
        # Основная сессия разбирает intent до сверки — замыкание эту защиту обходило.
        _it = ST.load_intent(bp)
        if _it:
            raise Refused(
                f'на диске незавершённое намерение ({_it}) — исход заявок неизвестен, '
                f'замыкание заверило бы книгу, которая может измениться поздним отчётом; '
                f'сначала разбор намерения торговой сессией или вручную (О-5)')
...
def _alarm_o3e(day, reason, src, done=True):
    ...
    ТЕКСТ ОБЯЗАН ГОВОРИТЬ ПРАВДУ О КНИГЕ (тридцать седьмой круг, №1). Прежде сообщение
    БЕЗУСЛОВНО утверждало «Сокращение выполнено автоматикой», хотя один из трёх вызовов
    стоит ДО расчёта и подачи заявок среза: при неизвестном запасе срез не выполняется
    вообще, а при низком — тревога fsync-ится раньше первой заявки. Обрыв питания в этом
    окне оставлял у брокера книгу около 2× и долговечный файл, сообщающий оператору, что
    она уже сокращена. Оператор в этот момент читает не подсказку, а единственное
    свидетельство: ложь здесь дороже молчания. `done` разделяет два разных события —
    «срез СОСТОЯЛСЯ» и «запас провален, состояние книги НЕ подтверждено».
    """
    _af = state_dir() / f'ALARM-o3e-{day}.txt'
    _what = ('Сокращение выполнено автоматикой по нормативу §8, '
             if done else
             'СОКРАЩЕНИЕ НЕ ПОДТВЕРЖДЕНО: заявки среза на момент записи тревоги не '
             'поданы — книга у брокера может оставаться НЕсокращённой, проверить её '
             'ПЕРВЫМ ДЕЙСТВИЕМ; ')
    # КОД ПРИЧИНЫ — ОТ ПРОИЗВОДИТЕЛЯ (ворота 3, правило 8в). Диагносту нечего угадывать по
    # прозе: здесь известно ТОЧНО, о чём тревога.
    import diagnose as _DGm
    _code = ('O3E-НЕИЗВЕСТЕН' if 'НЕИЗВЕСТЕН' in str(reason) else 'O3E-ПРОБИТ')
    _msg = (f'{_DGm.CAUSE_MARK} {_code}\n'
            f'{reason}. Вход: {src}. {_what}'
            f'причина провала запаса НЕ разобрана: контур под этой тревогой не торгует '
    ...
    try:
        ...
        # STOP — ПОСЛЕДНИЙ БАРЬЕР, И ОН ОБЯЗАН БЫТЬ ДОКАЗАН (тридцать восьмой круг, №11).
        # Здесь стоял голый write_text: без fsync файла и каталога, без проверки, что файл
        # появился, и с проглоченной ошибкой — а наружу ВСЕГДА уходило утверждение
        # «поставлен STOP». При ENOSPC или ACL это прямая ложь в единственном сообщении,
        # которое увидит оператор: следующий cron снова вошёл бы в торговый путь при
        # незавершённом намерении или уже исполненном срезе. Тот же класс, что и №1 этого
        # круга: сообщение утверждает состояние, которого не проверяло.
        _stop_ok = False
        try:
            _sp = state_dir() / 'autopilot.STOP'
            with open(_sp, 'w', encoding='utf-8') as _sh:
                _sh.write(f'не удалось записать ALARM-o3e-{day}: {_exa}\n')
                _sh.flush(); os.fsync(_sh.fileno())
            _sfd = os.open(str(_sp.parent), os.O_DIRECTORY)
            try:
                os.fsync(_sfd)
            finally:
                os.close(_sfd)
            ...
            _stop_ok = False
        # ТЕКСТ ОТКАЗА ТОЖЕ ОБЯЗАН ЗНАТЬ ПРО done (тридцать девятый круг, №11). Хвост
        # «заявки среза уже исполнены» стоял БЕЗУСЛОВНО — а при done=False срез не подан
        # вовсе (вызов идёт до расчёта, либо запас неизвестен), и само это исключение
        # прекращает выполнение ДО заявок. Оператор получал сообщение, прямо противоположное
        # факту, и мог не выполнить ручное сокращение, считая книгу уже сокращённой. В 38-м
        # круге я исправил здесь доказуемость STOP и не заметил, что рядом осталась та же
        # ложь про состояние книги, которую этот же круг чинил в основном сообщении.
        raise Refused(
            f'тревога О-3-Е НЕ записана ({_exa}) — '
            + ('поставлен STOP; ' if _stop_ok else
               'И STOP ПОСТАВИТЬ НЕ УДАЛОСЬ: контур НИЧЕМ не остановлен, остановить '
               'автопилот вручную НЕМЕДЛЕННО; ')
            + ('заявки среза уже исполнены' if done else
               'СОКРАЩЕНИЕ НЕ ПОДТВЕРЖДЕНО: заявки среза не подавались, книга у брокера '
               'может оставаться НЕсокращённой — проверить её ПЕРВЫМ ДЕЙСТВИЕМ')
            + ', немедленный ручной разбор (О-5)')
...
def do_o3e_cut(ib, route):
    ...
    with ST.hold_book_lock(ST.book_lock_dir(bp)):
        ...
        for _inst, _qty in _orders:
            ...
            try:
                # ЗАПАС ВРЕМЕНИ НА ПАРУ ПЕРЕД ПЕРВОЙ НОГОЙ (тридцать пятый круг, №2)
                DL._window_gate(_deadline, what=f'О-3-Е (вахта) {_inst} {_qty:+g}',
                                margin_min=not _placed)
                ...
                if not _placed:
                    ...
                    ST.clear_intent(bp)
                    # «КНИГА НЕ ТРОНУТА» — БОЛЬШЕ, ЧЕМ ДОКАЗАНО (сороковой круг, №5).
                    # Доказано ровно то, что ЭТОТ вызов заявок не подавал; поздний отчёт по
                    # прежним операциям или ручная сделка после последнего снимка не
                    # исключены. Утверждать состояние счёта, которого не мерили в этот
                    # момент, — тот же класс, что тревога «сокращено» до среза.
                    raise Refused(f'О-3-Е (вахта): {_inst} {_qty:+g} не подано ({ex}) — '
                                  f'ворота окна отказали ДО подачи, этим вызовом заявок не '
                                  f'было (поздний отчёт по прежним не исключён); '
                                  f'намерение снято')
...
def do_trade(ib, route, dry):
    ...
    # ПОВТОР ПОСЛЕ СМЕРТИ МЕЖДУ ST.save И touch traded-* — ШТАТНЫЙ, А НЕ АВАРИЯ (сорок
    # первый круг, №6). Книга СЕГОДНЯШНЕЙ даты с close_provisional=True означает: сессия
    # этого дня уже отработала и ждёт замыкания. Прежде такой повтор упирался в отказ «не
    # замкнута» — раньше проверки «не новее последней завершённой», по которой автопилот
    # узнаёт штатный повтор. Ветка BK2 поэтому была НЕДОСТИЖИМА в торговом окне: она ставила
    # traded-* и тут же писала ALARM-trade-*, и следующие сессии, включая РОЛЛ, стояли до
    # человека. Отказ здесь обязан нести то же узнаваемое слово, что и штатный повтор.
    if (getattr(book, 'close_provisional', False) and book.last_session
            and book.last_session == FD.exchange_today().strftime('%Y-%m-%d') and not dry):
        # НЕЗАВЕРШЁННОЕ НАМЕРЕНИЕ ПРОПУСКАЕТСЯ ВНУТРЬ (сорок третий круг, №5). Этот отказ я
        # завёл в 42-м круге, чтобы повтор после смерти между ST.save и touch читался как
        # штатный. Но смерть могла случиться и между ST.save и ST.clear_intent — тогда книга
        # тоже несёт сегодняшнюю дату, а на диске лежит НАМЕРЕНИЕ с неизвестным исходом.
        # Отказывая здесь, я делал _resume_intent (он живёт ВНУТРИ run_session) недостижимым,
        # а do_close одновременно запрещает замыкание при любом intent: получался вечный
        # тупик — намерение не разбирается, день не замыкается, следующая сессия и РОЛЛ
        # заперты до ручной хирургии. Ровно тот класс, что круг и ищет: защита, сделавшая
        # недостижимой соседнюю.
        import state as _STi
        if _STi.load_intent(bp):
            print('книга этого дня сохранена, но НАМЕРЕНИЕ не снято — вход разрешён '
                  'единственно ради разбора намерения (_resume_intent); новых заявок '
                  'решение не даст, если книга уже в цели')
        else:
            # ДЕНЬ БЕЗ ИТОГА — НЕ ШТАТНЫЙ ПОВТОР, А НЕПОЛНАЯ СЕССИЯ (СОРОК ЧЕТВЁРТЫЙ КРУГ,
            # №6). Порядок записи: ST.save -> clear_intent -> J.append(ИТОГ). Смерть ПОСЛЕ
            # clear_intent, но ДО итога оставляет книгу сегодняшней датой и журнал без
            # итоговой строки. Прежде оба пути (с намерением и без) отвечали одинаково
            # мягко: «не новее последней завершённой» — а автопилот узнаёт штатный повтор
            # ИМЕННО по этим словам, ставит traded-* и идёт дальше. Недостача обнаружилась
            # бы только на замыкании, когда день уже объявлен отторгованным: якорь WORM
            # отверг бы книгу, ALARM-backup встал бы навсегда, следующий ролл — заперт.
            # Автоматика ничего не лечит сама (О-5): дописывать итог задним числом нельзя,
            # это подмена истории. Но и молчать нельзя — отказ обязан звучать ИНАЧЕ, чтобы
            # автопилот поднял тревогу немедленно, пока день ещё разбираем.
            # ПРАВИЛО — ОДНО НА ДВА МЕСТА (J.session_incomplete): его же спрашивает якорь
            # WORM перед тем, как объявить день завершённым. Две копии разъехались бы в
            # легаси-оговорке (нулевой ИТОГ введён 42-м кругом, дата в journal.py), и тогда
            # вход отказывал бы на днях, которые якорь считает законными.
            # ЖУРНАЛ ИМПОРТИРУЕТСЯ ЗДЕСЬ (в do_trade его в области видимости нет).
            # ОШИБКА КОДА НЕ ПЕРЕОДЕВАЕТСЯ В ВЕРДИКТ (найдено собственным стендом сразу
            # после инцидента 19.08): первая редакция ловила Exception целиком, и NameError
            # 'J' превращался в «журнал непроверяем» — то есть в ОТКАЗ, который выглядел
            # разбором состояния и запрещал ЛЮБОЙ штатный повтор дня. Ошибку кода надо
            # ронять громко, а не подшивать к делу; нечитаемый журнал — законный отказ и
            # ловится своим типом.
            ...
            try:
                ...
                _incomplete = f'журнал непроверяем ({type(_ex6).__name__}: {_ex6})'
            if _incomplete:
                raise Refused(
                    f'СЕССИЯ {book.last_session} НЕПОЛНА: книга сохранена, но журнал §7 не '
                    f'закрыт итогом этой сессии (последняя запись: {_incomplete}). Обрыв '
                    f'между сохранением книги и записью итога — день НЕ отторгован до '
                    f'конца. Замыкание отвергнет книгу якорем WORM. Ручной разбор (О-5): '
                    f'дописывать итог задним числом запрещено')
            raise Refused(
                f'сессия {book.last_session} не новее последней завершённой: книга этого дня '
                f'уже сохранена и ждёт замыкания (session.py --close после 16:05 Чикаго); '
                f'повторная торговля в тот же день запрещена')
    ...
    if route == 'E':
        ...
        if not dry:
            kw['o3e_alarm_fn'] = (lambda _r, _done=True, _d=f'{m.date:%Y-%m-%d}':
                                  _alarm_o3e(_d, _r, src, done=_done))
    ...
    if route == 'E' and not dry and (bool(_cut0) or (dec.trade and orders)):
        ...
        if post_o3e_alarm(_pc, dec.book_after, _cut0):
            _day = f'{m.date:%Y-%m-%d}'
            # ТРЕВОГА ПИШЕТСЯ ОДИН РАЗ — ДОЛГОВЕЧНЫМ ПУТ�енм (тридцать пятый круг, №3).
            # Если срез состоялся, файл уже создан ИЗНУТРИ сессии, до сохранения книги и с
            # fsync. Повторная запись отсюда идёт ПОСЛЕ состояния и обычным write_text: она
            # не добавляет ничего, кроме сдвига времени записи, — то есть обесценивает
            # именно то свойство, ради которого порядок и менялся. Молча пропускать нельзя:
            # отсутствие файла при состоявшемся срезе — это сбой, а не «уже написано».
            if _cut0 and (state_dir() / f'ALARM-o3e-{_day}.txt').exists():
                print(f'тревога О-3-Е за {_day} поставлена изнутри сессии (до состояния)')
                return dec
            ...
            # ХВОСТ СООБЩЕНИЯ СТРОИТ _alarm_o3e (см. ниже про одного писателя): здесь
            # остаётся только ПРИЧИНА, а «выполнено / НЕ подтверждено» и порядок действий
            # добавляются там — одним текстом для обеих веток.
            ...
            if _cut:
                ...
                _msg = (f'{_txt} — книга сохранена, СОКРАЩЕНИЕ НЕ ВЫПОЛНЕНО (запас '
                        f'неизвестен либо книга пуста); сокращение до L=1 САМО НЕ '
                        f'ПРОИЗОЙДЁТ, при необходимости сократить вручную')
            # ОДИН ПИСАТЕЛЬ ТРЕВОГИ, А НЕ ДВА (тридцать седьмой круг, разбор непойманных
            # мутаций «пост-трейд None глотается» и «пост-трейд проверка О-3-Е удалена»).
            # Здесь стояла ВТОРАЯ, независимая запись того же файла — обычным write_text,
            # без fsync файла и каталога, которые есть у _alarm_o3e. Два следствия, оба
            # плохие: (1) на этой ветке тревога недолговечна ровно против того сбоя, ради
            # которого пишется; (2) решение о тревоге оказалось размазано по двум точкам, и
            # мутации, бившие во внешнюю, перестали что-либо менять — полный прогон честно
            # показал «НЕ ПОЙМАНА». Пишем той же функцией: одна точка отказа, один fsync,
            # одна точка наблюдения.
            _alarm_o3e(_day, _msg.rstrip('\n'), src, done=bool(_cut))
