...
def _connect(client_id):
    ...
    ib = IB()
    # ГОТОВНОСТЬ ЖДЁТ ОТДЕЛЬНЫЙ МОДУЛЬ, ПОДКЛЮЧЕНИЕ — ОДНО (перечинка 31.08.2026 по
    # рецензии: прежний слепой повтор тем же объектом и тем же clientId наследовал кэш
    # мёртвой попытки и не ловил соседние формы той же гонки — подробности в докстроке
    # gw_gotovnost). Отказ ожидания не запрещает подключаться: честная тревога лучше.
    import gw_gotovnost as GW
    GW.дождаться(HOST, PORT, бюджет_с=90)
