...
def do_trade(ib, route, dry):
    ...
    if route == 'E' and not dry and dec.trade and orders:
        ...
        if post_o3e_alarm(_pc, dec.book_after):
            ...
            _msg = (f'{_txt} — книга сохранена. ВНИМАНИЕ: под этой тревогой контур НЕ '
                    f'ТОРГУЕТ, поэтому сокращение до L=1 САМО НЕ ПРОИЗОЙДЁТ. Порядок: '
                    f'разобрать причину, при необходимости сократить вручную, затем снять '
                    f'тревогу — только после этого сессия сможет привести книгу (О-5)\n')
            try:
