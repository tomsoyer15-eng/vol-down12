...
if ! flock -n 9; then
    ...
    if [ -f "$hb" ]; then
        ...
        age=$(( $(date +%s) - $(counter_read "$hb") ))
        [ "$age" -ge 0 ] 2>/dev/null || age=0
        if [ "$(counter_read "$hb")" -eq 0 ] 2>/dev/null; then
        ...
        # ТО ЖЕ И ЗДЕСЬ: метка пишется как `date +%s > lock-nohb`, epoch лежит в содержимом.
        age=$(( $(date +%s) - $(counter_read "$nohb") ))
        [ "$age" -ge 0 ] 2>/dev/null || age=0
        if [ "$(counter_read "$nohb")" -eq 0 ] 2>/dev/null; then
        ...
        case "$_cw" in
            LOW\ *)
                ...
                if [ "$(chicago %H%M)" -lt "$(trade_till)" ]; then
                    _cut_out=$(cd "$LIVE" && timeout -k 60 900 "$PY" session.py --o3e --route E 2>&1 | grep -vE '^Error [0-9]+')
                    _cut_rc=$?
                    echo "$_cut_out" >> "$LOG"
