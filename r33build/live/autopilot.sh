...
BK2
    ...
    if [ $rc -eq 0 ] || [ "$_closed_ok" = 1 ]; then
        ...
        if backup_state "$day"; then
            touch "$ST/closed-$day"; log "замыкание $day: ок (копия состояния снята)"
        else
...
sys.stdout.write(('LOW %.3f' % c) if (c is not None and c < DL.O3E_MIN)
            ...
            case "$_cwa" in
                ...
                *)  _an="$ST/o3e-alarm-fail-$_d0"
                    _ac=$(( $( [ -s "$_an" ] && cat "$_an" || echo 0 ) + 1 ))
                    echo "$_ac" > "$_an"
                    log "вахта О-3-Е под тревогой: замер не удался ($_cwa), подряд $_ac"
                    if [ "$_ac" -ge 3 ]; then
                        alarm_write "$ST/ALARM-o3e-blind-$_d0.txt" \
                            "под тревогой запас О-3-Е не измеряется $_ac тика подряд: $_cwa — О-5" \
...
sys.stdout.write(('LOW %.3f' % c) if (c is not None and c < DL.O3E_MIN)
                ...
                case "$_cw" in
                    ...
                    *)  _hw="$ST/o3e-holiday-fail-$day"
                        _hn=$(( $( [ -s "$_hw" ] && cat "$_hw" || echo 0 ) + 1 ))
                        echo "$_hn" > "$_hw"
                        log "вахта О-3-Е (праздник): замер не удался ($_cw), подряд $_hn"
                        if [ "$_hn" -ge 3 ]; then
                            alarm_write "$ST/ALARM-o3e-blind-$day.txt" \
                                "запас О-3-Е не измеряется $_hn тика подряд в праздник: $_cw — О-5" \
                                || log "КРИТИЧНО: тревога слепой праздничной вахты не записана"
                        fi ;;
                esac
            fi
