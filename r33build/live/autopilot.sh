    ...
    if [ $rc -eq 0 ] || [ "$_closed_ok" = 1 ]; then
        ...
        if backup_state "$day"; then
            ...
            if touch "$ST/closed-$day" 2>/dev/null && [ -e "$ST/closed-$day" ]; then
                ...
                alarm_write "close-marker-$day" "замыкание $day прошло, но отметку closed-$day записать НЕ удалось — следующий тик повторил бы замыкание и WORM" || true
        ...
        if [ ! -e "$ST/sigup-$mon" ]; then
            ...
            if sout=$(cd "$LIVE" && timeout -k 30 300 "$PY" signal_update.py 2>&1 | grep -vE '^Error [0-9]+'); then
                ...
                if touch "$ST/sigup-$mon" 2>/dev/null && [ -e "$ST/sigup-$mon" ]; then
                    ...
                    alarm_write "sigup-marker-$mon" "сигнал $mon обновлён, но отметку sigup-$mon записать НЕ удалось — обновлятор повторялся бы каждым тиком" || true
                    log "КРИТИЧНО: отметка sigup-$mon не записана"
                fi
            else
