...
def _preflight_handover(from_route, to_route, _dst_names=(), _broker_p=None):
    ...
    if _broker_p is not None and hasattr(_broker_p, 'net_positions'):
        try:
            _pos_p = _broker_p.net_positions() or {}
        except Exception as _exp:
            raise RuntimeError(f'позиции брокера недоступны в предполёте ({_exp}) — книгу '
                               f'источника сверить нечем, переход запрещён')
        _oo_p = []
        if hasattr(_broker_p, 'open_orders'):
            try:
                _oo_p = list(_broker_p.open_orders() or [])
            except Exception as _exo:
                raise RuntimeError(f'открытые заявки недоступны в предполёте ({_exo}) — '
                                   f'переход запрещён')
        if _oo_p:
            raise RuntimeError(
...
def _execute_locked(broker, state_path, capital, legs, signal_id, from_route, to_route,
    ...
    # МЕТКИ СТАВЯТСЯ ДО ФИКСАЦИИ OPEN В СОСТОЯНИИ (тридцатый круг, №12). Прежде порядок
    # был обратный: OPEN в журнале и st['opened']=True, и только потом два файла-барьера.
    # Смерть процесса в этом зазоре оставляла нормативный журнал в OPEN БЕЗ маркера и без
    # тревоги, а ежедневный контур журнал МР не читает — он продолжал бы торговать старый
    # маршрут поверх начатого перехода. Маркеры дешевле: их снимает штатное завершение, а
    # лишний барьер безопаснее пропущенного.
    _mark_handover()
