...
def plan_lots(legs, capital):
    ...
    for name, spec in sorted(legs.items(), key=lambda kv: (-float(kv[1]['dst'][1]), kv[0])):
        ...
        for instr, units, unit_usd in spec['src']:
            ...
            # ГРАНИЦА НАРЕЗКИ — ТОТ ЖЕ ДОПУСК, ЧТО И У ВЫЧИТАНИЯ (разбор /code-review
            # 22.08). Здесь стояло 1e-9 при объявленном модулем TOL=1e-6, и лот величиной
            # между ними нарезался, а consume_partial его затем срезал в ноль: заявок нет,
            # но лот молча уходит в st['done'], и resume читает это как доказательство
            # выполненной работы (executed_usd при этом ноль). Две границы одного понятия
...
def unpaired_limit(legs, capital, grant_limit=None):
    ...
    if mx + h > pct:
        ...
        # РАЗРЕШЕНИЕ — ЭТО РАЗРЕШЕНИЕ, А НЕ РАБОЧИЙ ЛИМИТ (рецензия 04.09.2026). Здесь
        # стояло `return grant_limit`, то есть записанная заказчиком сумма уходила вниз
        # ДЕЙСТВУЮЩИМ потолком непарной дельты. Заказчик пишет её заранее и вслепую, с
        # запасом на движение цены: 04.09 записано 450 000 при действительной потребности
        # 390 393 — потолок оказался 45% счёта вместо 39%. А лишний ноль (4 500 000) дал бы
        # 450% счёта, и отменить его нечем: журнал только дописывается, события отзыва нет.
        # Возвращаем ПОТРЕБНОСТЬ плана: разрешение проверено выше, а работать обязан самый
        # тесный из двух, иначе запас превращается в право.
        return need
    return pct
...
def _check_dst_names(dst_names):
    """Контракт имён цели. ОТДЕЛЬНОЙ функцией (седьмой прогон, ворота 8в-1): инлайном в
...
def _preflight_handover(from_route, to_route, _dst_names=(), _broker_p=None,
    ...
    if getattr(_src, 'close_provisional', False):
        # АВАРИЯ — ИСКЛЮЧЕНИЕ, И БЕЗ НЕГО СТОРОЖ ЗАПИРАЛ ВЫХОД РОВНО В СТРЕСС
        # (сплошной аудит 27.08.2026, находка №7 реестра §5в).
        # Считано по коду, а не прикинуто: общее окно перехода 02:00-10:30 Чикаго
        # (feed.common_window), ворота замыкания Е — 10:35 (feed.e_close_gate), торговое
        # окно открывается в 08:45 (autopilot trade_from). Ежедневная сессия сохраняет
        # книгу с close_provisional=True (daily.py:2706), значит с 08:45 сторож отказывает,
        # а в 10:30 закрывается само окно — замыкание в 10:35 приходит уже поздно.
        # Итог: аварийный выход Е->Ф недоступен с 08:45 до 02:00 СЛЕДУЮЩЕГО дня, то есть
        # всю американскую сессию — ровно тогда, когда случается маржинальный стресс и
        # когда выход единственно и нужен. Соседний сторож строкой ниже такое исключение
        # уже имеет; этот его не получил.
        # ЧТО ТЕРЯЕТСЯ И ПОЧЕМУ ЭТО МЕНЬШЕЕ ЗЛО: незамкнутая книга не несёт зафиксированного
        # плеча закрытия, и нормативный триггер капа следующей сессии пропадает. При аварии
        # альтернатива — остаться в маржинальной книге под стрессом, что дороже. Потеря
        # объявляется ГРОМКО и требует разбора О-5, а не замалчивается.
        if not emergency:
            raise RuntimeError(
        ...
        print(f'ВНИМАНИЕ (АВАРИЯ): книга маршрута {from_route} НЕ ЗАМКНУТА '
              f'(close_provisional) — переход выполняется, но плечо закрытия предыдущей '
              f'сессии ТЕРЯЕТСЯ вместе с маршрутом. Триггер капа следующей сессии придётся '
              f'восстанавливать вручную (О-5).')
...
def _execute_locked(broker, state_path, capital, legs, signal_id, from_route, to_route,
    ...
    # ПОРОГ §8 ПРОВЕРЯЕТСЯ И В ОБРАТНУЮ СТОРОНУ (рецензия 04.09.2026). Ниже стоял только
    # запрет уходить в Ф при малом капитале. Но с 04.09 сигнал в Е выдаётся автоматически
    # при NLV ниже порога, а сигнал ЖИВЁТ до исполнения: одна ошибочная строка --nlv или
    # один просадочный день оставляют ожидающий переход, и подписанный тогда акт остаётся
    # действительным недели спустя — на восстановившемся счёте, где §8 прямо предпочитает Ф.
    # Проверка симметрична соседней и снимается тем же аварийным признаком.
    if to_route == 'E' and _cap_eff >= MIN_NLV_F and not emergency:
        raise Incident(
            f'NLV брокера ${_nlv:,.0f} НЕ ниже порога §8 ${MIN_NLV_F:,.0f} — маршрут Ф '
            f'доступен, и автоматический сигнал в Е выдан при другом капитале; исполнение '
            f'устаревшего сигнала запрещено, требуется новое решение заказчика')
...
def hand_over_book(broker, from_route, to_route, positions=None):
    ...
    if to_route == 'F':
        ...
        # заявки исполнены, и отказ оставил бы разорванное состояние. `except Exception:
        # _reg_keys = []` тоже неверен — он глотал ошибки кода и ВЫКЛЮЧАЛ сторожа целиком,
        # книга Ф выходила с пустыми ser_a/ser_b, leg_roll_due при held is None навсегда
        # отвечает «ролл не нужен», нога идёт в поставку.
        # Правило: нечитаемый реестр — это НЕИЗВЕСТНОСТЬ, и судим по ней консервативно.
