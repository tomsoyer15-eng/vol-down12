...
class StateCacheFailed(RuntimeError):
    """Событие УЖЕ долговечно в нормативном журнале, а производный кэш состояния — нет.

    Отдельный тип нужен ровно затем, чтобы вызывающий не спутал это с «событие не
    записалось»: журнал — источник истины, и открытый им переход обязан быть закрыт
    нормативно (ABORT/MIXED), а не молча забыт снятием файловых меток.
    """

    def __init__(self, kind, ex):
        super().__init__(f'{kind}: событие записано в журнал, кэш состояния НЕ записан ({ex})')
        self.kind = kind
        self.committed = True
...
def configure(journal_abspath):
    ...
    # СТАРОЕ РАЗВЁРТЫВАНИЕ БЕЗ sha256 — НЕ «ПРОВЕРЕНО», А НЕ ПРОВЕРЯЕМО (32-й круг, №7).
    # Поле добавлено в 31-м круге; конфигурация, созданная раньше, законно возвращает None,
    # и _verify_journal_digest молча выходит — то есть обновлённый процесс на старом
    # deploy-файле СНОВА допускает правку журнала «на месте»: одна валидная строка CSV
    # (OWNER_APPROVE или TRANSITION_COMPLETE) переводит счёт в другой маршрут. Миграция
    # обязана быть явной и дешёвой: перепровизия одной командой.
    if cfg.get('sha256') is None:
        raise JournalCorrupt(
            'конфигурация развёртывания создана до заверения содержимого журнала '
            '(нет sha256): правка журнала «на месте» была бы невидимой, а событиями '
            'этого файла разрешается переход маршрута. Перепровизируйте: '
            'python3 mr_engine.py --provision-journal АБСОЛЮТНЫЙ_ПУТЬ')
    if cfg['journal'] != j:
...
def _append_raw(journal, asof, event, detail):
    ...
    # СОБЫТИЕ УЖЕ ДОЛГОВЕЧНО К ЭТОМУ МОМЕНТУ (тридцать восьмой круг, №8). Строка выше
    # прошла fsync; отказ заверения digest — это отказ ПОСЛЕ фиксации, и вызывающий обязан
    # отличать его от «записи не было». Прежде отсюда выходило обычное исключение без
    # признака committed, обработчик _execute_locked считал его чистым отказом открытия и
    # снимал handover-метки — журнал оставался в OPEN без единственного файлового барьера
    # ежедневного контура. Тот же класс, что и отказ кэша состояния, только этажом ниже.
    try:
        _stamp_journal_digest(j)
    except BaseException as _exd:
        raise StateCacheFailed(event, _exd)
...
def run(inputs, tariff, state_path, journal, asof, nlv, emerg=None, prearranged=False, write=True):
    ...
    if emerg and decision == HOLD:
        ...
        # а этот вызов стоял голым. emerg_20d поднимает ValueError на ШТАТНОМ состоянии —
        # «аварийный файл устарел», если последняя строка старше семи дней, — то есть при
        # файле, который просто не обновляют ежедневно. Плюс FileNotFoundError и KeyError из
        # csv.DictReader. Любое из них давало traceback БЕЗ строки DECISION=: вызывающий,
        # который разбирает вывод, не находил решения вовсе, а ветка записи состояния
        # (elif write:) не исполнялась — журнал МР оставался с прошлым кварталом.
        try:
            m, g, avail = emerg_20d(emerg, asof)
        except Exception as _ex_em:
            # ВОЗВРАТ ТОТ ЖЕ, ЧТО У СОСЕДНИХ ОТКАЗОВ: INVALID, а не None. Вызывающий
            # различает исходы по этому значению, и своя константа здесь разошлась бы с
            # остальными четырьмя ветками при первой же правке.
            print(f'DECISION=INVALID (аварийный замер недоступен: {_ex_em})')
            return INVALID
...
def confirm_transition(journal, state_path, asof, target, kind='complete', tid='', sid=''):
    ...
    # СОБЫТИЕ УЖЕ В ЖУРНАЛЕ — ОБ ЭТОМ ОБЯЗАН УЗНАТЬ ВЫЗЫВАЮЩИЙ (тридцать седьмой круг, №13).
    # write_state_cache стоит ПОСЛЕ долговечной записи, и его отказ уходил наружу обычным
    # исключением. Исполнитель перехода трактовал исключение из hook('open') как ЧИСТЫЙ
    # отказ: снимал обе метки handover-inflight и не писал ни ALARM, ни ABORT. В итоге
    # нормативный журнал держал TRANSITION_OPEN, а единственный файловый барьер, который
    # читает ежедневный контур, был удалён — автопилот торговал поверх открытого перехода.
    # Кэш производен от журнала и восстановим; неотличимость «не записалось» от «не
    # случилось» — нет.
    try:
        write_state_cache(state_path, journal, d)
    except BaseException as _exc:
        raise StateCacheFailed(ev, _exc)
    return True
...
def find_grant(journal, asof, sid):
    """Действующее разрешение заказчика на превышение owner-cap: НАИБОЛЬШЕЕ из записанных
    для этого сигнала.

    БЫЛО «ПЕРВОЕ», И ЭТО ОКАЗАЛОСЬ ЗАПАДНЁЙ (04.09.2026, живой перевод пилота). Требуемый
    лимит привязан к ЦЕНЕ контракта и плавает с рынком каждый день, а разрешение
    записывается заранее и вслепую: за сутки ES вырос на 0,9%, требование стало 390 393 при
    записанном 387 026 — и переход отказал. Поправить было нечем: дописанное большее
    разрешение не читалось (возвращалось первое), новый сигнал движок не выдаёт при
    ожидающем, а ожидающий снимается только исполненным либо прерванным переходом. Одна
    заниженная цифра запирала переход НАВСЕГДА, хотя заказчик готов был поднять потолок.
    НАИБОЛЬШЕЕ — потому что GRANULARITY_EXCEPTION по смыслу ПОТОЛОК, который заказчик
    поднимает своим актом; ужесточение делается не записью меньшего числа, а отказом
    исполнять. Все записи остаются в журнале и видны аудитору: акт не подменяется, а
    дополняется. Порча ловится в ЛЮБОЙ строке сигнала, а не только в первой — прежний код
    до второй строки не доходил вовсе."""
    лучшее = None
    for d, ev, det in journal_rows(journal, date.fromisoformat(asof) if isinstance(asof, str) else asof):
