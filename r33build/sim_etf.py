...
дивидендный drag 0,25%/год на фондовую ногу (ред. 33) (физический фонд, нормативный вариант);
...
TER = 0.0007
# Норматив дивидендного drag повышен с 0,20% до 0,25% по измерению на ряде Шиллера
# (решение заказчика 11.08.2026, ред. 33). Прежняя величина была занижена.
DRAG_PHYS = 0.0025
BM_BASIS = 0.0010
...
def sim_etf(days, re, rb, rfv, spy_close, dref, st_eq, st_bd,
    ...
    for i, dte in enumerate(days):
        ...
        if bd_switch or abs(tgt_b-exp_b) > bd*e:
            new = round(tgt_b/p_b)
            if new != n_b:
