...
def _machine_snapshot():
    ...
    # ЖУРНАЛЫ И ЗАМКИ — ПО ПРАВИЛУ, А НЕ ПО СПИСКУ ИМЁН (01.09.2026, отвергнутый выпуск).
    # Список имён отставал от жизни: 13.08 в него внесли сердцебиение тика, 19.08 —
    # журнал wip-push, а 29.08 в ~/.addfut завелись ещё три расписания (страница статуса
    # каждые 30 минут, почтовый сторож каждые 10, наблюдатель по будням) плюс журналы
    # подъёма шлюза по событию. Часовой selfcheck ПО ПОСТРОЕНИЮ накрывает эти интервалы:
    # выпуск 01.09 отвергнут строкой «ИЗМЕНЕНЫ: status-stranitsa.log», то есть пакет
    # перестал собираться вовсе, а следующее имя (pochta-storozh.log, каждые 10 минут)
    # отвергло бы и следующую попытку. Чинить по одному имени за выпуск — гарантированный
    # ряд отказов, поэтому здесь названо ПРАВИЛО: журнал и замок торгового состояния не
    # несут. Оно проверяемо и узко — состояние живёт в .json/.csv/.txt, отметках дня,
    # тревогах и каталогах; ни один такой файл не кончается на .log или .lock.
    _журнал_или_замок = lambda _и: _и.endswith('.log') or _и.endswith('.lock')
    out = {}
    ...
    for _root, _dirs, _files in os.walk(_REAL_ADDFUT):
        for _n in sorted(_dirs) + sorted(_files):
            ...
            if _rel in _DISPATCH or _журнал_или_замок(_rel):
                continue
            _st = os.lstat(_fp)
...
ALLOW = ['ADD-FUT-v1_6_0-r33.pdf', 'ADD-FUT-v1_6_0-r33.txt', 'sim_v164.py', 'sim_etf.py', 'mr_engine.py', 'transition.py',
         ...
         # ОЖИДАНИЕ ПРОГРЕВА ШЛЮЗА (перечинка 31.08.2026). Модуль появился ПОСЛЕ последнего
         # выпуска и в список не попал: его импортируют session.py, signal_update.py,
         # diagnose.py, autopilot.sh, invariants.py и mutation.py, то есть распакованный
         # пакет не запускался вовсе — выпуск 01.09 отвергнут именно этим (invariants и
         # mutation в архиве падали импортом). Ровно тот класс, ради которого release.py
         # гоняет selfcheck на РАСПАКОВАННОМ архиве, а не на дереве.
         'live/gw_gotovnost.py'] + [
